...
static inline void writeByte( ubyte x ) {
	if( fputc( x, s_output ) == EOF ) exit( 32 );
}

static inline void writeShort( short x ) {
	if( fwrite( &x, 2, 1, s_output ) != 1 ) exit( 32 );
}
...
static inline void writeInt( int x ) {
	if( fwrite( &x, 4, 1, s_output ) != 1 ) exit( 32 );
}

static inline void writeString( const char *str ) {
	if( str != NULL && fputs( str, s_output ) == EOF ) exit( 32 );
	writeByte( 0 );
}
...
static void se_connected( int port ) {
	...
	if( controller ) {
		for( ushort i = 0; i < numButtons; i++ ) {
			writeByte( SDL_GameControllerGetButton( controller, (SDL_GameControllerButton)i ) );
		}
		for( ushort i = 0; i < numAxes; i++ ) {
			writeShort( SDL_GameControllerGetAxis( controller, (SDL_GameControllerAxis)i ) );
		}
	} else {
		for( ushort i = 0; i < numButtons; i++ ) {
...
int main( int argc, char *argv[] ) {
	if( argc < 2 || !argv[1] || !argv[1][0] ) return 22;

	s_output = fopen( argv[1], "wb" );
	if( !s_output ) return 2;
#ifdef _WIN32
		...
		while( SDL_PollEvent( &event ) ) {
			switch( event.type ) {
				...
				case SDL_CONTROLLERBUTTONUP:
					se_button( event.cbutton.which, event.cbutton.button, event.cbutton.state );
					break;
				case SDL_JOYBUTTONDOWN:
				case SDL_JOYBUTTONUP:
					if( SDL_GameControllerFromInstanceID( event.jbutton.which ) != NULL ) break;
					se_button( event.cbutton.which, event.cbutton.button, event.cbutton.state );
					break;
