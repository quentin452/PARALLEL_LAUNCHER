  ...
  void commit() const {
    try {
      ...
      if (dynamic_cast<const JsonWriterException *>(&exception)) {
        msg += "Failed to serialize data to JSON. Please report this "
               "bug!\nDetails: ";
...
public:
  FileCache(const fs::path &filePath, const T &defaultValue,
            T (*getter)(const Json &), void (*setter)(JsonWriter &, const T &))
    ...
    if (!fs::isRegularFileSafe(filePath)) {
      ...
      try {
        ...
        string msg =
            "Failed to load data from '"s + m_filePath.u8string() + "'.\n";
        if (dynamic_cast<const JsonReaderException *>(&exception)) {
          msg +=
              "Failed to parse JSON data. Please report this bug!\nDetails: ";
        } else if (dynamic_cast<const fs::filesystem_error *>(&exception)) {
          msg += "Filesystem Error: ";
        } else if (dynamic_cast<const std::ios_base::failure *>(&exception)) {
          msg += "I/O Error: ";
...
template <class T>
static void serializeToJsonArray(JsonWriter &writer,
...
static std::map<string, ControllerProfile> parseProfiles(InputDriver driver,
                                                         const Json &json) {
  const JArray &jsonArray = json.array();
  ...
  for (const Json &profileJson : jsonArray) {
    ControllerProfile profile =
        JsonSerializer::parse<ControllerProfile>(profileJson);
    profiles[profile.name] = profile;
  }

  for (const ControllerProfile &defaultProfile :
       DefaultProfile::getAll(driver)) {
...
static void
serializeProfiles(InputDriver driver, JsonWriter &writer,
...
static std::map<string, ControllerProfile>
parseDirectInputProfiles(const Json &json) {
  return parseProfiles(InputDriver::DirectInput, json);
}

static std::map<string, ControllerProfile>
parseJoyDevProfiles(const Json &json) {
  return parseProfiles(InputDriver::JoyDev, json);
}

static void
serializeHidProfiles(JsonWriter &writer,
                     const std::map<string, ControllerProfile> &profiles) {
  serializeProfiles(InputDriver::HID, writer, profiles);
}

static void
serializeSdlProfiles(JsonWriter &writer,
                     const std::map<string, ControllerProfile> &profiles) {
  serializeProfiles(InputDriver::SDL, writer, profiles);
}

static void serializeDirectInputProfiles(
    JsonWriter &writer, const std::map<string, ControllerProfile> &profiles) {
  serializeProfiles(InputDriver::DirectInput, writer, profiles);
}

static void
serializeJoyDevProfiles(JsonWriter &writer,
                        const std::map<string, ControllerProfile> &profiles) {
...
static void serializeInputModes(JsonWriter &writer,
...
static void serializeMappings(JsonWriter &writer,
...
static inline void insertXYButtonBindingsIfMissing(JArray &bindingsJson) {
  ...
  if (migratingFromOldJson) {
    ...
    // that index
    bindingsJson.emplace(bindingsJson.begin() + 18u,
                         Json((int64)KeyId::INVALID));
    bindingsJson.emplace(bindingsJson.begin() + 18u,
                         Json((int64)KeyId::INVALID));
  }
}
...
static std::vector<KeyId> parseHotkeys(const Json &json) {
  ...
  if (bindingsJson.size() == 43) {
    ...
    bindingsJson.emplace(
        bindingsJson.begin() + 18u,
        (int64)Hotkeys::Default[(ubyte)ControllerAction::ButtonX]);
    bindingsJson.emplace(
        bindingsJson.begin() + 19u,
        (int64)Hotkeys::Default[(ubyte)ControllerAction::ButtonY]);
  }

  if (bindingsJson.size() == 45) {
    bindingsJson.push_back(
        Json((int64)Hotkeys::Default[(ubyte)Hotkey::GrabMouse]));
  }
...
static void serializeHotkeys(JsonWriter &writer,
                             const std::vector<KeyId> &hotkeys) {
  writer.writeArrayStart();
...
static FileCache<AppSettings> &appSettingsCache() {
  static FileCache<AppSettings> s_cache = FileCache<AppSettings>(
      BaseDir::config() / _NFS("settings.cfg"), AppSettings::Default,
...
static FileCache<std::vector<RomSource>> &romSourcesCache() {
  static FileCache<std::vector<RomSource>> s_cache =
      FileCache<std::vector<RomSource>>(
          BaseDir::data() / _NFS("sources.json"), std::vector<RomSource>(),
          parseJsonArray<RomSource>, serializeToJsonArray<RomSource>);
  return s_cache;
}

static inline std::pair<string, ControllerProfile>
makeProfileEntry(InputDriver driver, ControllerType controller) {
  const ControllerProfile &profile = DefaultProfile::get(driver, controller);
  return {profile.name, profile};
}

static FileCache<std::map<string, ControllerProfile>> &
profilesCache(InputDriver driver) {
  switch (driver) {
  case InputDriver::HID: {
    static FileCache<std::map<string, ControllerProfile>> s_cache =
        FileCache<std::map<string, ControllerProfile>>(
            BaseDir::data() / _NFS("profiles-hid.json"),
            {makeProfileEntry(InputDriver::HID, ControllerType::XBox360)},
            parseHidProfiles, serializeHidProfiles);
    return s_cache;
  }
  case InputDriver::SDL: {
    static FileCache<std::map<string, ControllerProfile>> s_cache =
        FileCache<std::map<string, ControllerProfile>>(
            ...
            parseSdlProfiles, serializeSdlProfiles);
    return s_cache;
  }
  case InputDriver::DirectInput: {
    static FileCache<std::map<string, ControllerProfile>> s_cache =
        FileCache<std::map<string, ControllerProfile>>(
            ...
            {makeProfileEntry(InputDriver::DirectInput,
                              ControllerType::XBox360),
             makeProfileEntry(InputDriver::DirectInput,
                              ControllerType::Gamecube)},
            parseDirectInputProfiles, serializeDirectInputProfiles);
    return s_cache;
  }
  case InputDriver::JoyDev: {
    static FileCache<std::map<string, ControllerProfile>> s_cache =
        FileCache<std::map<string, ControllerProfile>>(
            ...
            parseJoyDevProfiles, serializeJoyDevProfiles);
    return s_cache;
  }
  default: {
    pl_assert(false);
...
static FileCache<std::map<Uuid, InputMode>> &inputModesCache() {
  static FileCache<std::map<Uuid, InputMode>> s_cache =
      FileCache<std::map<Uuid, InputMode>>(
          ...
          parseInputModes, serializeInputModes);
  return s_cache;
}
...
static FileCache<HashMap<Uuid, string>> &mappingsCache(InputDriver driver) {
  switch (driver) {
  case InputDriver::HID: {
    static FileCache<HashMap<Uuid, string>> s_cache =
        FileCache<HashMap<Uuid, string>>(
            BaseDir::data() / _NFS("devices-hid.json"), HashMap<Uuid, string>(),
            parseMappings, serializeMappings);
    return s_cache;
  }
  case InputDriver::SDL: {
    static FileCache<HashMap<Uuid, string>> s_cache =
        FileCache<HashMap<Uuid, string>>(
            BaseDir::data() / _NFS("devices-sdl.json"), HashMap<Uuid, string>(),
            parseMappings, serializeMappings);
    return s_cache;
  }
  case InputDriver::DirectInput: {
    static FileCache<HashMap<Uuid, string>> s_cache =
        FileCache<HashMap<Uuid, string>>(
            ...
            HashMap<Uuid, string>(), parseMappings, serializeMappings);
    return s_cache;
  }
  case InputDriver::JoyDev: {
    static FileCache<HashMap<Uuid, string>> s_cache =
...
static FileCache<UiState> &uiStateCache() {
  static FileCache<UiState> s_cache = FileCache<UiState>(
      BaseDir::cache() / _NFS("ui-state.json"), UiState::Default,
      JsonSerializer::parse<UiState>, JsonSerializer::serialize<UiState>);
  return s_cache;
}
...
static FileCache<std::vector<KeyId>> &hotkeysCache() {
  static FileCache<std::vector<KeyId>> s_cache = FileCache<std::vector<KeyId>>(
      ...
      std::vector<KeyId>(Hotkeys::Default,
                         Hotkeys::Default + (int)Hotkey::NUM_HOTKEYS),
      parseHotkeys, serializeHotkeys);
  return s_cache;
}

static FileCache<InstalledVersionsInfo> &installedVersionsCache() {
  static FileCache<InstalledVersionsInfo> s_cache =
      FileCache<InstalledVersionsInfo>(
          BaseDir::data() / _NFS("cores.json"), InstalledVersionsInfo::Default,
...
static FileCache<RhdcSettings> &rhdcSettingsCache() {
  static FileCache<RhdcSettings> s_cache = FileCache<RhdcSettings>(
      BaseDir::config() / _NFS("rhdc.cfg"), RhdcSettings::Default(),
...
static FileCache<string> &lastControllerProfileNameCache(InputDriver driver) {
  switch (driver) {
  case InputDriver::HID: {
    static FileCache<string> s_cache =
        FileCache<string>(BaseDir::cache() / _NFS("last_profile-hid.json"), "",
                          parseString, serializeString);
    return s_cache;
  }
  case InputDriver::SDL: {
    static FileCache<string> s_cache =
        FileCache<string>(BaseDir::cache() / _NFS("last_profile-sdl.json"), "",
                          parseString, serializeString);
    return s_cache;
  }
  case InputDriver::DirectInput: {
    static FileCache<string> s_cache =
        FileCache<string>(BaseDir::cache() / _NFS("last_profile-dinput.json"),
                          "", parseString, serializeString);
    return s_cache;
  }
  case InputDriver::JoyDev: {
    static FileCache<string> s_cache =
        FileCache<string>(BaseDir::cache() / _NFS("last_profile-joydev.json"),
                          "", parseString, serializeString);
    return s_cache;
  }
  default: {
...
const std::map<string, ControllerProfile> &
FileController::loadControllerProfiles(InputDriver driver) {
  return profilesCache(driver).get();
}

void FileController::saveControllerProfiles(
    InputDriver driver, const std::map<string, ControllerProfile> &profiles) {
  profilesCache(driver).set(profiles);
}

const HashMap<Uuid, string> &
FileController::loadControllerMappings(InputDriver driver) {
  return mappingsCache(driver).get();
}

void FileController::saveControllerMappings(
    InputDriver driver, const HashMap<Uuid, string> &mappings) {
  mappingsCache(driver).set(mappings);
}

const UiState &FileController::loadUiState() { return uiStateCache().get(); }
...
void FileController::saveInstalledVersions(
...
FileController::loadLastControllerProfile(InputDriver driver) {
...
void FileController::saveLastControllerProfile(InputDriver driver,
                                               const string &name) {
