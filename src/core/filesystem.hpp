...
inline bool existsSafe(const fs::path &path) noexcept {
  ...
  return fs::symlink_status(path, err).type() !=
             fs::file_type::file_not_found &&
         !err;
#elif defined(_WIN32)
  return fs::status(path, err).type() != fs::file_type::not_found && !err;
#else
  return fs::symlink_status(path, err).type() != fs::file_type::not_found &&
         !err;
#endif
}
...
#ifdef __APPLE__
extern bool tryCopyRecursive(const fs::path &srcDir,
