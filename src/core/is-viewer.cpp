...
void ISViewerThread::run() {
  ...
  if (!m_pipe->connect(err)) {
    if (err == std::make_error_code(std::errc::operation_canceled))
      return;
    logWarn(
        "Read error attempting to get the first byte from the IS Viewer pipe:"s +
        err.message());
    emit error(false);
    return;
  }
...
int ISViewer::start() {
  ...
  connect((ISViewerThread *)m_thread, &ISViewerThread::messageReceived, this,
          &ISViewer::messageReceived);
  connect((ISViewerThread *)m_thread, &ISViewerThread::error, this,
          &ISViewer::error);
