...
inline static void require(istream &jsonStream, char character) {
  ...
  if (c != character) {
    throw JsonReaderException("Unexpected character '"s + c + "' (expected '" +
                              character + "').");
  }
}
...
inline static char parseHexCode(istream &jsonStream) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (!(c >= '0' && c <= '9') && !(c >= 'a' && c <= 'f') &&
        !(c >= 'A' && c <= 'F'))
      throw JsonReaderException("Unexpected character '"s + c +
                                "' (expected hex digit).");
    code[i] = c;
    isNull &= (c == '0');
  }
...
static JObject parseObject(istream &jsonStream) {
  ...
  while (true) {
    ...
    if (colon != ':') {
      throw JsonReaderException("Unexpected character '"s + colon +
                                "' (expected colon).");
    }
...
static JArray parseArray(istream &jsonStream) {
  ...
  while (true) {
    ...
    if (delimiter == ']') {
      ...
      throw JsonReaderException("Unexpected character '"s + delimiter +
                                "' (expected comma or close bracket).");
    }
  }
  return jArray;
}
...
inline static Json parseNumber(istream &jsonStream) {
  ...
  while (true) {
    ...
    case NumberParseState::FirstDigit:
      state =
          (c == '0') ? NumberParseState::Delimiter : NumberParseState::Integer;
      break;
  ...
  if (state == NumberParseState::Fraction ||
      state == NumberParseState::Exponent) {
    return Json(Number::parseDouble(numberString));
  } else if (state == NumberParseState::Integer ||
             state == NumberParseState::Delimiter) {
...
inline static bool parseBoolean(istream &jsonStream) {
  ...
  if (firstChar == 't') {
    ...
    throw JsonReaderException("Unexpected character '"s + firstChar +
                              "' (expected boolean).");
}
...
Json Json::parse(istream &jsonStream) {
  ...
  default:
    ...
    throw JsonReaderException("Unexpected character '"s + token +
                              "' (expected value).");
  }
}
...
void JsonWriter::writePropertyName(const string &name) {
  ...
  if (objectState.context != ContainerType::Object ||
      !objectState.expectPropertyName) {
    throw JsonWriterException();
  }
