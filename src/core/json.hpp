...
typedef std::variant<int64, double, string, bool, JArray, JObject, JNull,
                     JUndefined>
    JsonType;
...
template <typename TT, typename TS> constexpr bool fits(TS value) {
  if constexpr (std::is_floating_point_v<TT> || std::is_floating_point_v<TS>) {
    return (value >= std::numeric_limits<TT>::min() &&
    ...
    return value >= 0 &&
           (std::make_unsigned_t<TS>)value <= std::numeric_limits<TT>::max();
  }
}

enum class ContainerType { None, Object, Array };
...
public:
  ...
  inline const JArray &array() const { return std::get<JArray>(m_value); }

  inline const JObject &object() const { return std::get<JObject>(m_value); }
  ...
  inline bool hasValue() const { return exists() && !isNull(); }
  ...
  if constexpr (std::is_enum_v<T>) {
    ...
    if (maybeLong != nullptr) {
      if (JsonInternals::fits<T>(*maybeLong)) {
        ...
        throw JsonReaderException("Number out of range of the requested type.");
    } else {
...
template <typename T> std::optional<T> Json::tryGet() const {
  ...
  if constexpr (std::is_enum_v<T>) {
    ...
    return longVal.has_value() ? std::optional<T>((T)longVal.value())
                               : std::optional<T>();
  } else if constexpr (std::is_same_v<T, bool>) {
    ...
    if (maybeLong != nullptr && JsonInternals::fits<T>(*maybeLong)) {
      ...
      const double *maybeDouble = std::get_if<double>(&m_value);
      if (maybeDouble != nullptr && *maybeDouble == std::trunc(*maybeDouble) &&
          JsonInternals::fits<T>(*maybeDouble)) {
        value = (T)*maybeDouble;
      }
    }
  } else if constexpr (std::is_floating_point_v<T>) {
...
public:
  JsonWriter(std::ostream *stream, bool humanReadable)
      : m_out(stream), m_pretty(humanReadable) {
    m_state.push({true, false, JsonInternals::ContainerType::None});
  }

  inline void writeNull() { writeRawValue("null"); }

  inline void writeBool(bool value) { writeRawValue(value ? "true" : "false"); }
  ...
  template <typename T>
  inline void writeProperty(const string &name, const T &value) {
...
template <typename T> void serialize(JsonWriter &, const T &) {
  throw JsonWriterException(
      "No JSON serializer has been registered for this type");
}
template <typename T> T parse(const Json &) {
  throw JsonReaderException(
      "No JSON deserializer has been registered for this type");
}
} // namespace JsonSerializer
