...
static bool handleCoreLibplRequest( ubyte commandId, ushort payloadSize, const ubyte *payload, const WritableFifoPipe &pipe, RomInfo &romInfo, std::error_code &err ) {
	switch( commandId ) {
		case 0x00: { // libpl_get_launcher_version
			const ushort response[5] = {
				0x0000,
				htons( 6 ),
		...
		case 0x01: { // libpl_get_sd_card_info
			...
			SdCardInfo cardInfo;
			if( !SdCardManager::tryFetch( name, cardInfo ) || cardInfo.numBlocks >= 0x3FFFFFu ) {
			...
			const ubyte header[4] = { 0x00, 0x00, (ubyte)(responseSize >> 8), (ubyte)(responseSize & 0xff) };
			if( !pipe.write( 4, header, err ) ) return false;

			const uint cardBytes = htonl( cardInfo.numBlocks * 1024 );
			if( !pipe.write( 4, &cardBytes, err ) ) return false;

			ubyte format = 0;
			switch( cardInfo.format ) {
				case SdCardFormat::FAT12: format = 12; break;
				case SdCardFormat::FAT16: format = 16; break;
				case SdCardFormat::FAT32: format = 32; break;
				default: break;
			}

			if( !pipe.write( 1, &format, err ) ) return false;
		...
		case 0x02: { // libpl_create_sd_card
			...
			if( cardInfo.name.length() < 1 || cardInfo.name.length() > 11 || cardInfo.name == "?" ) {
				const ubyte response[4] = { 0x06, 0x00, 0x00, 0x00 };
				return pipe.write( 4, response, err );
			}
			...
			const string uid( (const char*)&payload[cardInfo.name.length() + 3], (size_t)payloadSize - cardInfo.name.length() - 4 );
			if( uid.length() < 1 || uid.length() > 36 || string( uid.c_str() ).length() != uid.length() ) {
				const ubyte response[4] = { 0x06, 0x00, 0x00, 0x00 };
				return pipe.write( 4, response, err );
			}
			...
			const string sdCardPath = (BaseDir::data() / _NFS("sdcard") / fs::to_path(cardInfo.name + ".iso")).u8string();
			const ubyte header[4] = { 0x00, 0x00, (ubyte)(sdCardPath.length() >> 8), (ubyte)(sdCardPath.length() & 0xff) };
			if( !pipe.write( 4, header, err ) ) return false;
...
inline static void lplLoopImpl( ReadableFifoPipe &input, WritableFifoPipe &output, RomInfo &romInfo ) {
	std::error_code err;

	if( !input.connect( err ) ) {
		logError( "Failed to connect libpl integration to emulator: "s + err.message() );
		return;
	}
	...
	while( input.active() ) {
		...
		if( payloadSize > 0 && !input.read( (size_t)payloadSize, inputBuffer.data(), err ) ) {
			logDebug( "libpl read error: "s + err.message() );
			return;
		}
		...
		switch( header[0] ) {
			case 0x01: {
				handled = handleCoreLibplRequest( header[1], payloadSize, inputBuffer.udata(), output, romInfo, err );
				break;
			}
			case 0x02: {
				handled = handleRhdcLibplRequest( header[1], payloadSize, inputBuffer.udata(), output, err );
				break;
			}
			default: {
...
static void lplLoop( ReadableFifoPipe &input, WritableFifoPipe &output, RomInfo romInfo ) {
...
void LibplHandler::connect( const RomInfo &romInfo ) {
	m_thread = std::thread( lplLoop, std::ref( m_input ), std::ref( m_output ), romInfo );
}
