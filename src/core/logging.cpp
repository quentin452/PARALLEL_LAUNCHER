...
void Logs::write(LogLevel level, const string &message) {
  ...
  case LogLevel::Debug:
    ...
    break;
  case LogLevel::Info:
    std::cout << "[INFO] " << message << std::endl << std::flush;
    break;
  case LogLevel::Warn:
  ...
  case LogLevel::Error:
    ...
    break;
  case LogLevel::Fatal:
    std::cerr << "[FATAL] " << message << std::endl << std::flush;
    break;
  }

  QtThread::execOnUiThread([level, message]() {
    DataProvider::addLogEntry({level, Time::nowMs(), message});
  });
}
#else
void Logs::write(LogLevel level, const string &message) {
...
std::vector<LogEntry> Logs::fetch() { return DataProvider::fetchLogs(); }
