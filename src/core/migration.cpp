...
static inline void processRom(
	...
	if( romInfoTable.count( sha1 ) > 0 ) {
		...
		romInfoTable[sha1] = RomInfo{
			...
			json["widescreen"].getOrDefault( false ) ? WidescreenMode::Stretched : WidescreenMode::Off,
			false,
			false,
...
static inline void migrateInputFiles() {
	...
	if( fs::exists( BaseDir::data() / _NFS("profiles.json"), err ) ) {
		fs::rename( BaseDir::data() / _NFS("profiles.json"), BaseDir::data() / (_NFS("profiles") INPUT_DRIVER_SUFFIX ".json"), err );
	}

	if( fs::exists( BaseDir::data() / _NFS("devices.json"), err ) ) {
		fs::rename( BaseDir::data() / _NFS("devices.json"), BaseDir::data() / (_NFS("devices") INPUT_DRIVER_SUFFIX ".json"), err );
	}

	if( fs::exists( BaseDir::cache() / _NFS("last_profile.json"), err ) ) {
		fs::rename( BaseDir::cache() / _NFS("last_profile.json"), BaseDir::cache() / (_NFS("last_profile") INPUT_DRIVER_SUFFIX ".json"), err );
	}
...
static inline void updateEmulatorDefaults( AppSettings &settings ) {
	...
	if( settings.defaultParallelPlugin == GfxPlugin::Glide64 || settings.defaultParallelPlugin == GfxPlugin::Rice ) {
...
static inline void getRomInputTypes() {
	for( const RomFile &rom : DataProvider::fetchLocalRomFiles() ) {
		if( !rom.local || !fs::existsSafe( rom.path ) ) continue;
		SqlCommand cmd( R"#(
UPDATE ROMS
	SET input_types = ?2
WHERE sha1 = ?1
...
#define PC_LIB_EXT ".so"
	...
	if( fs::exists( coreDir / (_NFS("parallel_n64_libretro") PC_LIB_EXT) ) ) {
		fs::error_code err;
		fs::rename(
			coreDir / (_NFS("parallel_n64_libretro") PC_LIB_EXT),
			coreDir / (_NFS("parallel_n64_next_libretro") PC_LIB_EXT),
			err
		);
	}
...
static inline void fixMisplacedSaves() {
	...
	if( !fs::is_directory( saveDir, err ) || err ) return;
	for( const fs::directory_entry &node : fs::directory_iterator( saveDir, err ) ) {
		err.clear();
#ifdef __APPLE__
		if( node.status( err ).type() != fs::directory_file || err ) continue;
#else
		if( !node.is_directory( err ) || err ) continue;
#endif
		if( node.path().filename().u8string() == "sync" ) continue;
	...
	if( !fs::is_directory( saveDir, err ) || err ) return;
	for( [[maybe_unused]] const fs::directory_entry &node : fs::directory_iterator( saveDir, err ) ) {
		err.clear();
#ifdef __APPLE__
		if( node.status( err ).type() != fs::directory_file || err ) continue;
#else
...
ROM_PATHS INNER JOIN ROMS USING( sha1 )
	...
	if( settings.migrationState < 19 ) {
		...
		if( settings.migrationState < 11 ) {
			FileController::saveControllerProfiles( InputDriver::HID, {{ profile.name, profile }});
		}
#endif
	}
	...
	if( settings.migrationState < 26 ) {
		DataProvider::deleteGroup( "\x01""80" );
		removePatchesAsRoms();
	}
	...
	if( settings.migrationState < 32 ) {
		if( fs::isDirectorySafe( BaseDir::data() / _NFS("thumbnails") ) ) {
			fs::error_code err;
			fs::rename( BaseDir::data() / _NFS("thumbnails"), BaseDir::cache() / _NFS("thumbnails"), err );
		}
	}
	...
	if( settings.migrationState < 36 ) {
		...
		if( fs::is_directory( BaseDir::data() / _NFS("retroarch"), err ) && !err ) {
			constexpr char s_text[] =
				"Parallel Launcher now manages RetroArch by downloading appimages instead of using Flatpak.\n\n"
				"Your Flatpak installation of RetroArch that was created by an older version of Parallel Launcher "
				"can now be safely deleted to free up disk space (~3 GiB).\n\nWould you like to do that now?";

			bool deleted = false;
			if( QMessageBox::question( nullptr, "RetroArch Cleanup", s_text ) == QMessageBox::Yes ) {
				deleted = !AsyncProcess( "pkexec", { "rm", "-rf", (BaseDir::data() / "retroarch").u8string().c_str() }).join();
			}
