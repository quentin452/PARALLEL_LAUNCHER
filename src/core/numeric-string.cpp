...
#else
#define DEFINE_FLOAT_TO_STRING(type, bufferSize)                               \
  DEFINE_INT_TO_STRING(type, bufferSize)
#endif
  ...
  type Number::func(const string &str) {                                       \
    ...
    if (result.ec != std::errc()) {                                            \
      ...
      case std::errc::result_out_of_range:                                     \
        throw std::out_of_range(std::make_error_code(result.ec).message());    \
      default:                                                                 \
        throw std::logic_error(std::make_error_code(result.ec).message());     \
      }                                                                        \
    }                                                                          \
    return value;                                                              \
  }
