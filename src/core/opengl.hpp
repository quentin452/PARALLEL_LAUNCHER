...
private:
  OpenGlInfo(string &&_name, string &&_vendor, const Version &_glVersion,
             string &&_shaderVersion, HashSet<string> &&_extensions)
      : name(std::move(_name)), vendor(std::move(_vendor)),
        glVersion(_glVersion), shaderVersion(std::move(_shaderVersion)),
