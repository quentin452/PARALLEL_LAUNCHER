...
#endif
	} else return BaseDir::expandHome( path );
}
...
ubyte RetroArch::resolveUpscaling(
	...
	if( requestedUpscaling != ParallelUpscaling::Auto ) return 1 << ((ubyte)requestedUpscaling - 1);
...
static void loadConfigHelper( const fs::path &configPath, std::map<string,string> &configs ) {
	if( !fs::existsSafe( configPath ) ) return;
...
static void loadConfig( std::map<string, string> &configs ) {
	...
	loadConfigHelper( BaseDir::data() / _NFS("retroarch-core-options.cfg"), configs );
	...
	configs["video_filter_dir"] = (BaseDir::data() / "retro-data" / "filters" / "video").u8string();
	configs["video_shader_dir"] = (BaseDir::data() / "retro-data" / "shaders").u8string();
	configs["cheat_database_path"] = (BaseDir::data() / "retro-data" / "cheats").u8string();
...
static void saveConfig( [[maybe_unused]] const std::map<string,string> &configs ) {
	...
	OutputFile coreConfigFile( BaseDir::data() / _NFS("retroarch-core-options.cfg"), false );
...
static const char *s_unusedInputs[] = {
	"a",
	"x",
	"r2",
	"select"
...
static void bindInput( std::map<string,string> &configs, const string &key, const Binding &binding ) {
	...
	switch( binding.type ) {
		...
		case BindingType::HatRight:
			configs[buttonKey] = "h"s + Number::toString( binding.buttonOrAxis ) + "right";
			configs[axisKey] = "+99";
			break;
	}
}
...
static inline HashMap<Uuid,std::queue<int>> getDevicePorts( int &numMapped ) {
	const std::vector<Uuid> uuids = GamepadController::instance().getDeviceOrder();
...
static inline string getAngrylionFilteringString( AngrylionFiltering filter ) {
	switch( filter ) {
		case AngrylionFiltering::Filter: return "Filtered";
		case AngrylionFiltering::Blur: return "AA+Blur";
		case AngrylionFiltering::Dedither: return "AA+Dedither";
		case AngrylionFiltering::AntiAlias: return "AA only";
...
static inline string getMupenWidescreenResolution( ubyte windowScale ) {
	switch( windowScale ) {
		case 1: return "640x360";
		case 2: return "960x540";
		case 3: return "1280x720";
		case 4: return "1920x1080";
		case 5: return "2560x1440";
		case 6: return "2560x1440";
		case 7: return "3840x2160";
		case 8: return "3840x2160";
		case 9: return "3840x2160";
		default: return "1920x1080";
	}
}

static inline bool useDeckMode(
	WidescreenMode widescreenMode,
	const RomInfo &romInfo
) {
	if( !SteamDeck::isSteamDeck() ||
		SteamDeck::inGamingMode() ||
		(widescreenMode != WidescreenMode::Off)
	) return false;

	const RhdcSettings &rhdcSettings = FileController::loadRhdcSettings();
	if( !rhdcSettings.enableStarDisplay ) return false;

	RhdcHack hack;
	const bool isRhdcHack = DataProvider::tryFetchRhdcHackByChecksum( romInfo.sha1, &hack );
	if( !isRhdcHack ) return false;

	StarLayout layout;
	return rhdcSettings.enableDefaultLayouts || StarLayout::tryLoadLayout( hack.info.hackId, layout );
}

static inline bool use1610(
	WidescreenMode widescreenMode,
	bool fullscreen
) {
	...
	const double aspectRatio = ((double)resolution.width()) / ((double)resolution.height());
...
AsyncProcess RetroArch::launchRom(
	...
	if( widescreenMode == WidescreenMode::ViewportHack && plugin != GfxPlugin::GLideN64 && plugin != GfxPlugin::OGRE ) {
		widescreenMode = WidescreenMode::Off;
	}
	...
	if( deckMode ) {
		...
		const int maxScale = (widescreenMode == WidescreenMode::Off) ?
	...
	const string widescreenWidth = deck1610 ?
		Number::toString( windowScale * 384 ) :
		Number::toString( (int)std::round( (double)windowScale * 1280.0 / 3.0 ) );
	...
	const bool emulateFrameBuffer = romInfo.glidenFramebufferEmulation && plugin != GfxPlugin::OGRE;
	const bool emulateDepthBuffer = romInfo.glidenCorrectDepthCompare && emulateFrameBuffer;

	if( !saveGroup.empty() ) {
		configs["savefile_directory"] = (RetroArch::getBasePath() / _NFS("saves") / _NFS("sync")).u8string();
	}

	configs["systemfiles_in_content_dir"] = "false";
	configs["video_fullscreen"] = (settings.fullscreen && !deckMode) ? "true" : "false";
	...
	configs["video_smooth"] = (settings.fullscreen || widescreenMode == WidescreenMode::Stretched) ? "true" : "false";
	configs["parallel-n64-cpucore"] = configs["mupen64plus-cpucore"] = romInfo.useInterpreter ? "pure_interpreter" : "dynamic_recompiler";
	...
	configs["parallel-n64-allow-unaligned-dma"] = (romInfo.crc32 == 0x0712C306U) ? "False" : "True";
	configs["autosave_interval"] = "4";
	configs["parallel-n64-ISViewer"] = settings.enableIsViewer ? "parallel" : "silent";
	configs["parallel-n64-sdcard"] = (!romInfo.sdCard.empty() && romInfo.sdCard != "?") ? "SummerCart64" : "disabled";
	...
	configs["video_windowed_position_width"] = configs["custom_viewport_width"] = deckMode ? deckModeWidth : (isWidescreen ? widescreenWidth : width);
	configs["video_windowed_position_height"] = configs["custom_viewport_height"] = deckMode ? deckModeHeight : height;
	if( deckMode ) {
		configs["video_windowed_position_x"] = configs["video_windowed_position_y"] = "0";
	}
	configs["aspect_ratio_index"] = isWidescreen ? (deck1610 ? "2" : "1") : "22";
	configs["parallel-n64-aspectratiohint"] = isWidescreen ? "widescreen" : "normal";
	configs["parallel-n64-screensize"] = width + "x" + height;
	configs["mupen64plus-43screensize"] = width + "x" + height;
	configs["mupen64plus-169screensize"] = getMupenWidescreenResolution( windowScale );
	configs["parallel-n64-gliden64-viewport-hack"] = (widescreenMode == WidescreenMode::ViewportHack) ? (deck1610 ? "steamdeck" : "enabled") : "disabled";
	configs["parallel-n64-framerate"] = romInfo.overclockCPU ? "fullspeed" : "original";
	configs["mupen64plus-Framerate"] = romInfo.overclockCPU ? "Fullspeed" : "Original";
	...
	configs["parallel-n64-parallel-rdp-upscaling"] = Number::toString( (int)resolveUpscaling( settings.parallelUpscaling, windowScale ) ) + "x";
	configs["mupen64plus-parallel-rdp-upscaling"] = Number::toString( (int)resolveUpscaling( settings.parallelUpscaling, windowScale ) ) + "x";
	configs["parallel-n64-parallel-rdp-vi-aa"] = configs["parallel-n64-parallel-rdp-vi-bilinear"] = settings.parallelAntiAliasing ? "enabled" : "disabled";
	configs["mupen64plus-parallel-rdp-vi-aa"] = configs["mupen64plus-parallel-rdp-vi-bilinear"] = settings.parallelAntiAliasing ? "True" : "False";
	configs["parallel-n64-angrylion-vioverlay"] = configs["mupen64plus-angrylion-vioverlay"] = getAngrylionFilteringString( settings.angrylionFiltering );
	configs["parallel-n64-parallel-rdp-native-tex-rect"] = romInfo.parallelTexRectUpscaling ? "disabled" : "enabled";
	configs["mupen64plus-parallel-rdp-native-tex-rect"] = romInfo.parallelTexRectUpscaling ? "False" : "True";
	configs["parallel-n64-remove-vi-borders"] = romInfo.parallelRemoveBorders ? "enabled": "disabled";
	configs["mupen64plus-BilinearMode"] = settings.glidenThreePointFiltering ? "3point" : "standard";
	configs["parallel-n64-gliden64-BilinearMode"] = settings.glidenThreePointFiltering ? "3point" : "standard";
	configs["mupen64plus-EnableFBEmulation"] = configs["parallel-n64-gliden64-EnableFBEmulation"] = emulateFrameBuffer ? "True" : "False";
	configs["mupen64plus-EnableN64DepthCompare"] = configs["parallel-n64-gliden64-EnableN64DepthCompare"] = emulateDepthBuffer ? "Compatible" : "False";
	configs["parallel-n64-gliden64-EnableFragmentDepthWrite"] = "True";
#ifdef __APPLE__
	configs["mupen64plus-FXAA"] = configs["parallel-n64-gliden64-FXAA"] = "0";
	configs["mupen64plus-MultiSampling"] = configs["parallel-n64-gliden64-MultiSampling"] = "0";
#else
	configs["mupen64plus-FXAA"] = configs["parallel-n64-gliden64-FXAA"] = (settings.glidenAntiAliasing && emulateFrameBuffer) ? "1" : "0";
	configs["mupen64plus-MultiSampling"] = configs["parallel-n64-gliden64-MultiSampling"] = (settings.glidenAntiAliasing && !emulateFrameBuffer) ? "4" : "0";
#endif
	configs["parallel-n64-gfxplugin"] = s_plugins[(int)plugin];
	if( emulator == EmulatorCore::Mupen64plusNext && plugin == GfxPlugin::GLideN64 ) {
	...
	configs["parallel-n64-rspplugin"] = (plugin == GfxPlugin::Angrylion || plugin == GfxPlugin::ParaLLEl || (plugin == GfxPlugin::GLideN64 && romInfo.glidenParallelRsp)) ? "parallel" : "hle";
	configs["parallel-n64-gliden64-LegacySm64ToolsHacks"] = (plugin == GfxPlugin::OGRE) ? "enabled" : "disabled";
	...
	configs["parallel-n64-OverrideSaveType"] = romInfo.bigEEPROM ? "EEPROM_16KB" : "IGNORE";
	configs["input_autodetect_enable"] = "false";
	configs["game_specific_options"] = "false";
	configs["video_font_enable"] = settings.hideRetroArchNotifications ? "false" : "true";
	...
	configs["parallel-n64-rtc-savestate"] = settings.rtcSavestates ? "enabled" : "disabled";
	...
	configs["input_max_users"] = Number::toString( (int)players.size() * (multiPort ? 2 : 1) );
	...
	for( int i = 0; i < (int)players.size(); i++ ) {
		...
		if( !devicePorts[players[i].deviceUuid].empty() ) {
			const string deviceIndex = Number::toString( devicePorts.at( players[i].deviceUuid ).front() );
		...
		if( players[i].profile.rumble && romInfo.inputTypes[i] != N64InputType::MemPak ) {
		...
		for( int j = 0; j < (int)ControllerAction::NUM_INPUTS; j++ ) {
			...
			bindInput( configs, key, (vi[0] == ControllerAction::None) ? nullBinding : bindings[(ubyte)vi[0]] );
			if( multiPort ) {
				const string key = "input_player"s + pc2 + '_' + s_inputNames[j];
				bindInput( configs, key, (vi[1] == ControllerAction::None) ? nullBinding : bindings[(ubyte)vi[1]] );
			}
		}

		bindInput( configs, "input_player"s + pc1 + "_turbo"s, { BindingType::None, 0 } );
		for( size_t i = 0; i < sizeof( s_unusedInputs ) / sizeof( char* ); i++ ) {
			bindInput( configs, "input_player"s + pc1 + s_unusedInputs[i], nullBinding );
		}

		if( multiPort ) {
			bindInput( configs, "input_player"s + pc2 + "_turbo"s, { BindingType::None, 0 } );
			for( size_t i = 0; i < sizeof( s_unusedInputs ) / sizeof( char* ); i++ ) {
				bindInput( configs, "input_player"s + pc2 + s_unusedInputs[i], nullBinding );
			}
		}
	...
	if( bindSavestate ) {
		...
		bindInput( configs, "input_toggle_fast_forward", players.at( 0 ).profile.bindings[(int)ControllerAction::FastForward] );
		bindInput( configs, "input_toggle_slowmotion", players.at( 0 ).profile.bindings[(int)ControllerAction::SlowMotion] );
	} else {
	...
	if( players.size() >= 1 ) {
		configs["input_axis_threshold"] = Number::toString( players.at( 0 ).profile.axisThreshold );
	} else {
	...
	const char *const coreName = (emulator == EmulatorCore::ParallelN64) ? "parallel_n64_next_libretro" : "mupen64plus_next_libretro";
	...
	if( !saveGroup.empty() ) {
		...
		if( err ) {
			logError( "Failed to copy rom file from "s + romPath.u8string() + " to " + tempRomPath.u8string() + ": \n" + err.message() );
		}
		romPath = tempRomPath;
	}
	...
	if( !romInfo.sdCard.empty() && romInfo.sdCard != "?" ) {
		Environment::set( "PL_SD_CARD_IMAGE", (BaseDir::data() / _NFS("sdcard") / fs::to_path(romInfo.sdCard + ".iso")).u8string().c_str() );
	} else {
		Environment::clear( "PL_SD_CARD_IMAGE" );
	}

	if( inputBindingError ) {
		Environment::set( "PL_ALERT_MESSAGE", QCoreApplication::translate( "RetroArch", "[Parallel Launcher] Input driver encountered an unexpected error. Connected controllers may not be mapped correctly." ).toStdString().c_str() );
	} else {
		Environment::clear( "PL_ALERT_MESSAGE" );
	}

	Environment::set( "PL_RTC_OFFSET", Number::toString( settings.rtcOffset ).c_str() );
	...
	if( usingController && settings.inputDriver == InputDriver::SDL && SdlDriver::getSdlVersion() > Version{ 2, 0, 4 } ) {
		...
		if( !sdlLibraryPath.empty() ) {
			Environment::set( "LD_LIBRARY_PATH", fs::path( sdlLibraryPath ).parent_path().u8string().c_str() );
		}
	}
#endif
	...
	args.reserve( 8 );
#ifdef __APPLE__
	string launchBinary = (RetroArch::getExePath() / "Contents" / "MacOS" / "RetroArch").u8string();
...
#endif
	args.insert( args.end(), { "-L"s, corePath.u8string(), "--config"s, (BaseDir::data() / _NFS("retroarch.cfg")).u8string() } );
...
fs::path RetroArch::getGroupSaveFilePath( const string &groupId ) {
	fs::error_code err;
	const fs::path saveDir = RetroArch::getBasePath() / _NFS("saves") / _NFS("sync");
	fs::create_directories( saveDir, err );
...
#ifdef __APPLE__
	return fs::isRegularFileSafe( corePath ) && (AppleUtil::shouldUseArmCore() == AppleUtil::isArmLibrary( corePath.u8string().c_str() ));
#else
...
bool RetroArch::isEmulatorCoreInstalled( EmulatorCore core ) {
	switch( core ) {
		case EmulatorCore::ParallelN64: return isCoreInstalled( "parallel_n64_next_libretro" );
		case EmulatorCore::Mupen64plusNext: return isCoreInstalled( "mupen64plus_next_libretro" );
...
bool RetroArch::resetConfig() {
	...
	const fs::path coreConfigPath = BaseDir::data() / _NFS("retroarch-core-options.cfg");

	fs::error_code err;
	if( fs::existsSafe( mainConfigPath ) ) fs::remove( mainConfigPath, err );
	if( fs::existsSafe( coreConfigPath ) ) fs::remove( coreConfigPath, err );
	return !err;
}
...
void RetroArch::migrateSaves() {
	...
	if( configs.count( "savefile_directory" ) > 0 ) {
		const fs::path oldSaveDir = expand_path( fs::to_path( configs["savefile_directory"] ) );
	...
	if( configs.count( "savestate_directory" ) > 0 ) {
		const fs::path oldStateDir = expand_path( fs::to_path( configs["savestate_directory"] ) );
