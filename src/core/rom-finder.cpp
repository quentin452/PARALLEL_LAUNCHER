...
static void updateDatabase(
	std::shared_ptr<std::vector<RomFileWithGroups>> newRoms,
	...
	for( const RomFile &rom : *changedRoms ) {
		RomFile oldFile;
		if( DataProvider::tryFetchRomFile( rom.path, &oldFile ) && rom.sha1 != oldFile.sha1 ) {
...
static void checkForPlugins(
	...
	RhdcApi::getPluginSettings( checksums, [=](const std::vector<RhdcSettingHintsExt> &results){
...
static void recomputeRomData(
	...
	std::shared_ptr<HashMap<string,RomIdentifier>> romData( new HashMap<string,RomIdentifier> );
	std::shared_ptr<std::vector<RomFileWithGroups>> newRomFiles( new std::vector<RomFileWithGroups> );
	std::shared_ptr<std::vector<RomFile>> changedRomFiles( new std::vector<RomFile> );

	for( auto &[path,groups] : *newRoms ) {
		if( cancellationToken.isCancelled() ) {
			QtThread::safeAsync( [qtCallback](){
		...
		(*romData)[sha1] = {
			RomUtil::getInternalName( path ),
			RomUtil::getCrc32( path ),
...
static void analyzeChanges(
	...
	if( foundRoms->empty() && changedRoms->empty() ) {
		...
		std::thread( recomputeRomData, foundRoms, changedRoms, lostRoms, qtCallback, cancellationToken ).detach();
	}

}

static void scanRomSource(
	...
	const std::vector<fs::path> sourceRoms = FileSearch::find(
		...
		"*.[nvz]64",
#endif
		source.recursive ? source.maxDepth : (ubyte)1,
...
static void scan(
	std::function<void(RomFinder::SearchResults)> qtCallback,
	fs::path rhdcDir,
	CancellationToken cancellationToken
) {
	std::shared_ptr<HashMap<fs::path,std::set<string>>> foundRoms( new HashMap<fs::path,std::set<string>> );
...
CancellationToken RomFinder::scanAsync(
	const std::function<void(SearchResults)> &qtCallback
) {
	CancellationToken cancellationToken;
	std::thread( scan, qtCallback, FileController::loadRhdcSettings().downloadDirectory, cancellationToken ).detach();
