...
const N64InputTypes UNCHECKED_INPUT_TYPES = {
    (N64InputType)0xDE, (N64InputType)0xAD, (N64InputType)0xBE,
    (N64InputType)0xEF};
...
template <>
void JsonSerializer::serialize<RomSource>(JsonWriter &jw,
                                          const RomSource &obj) {
  jw.writeObjectStart();
...
template <> RomSource JsonSerializer::parse<RomSource>(const Json &json) {
  ...
  return RomSource{fs::to_path(json[P_FOLDER].get<string>()),
...
template <>
void JsonSerializer::serialize<fs::path>(JsonWriter &jw, const fs::path &obj) {
...
static const ushort s_kanaTable[64] = {
    0x00A0, 0x8082, 0x808C, 0x808D, 0x8081, 0x83BB, 0x83B2, 0x82A1,
    0x82A3, 0x82A5, 0x82A7, 0x82A9, 0x83A3, 0x83A5, 0x83A7, 0x8383,
    0x83BC, 0x82A2, 0x82A4, 0x82A6, 0x82A8, 0x82AA, 0x82AB, 0x82AD,
    0x82AF, 0x82B1, 0x82B3, 0x82B5, 0x82B7, 0x82B9, 0x82BB, 0x82BD,
    0x82BF, 0x8381, 0x8384, 0x8386, 0x8388, 0x838A, 0x838B, 0x838C,
    0x838D, 0x838E, 0x838F, 0x8392, 0x8395, 0x8398, 0x839B, 0x839E,
    0x839F, 0x83A0, 0x83A1, 0x83A2, 0x83A4, 0x83A6, 0x83A8, 0x83A9,
    0x83AA, 0x83AB, 0x83AC, 0x83AD, 0x83AF, 0x83B3, 0x829B, 0x829C};
...
string RomUtil::getInternalName(const fs::path romPath) {
  ...
  for (int i = 0; i < 20; i++) {
    ...
    if (c == 0x5C) {
      ...
      name += (char)0x80;
      name += (char)0xBE;
    } else if (c == 0xDE && i != 0 && (ubyte)sjsName[i - 1] > 0xA0 &&
               (ubyte)sjsName[i - 1] < 0xDE) {
      name += (char)0xE3;
      name += (char)0x82;
      name += (char)0x99;
    } else if (c == 0xDF && i != 0 && (ubyte)sjsName[i - 1] > 0xA0 &&
               (ubyte)sjsName[i - 1] < 0xDE) {
      name += (char)0xE3;
      name += (char)0x82;
      name += (char)0x9A;
...
uint RomUtil::getCrc32(const fs::path &romPath) {
  ...
  while (true) {
    ...
    if (!romFile.good()) {
      logWarn("Failed to compute CRC32 checksum for rom "s +
              romPath.u8string());
      return 0xDEADBEEFu;
    }
...
bool RomUtil::coveredBySearchPath(const fs::path &romPath) {
  ...
  for (const RomSource &source : sources) {
    if (isHidden && source.ignoreHidden)
      continue;

    if (source.recursive) {
      fs::path aPath = romPath.parent_path();
      for (ubyte i = 0; i < source.maxDepth && !aPath.empty(); i++) {
        if (aPath == source.folder)
          return true;

        if (source.ignoreHidden && aPath.stem().u8string()[0] == '.')
          break;
        if (!source.followSymlinks && fs::isSymlinkSafe(aPath))
          break;
