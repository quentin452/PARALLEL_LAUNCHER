...
static SdCardFormat getCardFormat(InputFile &file) {
  ...
  try {
    ...
    if (file.good() && std::memcmp(typeNameBuffer, "FAT32", 5) == 0)
...
std::vector<SdCardInfo> SdCardManager::fetchAll() {
  ...
  for (auto const &inode : fs::directory_iterator(directory)) {
    ...
    if (!fs::is_regular_file(inode.path(), err) || err)
      continue;
    if (inode.path().extension().string() != ".iso")
      continue;
    const uintmax_t fileSize = fs::file_size(inode.path(), err);
    if (err || fileSize > 2199023255552ull)
      continue;

    InputFile file(inode.path(), true);
    cards.push_back({inode.path().stem().u8string(), getCardFormat(file),
                     (uint)(fileSize / 1024)});
  }
...
bool SdCardManager::tryFetch(const std::string &name, SdCardInfo &info) {
  const fs::path filePath =
      BaseDir::data() / _NFS("sdcard") / fs::to_path(name + ".iso");

  fs::error_code err;
  if (!fs::is_regular_file(filePath, err) || err)
    return false;
...
AsyncProcess SdCardManager::createCardAsync(const SdCardInfo &info) {
  ...
  const fs::path filePath =
      BaseDir::data() / _NFS("sdcard") / fs::to_path(info.name + ".iso");
  if (fatType == 0) {
    // Create a sparse file
#ifdef _WIN32
    OutputFile(filePath, true).close();
    AsyncProcess makeSparseCmd("fsutil",
                               {"sparse", "setflag", filePath.u8string(), "1"});
    if (makeSparseCmd.join() != 0)
      return makeSparseCmd;
    return AsyncProcess("fsutil", {"file", "seteof", filePath.u8string(),
                                   Number::toString(info.numBlocks * 1024)});
#else
    return AsyncProcess(
        "dd", {"if=/dev/zero", "of="s + filePath.u8string(), "bs=1", "count=0",
               "seek="s + Number::toString(info.numBlocks) + "k"});
#endif
  }

  std::vector<string> args = {
      "-C", "-F", Number::toString(fatType), "-n", info.name, "-S", "512"};
  ...
  return AsyncProcess(
      (BaseDir::program().parent_path() / "Resources" / "mkfs.fat").u8string(),
      args);
#else
...
bool SdCardManager::copyCard(const std::string &source,
...
std::string SdCardManager::tryReadVolumeName(const fs::path &filePath) {
  ...
  for (int i = 0; name[i] != '\0'; i++) {
    const char c = name[i];
    if ((ubyte)c < 32 || (ubyte)c > 126 || c == '<' || c == '>' || c == ':' ||
        c == '"' || c == '/' || c == '\\' || c == '|' || c == '?' || c == '*')
      return std::string();
  }
...
bool SdCardManager::import(const fs::path &filePath, const std::string &name,
                           bool copy) {
  const fs::path dest =
      BaseDir::data() / _NFS("sdcard") / fs::to_path(name + ".iso");
  fs::error_code err;
