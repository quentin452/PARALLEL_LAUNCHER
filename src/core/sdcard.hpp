...
enum class SdCardFormat : int { FAT12 = 0, FAT16 = 1, FAT32 = 2, None = 3 };
...
public:
  MountedSdCard(const std::string &mountDirectory, const std::string &devicePath
#ifdef __linux__
                ,
                int fd
#endif
#ifdef _WIN32
                ,
                void *process, SdCardInfo &&info
#endif
                )
      : m_path(mountDirectory), m_device(devicePath),
#ifdef __linux__
        m_fd(fd),
#endif
#ifdef _WIN32
        m_process(process), m_info(info),
#endif
        m_mounted(true) {
  }
  ~MountedSdCard();
...
extern bool import(const fs::path &filePath, const std::string &name,
                   bool copy);
