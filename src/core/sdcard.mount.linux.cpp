...
static bool tryUnmount(const QDBusConnection &connection,
                       const QString &loopObjectPath, bool force) {
  QDBusMessage message = QDBusMessage::createMethodCall(
      "org.freedesktop.UDisks2", loopObjectPath,
      "org.freedesktop.UDisks2.Filesystem", "Unmount");

  message.setArguments({QVariant(QMap<QString, QVariant>{
  ...
  if (reply.type() == QDBusMessage::ErrorMessage) {
    if (!force &&
        reply.errorName() == "org.freedesktop.UDisks2.Error.DeviceBusy")
      return false;
...
static void loopDelete(const QDBusConnection &connection,
                       const QString &loopObjectPath) {
  QDBusMessage message =
      QDBusMessage::createMethodCall("org.freedesktop.UDisks2", loopObjectPath,
...
MountedSdCard::~MountedSdCard() {
  ...
  if (m_mounted)
    tryUnmount(connection, QString::fromStdString(m_device), true);
  loopDelete(connection, QString::fromStdString(m_device));
  ::close(m_fd);
}

// The modifications that need to be done to the SD card images to make them
// work on Windows can cause them to break here. Check if Windows modifications
// are present, and if so, try mounting with them removed
static std::shared_ptr<MountedSdCard>
retryMountWithoutWindowsPatch(const std::string &name) {
  const fs::path imagePath =
      BaseDir::data() / "sdcard" / fs::to_path(name + ".iso");

  std::fstream image(imagePath, std::ios_base::in | std::ios_base::out |
                                    std::ios_base::binary);

  char windowsPatch[16];
  image.seekg(0x1BE, std::ios_base::beg);
  image.read(windowsPatch, 16);
  if (!image.good() || windowsPatch[0] != (char)0x80)
    return nullptr;
...
std::shared_ptr<MountedSdCard> SdCardManager::mount(const std::string &name) {
  ...
  const QDBusObjectPath block =
      reply.arguments().at(0).value<QDBusObjectPath>();

  message = QDBusMessage::createMethodCall(
      "org.freedesktop.UDisks2", block.path(),
