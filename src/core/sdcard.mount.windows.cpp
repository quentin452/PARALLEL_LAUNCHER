...
static std::string getErrorMessage(DWORD errorCode) {
  ...
  const size_t messageSize = FormatMessageA(
      ...
      nullptr, errorCode, LANG_USER_DEFAULT, (char *)&buffer, 0, nullptr);
...
static bool removeVhdFooter(const fs::path &filePath, const SdCardInfo &info,
                            bool rename) {
  const uintmax_t originalSize = (uintmax_t)info.numBlocks * 1024;

  HANDLE fileHandle =
      CreateFileW(filePath.wstring().c_str(), GENERIC_READ | GENERIC_WRITE,
  ...
  if (!SetEndOfFile(fileHandle)) {
    ...
    logWarn("Failed to remove VHD footer from SD Card image. SD Card image may "
            "now be corrupt.");
    return false;
  }

  // Work around a Windows bug that prevents disk images from being mounted if
  // file compression is enabled
  ushort compressionMode = COMPRESSION_FORMAT_NONE;
  DWORD scratch;
  DeviceIoControl(fileHandle, FSCTL_SET_COMPRESSION, (void *)&compressionMode,
                  sizeof(ushort), nullptr, 0, &scratch, nullptr);

  CloseHandle(fileHandle);

  if (rename) {
    const fs::path newPath =
        filePath.parent_path() / (filePath.stem().wstring() + L".iso");
...
bool MountedSdCard::unmount() {
  ...
  if (m_process) {
    ...
    ubyte b = 0;
    std::error_code err;
    if (!proc->procIn.write(1, &b, err) || !proc->procOut.read(1, &b, err) ||
        b != 0)
      return false;
...
static inline SdCardFormat getFilesystemFormat(std::fstream &image) {
  ...
  if (std::memcmp(typeNameBuffer, "FAT12", 5) == 0)
    return SdCardFormat::FAT12;
  if (std::memcmp(typeNameBuffer, "FAT16", 5) == 0)
    return SdCardFormat::FAT16;

  image.seekg(0x52, std::ios_base::beg);
  image.read(typeNameBuffer, 5);

  if (image.good() && std::memcmp(typeNameBuffer, "FAT32", 5) == 0)
    return SdCardFormat::FAT32;
  return SdCardFormat::None;
}

static bool writeVhdFooter(fs::path &filePath, SdCardInfo &info) {
  std::fstream image(filePath, std::ios::binary | std::ios::in | std::ios::out |
                                   std::ios::ate);
  ...
  const uintmax_t numCylinders =
      1 + ((fileSize - 1) / (uintmax_t)512 / (uintmax_t)numHeads /
           (uintmax_t)sectorsPerTrack);
  if (numCylinders > (uintmax_t)0xFFFF) {
    logWarn("Sorry, but Windows cannot mount this type of disk if it is larger "
            "than 2032 GiB.");
    return false;
  }
  ...
  if (image.get() != (char)0x80) {
    ...
    case SdCardFormat::FAT32:
      ...
      break;
    default:
      image.put((numSectors < (uint)numHeads * (uint)sectorsPerTrack * 1024)
                    ? 0x06
                    : 0x0E);
      break;
    }
    if (numSectors >= (uint)sectorsPerTrack * (uint)numHeads * 1024) {
      ...
      image.put((
          char)(ubyte)(((1 + (uint)numSectors % (uint)sectorsPerTrack) & 0x3F) |
                       (((numSectors /
                          ((uint)numHeads * (uint)sectorsPerTrack)) >>
                         8)
                        << 6)));
      image.put((
          char)(ubyte)((numSectors / ((uint)numHeads * (uint)sectorsPerTrack)) &
                       0xFF));
    }
  ...
  const ubyte data1[] = {0x00, 0x00, 0x00, 0x02, 0x00, 0x01, 0x00, 0x00,
                         0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
  image.write((const char *)data1, 16);

  const auto lastModified = fs::last_write_time(filePath, err);
  const int64 time =
      (err ? Time::now() : Time::toUnixTime(lastModified)) - 946684800ull;
  ...
  const fs::path newPath =
      filePath.parent_path() / (filePath.stem().wstring() + L".vhd");
  fs::remove(newPath, err);
...
std::shared_ptr<MountedSdCard> SdCardManager::mount(const std::string &name) {
  ...
  SdCardInfo info;
  if (!writeVhdFooter(imagePath, info))
    return nullptr;
  ...
  static const std::wstring procPath =
      (extensionDir / L"mount-sd-card.exe").wstring();
  proc->args = Unicode::toUtf16(proc->procIn.filePath() + ' ' +
                                proc->procOut.filePath());
  ...
  if (!ShellExecuteExW(&proc->shell) || !proc->shell.hProcess) {
    logWarn("Failed to launch mount-sd-card.exe helper program: "s +
            getLastErrorMessage());
    delete proc;
    removeVhdFooter(imagePath, info, true);
    return nullptr;
  ...
  ubyte status;
  if (!proc->procOut.connect(err) || !proc->procIn.connect(err) ||
      !proc->procIn.write(sizeof(ushort), &pathLenBE, err) ||
      !proc->procIn.write((uintmax_t)pathLen * sizeof(wchar_t),
                          imagePath.wstring().c_str(), err) ||
      !proc->procOut.read(1, &status, err)) {
    logWarn("I/O error communicating with mount-sd-card.exe helper program: "s +
            err.message());
    proc->procIn.cancel();
    proc->procOut.cancel();
    TerminateProcess(proc->shell.hProcess, 0);
  ...
  if (status != 0) {
    ...
    case 1:
      errorMsg =
          "Failed to open virtual SD card: "s + getErrorMessage(errorCode);
      break;
    case 2:
      errorMsg =
          "Failed to mount virtual SD card: "s + getErrorMessage(errorCode);
      break;
    case 3:
      errorMsg = "Windows either could not find any volumes on the SD card "
                 "image or it failed to assign a drive letter.";
      break;
    default:
      errorMsg =
          "I/O error communicating with mount-sd-card.exe helper program.";
      break;
    }
  ...
  return std::make_shared<MountedSdCard>(
      ""s + driveLetter + ":", imagePath.u8string(), proc, std::move(info));
}
...
static inline int64 getNumericSuffix(const string &name, size_t &baseLength) {
  ...
  if (start++ >= end++)
    return 0;
  if (end - start > 1 && name[start] == '0')
    return 0;

  for (size_t i = start; i < end; i++) {
    if ((ubyte)name[i] < (ubyte)'0' || (ubyte)name[i] > (ubyte)'9')
      return 0;
  }
...
void SdCardManager::recoverSdCards() {
  ...
  for (const fs::directory_entry &node :
       fs::directory_iterator(BaseDir::data() / L"sdcard", err)) {
    err.clear();
    if (!node.is_regular_file(err) || err)
      continue;
    if (node.path().extension().u8string() != ".vhd")
      continue;
    ...
    if (!removeVhdFooter(node.path(), info, false))
      continue;

    err.clear();
    while (!err) {
      const fs::path target =
          BaseDir::data() / L"sdcard" / fs::to_path(info.name + ".iso");
      ...
      const string suffix =
          "~"s + Number::toString(getNumericSuffix(info.name, baseLength) + 1);
      info.name =
          info.name.substr(0, min(baseLength, (size_t)11 - suffix.length())) +
          suffix;
    }
  }
}
