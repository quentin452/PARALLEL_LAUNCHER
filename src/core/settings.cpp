...
static constexpr char P_PARALLEL_UPSCALE_TEXRECT[] = "parallel_upscale_texrects";
...
static constexpr char P_IGNORE_GC_WARN[] = "ignore_gamecube_controller_binding_warnings";
...
template<> AppSettings JsonSerializer::parse<AppSettings>( const Json &json ) {
	return AppSettings{
		...
		json[P_PARALLEL_UPSCALING].getOrDefault<ParallelUpscaling>( AppSettings::Default.parallelUpscaling ),
		json[P_PARALLEL_AA].getOrDefault<bool>( AppSettings::Default.parallelAntiAliasing ),
		json[P_PARALLEL_UPSCALE_TEXRECT].getOrDefault<bool>( AppSettings::Default.parallelTexRectUpscaling ),
		json[P_PARALLEL_REMOVE_BORDERS].getOrDefault<bool>( AppSettings::Default.parallelRemoveBorders ),
		json[P_ANGRYLION_FILTERING].getOrDefault<AngrylionFiltering>( AppSettings::Default.angrylionFiltering ),
		json[P_GLIDEN_AA].getOrDefault<bool>( AppSettings::Default.glidenAntiAliasing ),
		json[P_GLIDEN_3POINT].getOrDefault<bool>( AppSettings::Default.glidenThreePointFiltering ),
		json[P_GLIDEN_FIX_DEPTH].getOrDefault<bool>( AppSettings::Default.glidenCorrectDepthCompare ),
		json[P_GLIDEN_EMU_FRAMEBUF].getOrDefault<bool>( AppSettings::Default.glidenFramebufferEmulation ),
		SteamDeck::isSteamDeck() ? (ubyte)4 : json[P_WINDOW_SCALE].getOrDefault<ubyte>( AppSettings::Default.windowScale ),
		json[P_VSYNC].getOrDefault<bool>( AppSettings::Default.vsync ),
		SteamDeck::isSteamDeck() ? false : json[P_HIDE_WHILE_PLAYING].getOrDefault<bool>( AppSettings::Default.hideWhenPlaying ),
		...
		json[P_CHECK_FOR_UPDATES].getOrDefault<bool>( AppSettings::Default.checkForUpdates ),
		json[P_DARK_MODE].getOrDefault<bool>( AppSettings::Default.darkMode ),
#endif
		json[P_PATCH_TO_SAME_FOLDER].getOrDefault<bool>( AppSettings::Default.patchToSameFolder ),
		fs::to_path( json[P_PATCHED_ROM_FOLDER].getOrDefault<string>( AppSettings::Default.patchedRomFolder.u8string() ) ),
		json[P_MIGRATION_STATE].getOrDefault<short>( 0 ),
		json[P_CORE_UPDATE_INTERVAL].getOrDefault<CoreUpdateInterval>( AppSettings::Default.coreUpdateInterval ),
		json[P_MUPEN_DEV].getOrDefault<bool>( AppSettings::Default.mupenDevBranch ),
		json[P_PAUSE_ON_BLUR].getOrDefault<bool>( AppSettings::Default.pauseOnBlur ),
		json[P_LOCALE].getOrDefault<string>( AppSettings::Default.locale ),
		json[P_DISCORD_INTEGRATION].getOrDefault<bool>( AppSettings::Default.discordIntegration ),
		JsonSerializer::parse<InputDriver>( json[P_INPUT_DRIVER] ),
#ifdef _WIN32
		json[P_SDL_RAWINPUT].getOrDefault<bool>( AppSettings::Default.sdlRawInput ),
#endif
		json[P_HIDE_NOTIFICATIONS].getOrDefault<bool>( AppSettings::Default.hideRetroArchNotifications ),
		json[P_SHOW_ADVANCED].getOrDefault<bool>( AppSettings::Default.showAdvancedSettings ),
		json[P_SEEN_RHDC_VIEW].getOrDefault<bool>( AppSettings::Default.knowsAboutRhdcView ),
		json[P_IGNORE_GC_WARN].getOrDefault<bool>( AppSettings::Default.ignoreGamecubeWarnings ),
		SteamDeck::isSteamDeck() ? true : json[P_FULLSCREEN].getOrDefault<bool>( AppSettings::Default.fullscreen ),
#ifdef _WIN32
		json[P_AUDIO_DRIVER].getOrDefault<AudioDriver>( AppSettings::Default.audioDriver ),
#endif
#if defined(__APPLE__)
		json[P_FORCE_X86].getOrDefault<bool>( AppSettings::Default.forceX86 ),
#endif
		json[P_IS_VIEWER].getOrDefault<bool>( AppSettings::Default.enableIsViewer ),
		json[P_IS_VIEWER_HISTORY].getOrDefault<int>( AppSettings::Default.isViewerHistorySize ),
		...
		json[P_RTC_SAVESTATE].getOrDefault<bool>( AppSettings::Default.rtcSavestates )
	};
}
