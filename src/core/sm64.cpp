...
const char *const SM64::CourseNames[25] = {"Bob-Omb Battlefield",
                                           "Whomp's Fortress",
                                           "Jolly Rodger Bay",
                                           "Cool Cool Mountain",
                                           "Big Boo's Haunt",
...
static Checksum getChecksum(const ubyte *data) noexcept {
  ...
  return Checksum{(ubyte)(checksum >> 8), (ubyte)(checksum & 0xFF)};
}
...
static inline void readSaveSlot(std::istream &saveFile, SaveSlot &slot) {
  ...
  slot.flags = (SaveFileFlag)(((uint)(data[9] & 0x1F) << 16) |
                              ((uint)data[10] << 8) | (uint)data[11]);
...
bool SM64::tryGetPath(fs::path &path) {
  return DataProvider::tryFetchRomPathWithSha1(
      "9bef1128717f958171a4afac3ed78ee2bb4e86ce", path);
}
