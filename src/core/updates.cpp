...
template<> void JsonSerializer::serialize<MupenCoreVersion>( JsonWriter &jw, const MupenCoreVersion &obj ) {
	jw.writeObjectStart();
...
template<> MupenCoreVersion JsonSerializer::parse<MupenCoreVersion>( const Json &json ) {
	return MupenCoreVersion {
...
void ParallelCoreBuilds::getLatest(
	...
	getAsync(
		web,
		"https://parallel-launcher.ca/latest/paralleln64/version.json",
		[=]( const Json &body ) {
			try {
				const ParallelCoreVersion version = JsonSerializer::parse<ParallelCoreVersion>( body );
				onSuccess( version );
				web->deleteLater();
			} catch( ... ) {
				web->deleteLater();
