...
static const regex VersionRegex("(\\d{1,5}).(\\d{1,5}).(\\d{1,5})",
                                regex_constants::ECMAScript |
                                    regex_constants::optimize);

string Version::toString() const {
  return Number::toString(major) + "." + Number::toString(minor) + "." +
         Number::toString(patch);
}

bool Version::tryParse(const string &versionString, Version &result) {
  smatch matches;
  if (!regex_search(versionString, matches, VersionRegex) ||
      matches.size() != 4) {
    return false;
  }
...
template <>
void JsonSerializer::serialize<Version>(JsonWriter &jw, const Version &obj) {
