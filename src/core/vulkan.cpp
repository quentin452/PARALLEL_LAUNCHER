...
static inline bool tryGetVulkanApi(VulkanApi &api) noexcept {
  PFN_vkGetInstanceProcAddr loader =
      (PFN_vkGetInstanceProcAddr)getVulkanApiLoader();
  if (!loader)
    return false;

  const PFN_vkCreateInstance instanceFactory =
      (PFN_vkCreateInstance)loader(nullptr, "vkCreateInstance");
  if (!instanceFactory)
    return false;
  ...
  api.vkEnumeratePhysicalDevices = (PFN_vkEnumeratePhysicalDevices)loader(
      api.instance, "vkEnumeratePhysicalDevices");
  api.vkGetPhysicalDeviceProperties = (PFN_vkGetPhysicalDeviceProperties)loader(
      api.instance, "vkGetPhysicalDeviceProperties");
  api.vkEnumerateInstanceExtensionProperties =
      (PFN_vkEnumerateInstanceExtensionProperties)loader(
          api.instance, "vkEnumerateInstanceExtensionProperties");
  api.vkEnumerateDeviceExtensionProperties =
      (PFN_vkEnumerateDeviceExtensionProperties)loader(
          api.instance, "vkEnumerateDeviceExtensionProperties");

  return api.vkEnumeratePhysicalDevices && api.vkGetPhysicalDeviceProperties &&
         api.vkEnumerateInstanceExtensionProperties &&
         api.vkEnumerateDeviceExtensionProperties;
}
...
VulkanInfo VulkanInfo::init() {
  ...
  if (!tryGetVulkanApi(api)) {
    return VulkanInfo(false, HashSet<string>(),
                      std::vector<VulkanDeviceInfo>());
  }

  uint32_t numApiExtensions;
  HashSet<string> apiExtensions;

  if (api.vkEnumerateInstanceExtensionProperties(nullptr, &numApiExtensions,
                                                 nullptr) == VK_SUCCESS) {
    std::vector<VkExtensionProperties> extensions(numApiExtensions);
    if (api.vkEnumerateInstanceExtensionProperties(
            nullptr, &numApiExtensions, extensions.data()) == VK_SUCCESS) {
  ...
  uint32_t numGpus;
  if (api.vkEnumeratePhysicalDevices(api.instance, &numGpus, nullptr) !=
      VK_SUCCESS) {
    return VulkanInfo(true, std::move(apiExtensions),
                      std::vector<VulkanDeviceInfo>());
  }

  std::vector<VkPhysicalDevice> gpus(numGpus);
  if (api.vkEnumeratePhysicalDevices(api.instance, &numGpus, gpus.data()) !=
      VK_SUCCESS) {
    return VulkanInfo(true, std::move(apiExtensions),
                      std::vector<VulkanDeviceInfo>());
  }
  ...
  for (uint32_t i = 0; i < numGpus; i++) {
    ...
    info.apiVersion = Version{(ushort)VK_API_VERSION_MAJOR(props.apiVersion),
    ...
    if (api.vkEnumerateDeviceExtensionProperties(gpu, nullptr, &numExtensions,
                                                 nullptr) == VK_SUCCESS) {
      std::vector<VkExtensionProperties> extensions(numExtensions);
      if (api.vkEnumerateDeviceExtensionProperties(
              gpu, nullptr, &numExtensions, extensions.data()) == VK_SUCCESS) {
...
void VulkanInfo::serialize(
    ...
    const std::initializer_list<const char *> &relevantDeviceExtensions) const {
  if (!hasVulkanApi) {
    jw.writeNull();
    return;
  }
