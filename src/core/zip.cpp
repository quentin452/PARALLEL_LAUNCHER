...
bool Zip::unzip(const fs::path &zipFilePath, const fs::path &outputDir) {
  mz_zip_archive zipFile;
  std::memset(&zipFile, 0, sizeof(mz_zip_archive));

  if (!fs::exists(zipFilePath)) {
    logError("Unzip failed because the zip file does not exist: "s +
             zipFilePath.u8string());
    return false;
  }

  if (!mz_zip_reader_init_file(&zipFile, zipFilePath.u8string().c_str(), 0)) {
    logError("Unzip failed. Is this file actually a zip archive? "s +
             zipFilePath.u8string());
    return false;
  }
  ...
  for (mz_uint i = 0; i < numFiles; i++) {
    if (!mz_zip_reader_file_stat(&zipFile, i, &fileInfo) ||
    ...
    if (!mz_zip_reader_extract_to_file(&zipFile, i,
                                       extractPath.u8string().c_str(), 0)) {
      logError("Unzip failed. Failed to extract '"s + extractPath.u8string() +
               "' from zip file: "s + zipFilePath.u8string());
      mz_zip_reader_end(&zipFile);
      return false;
    }
  }
