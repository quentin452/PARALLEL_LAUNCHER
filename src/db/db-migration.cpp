...
FROM RHDC_HACK_VERSIONS
	...
	SqlCommand( "DROP TABLE RHDC_HACK_VERSIONS" ).execNonQuery();
	SqlCommand( "ALTER TABLE RHDC_HACK_VERSIONS_v2 RENAME TO RHDC_HACK_VERSIONS" ).execNonQuery();
	SqlCommand( "CREATE INDEX ix_hack_versions_by_id ON RHDC_HACK_VERSIONS( id )" ).execNonQuery();
...
WHERE name = 'sd_card'
	...
	SqlCommand( "ALTER TABLE ROMS ADD COLUMN sd_card TEXT NOT NULL DEFAULT( '' )" ).execNonQuery();
}
...
WHERE name = 'gliden_lle_rsp'
	...
	SqlCommand( "ALTER TABLE ROMS ADD COLUMN gliden_lle_rsp INTEGER NOT NULL DEFAULT( 0 )" ).execNonQuery();
}
