...
public:
  ...
  inline bool good() const noexcept {
    return m_status == SQLITE_DONE || m_status == SQLITE_ROW ||
           m_status == SQLITE_OK;
  }
  ...
  inline fs::path getPath(int index) const {
    return fs::to_path(getString(index));
  }
  ...
  inline N64InputTypes getInputTypes(int index) const noexcept {
    const N64InputType *data =
        (const N64InputType *)sqlite3_column_blob(m_statement, index);
...
public:
  ...
  inline int addParameter(const char *value, bool copy) noexcept {
    return sqlite3_bind_text(m_statement, ++m_bindingIndex, value, -1,
                             copy ? SQLITE_TRANSIENT : SQLITE_STATIC);
  }

  inline int addParameter(const string &value, bool copy) noexcept {
    return sqlite3_bind_text(m_statement, ++m_bindingIndex, value.c_str(),
                             (int)value.length(),
                             copy ? SQLITE_TRANSIENT : SQLITE_STATIC);
  }

  inline int addParameter(const Uuid &value, bool copy) noexcept {
    return sqlite3_bind_blob(m_statement, ++m_bindingIndex,
                             (const void *)value.data(), 16,
                             copy ? SQLITE_TRANSIENT : SQLITE_STATIC);
  }

  inline int addParameter(const N64InputTypes &value, bool copy) noexcept {
    return sqlite3_bind_blob(m_statement, ++m_bindingIndex,
                             (const void *)value.data(), sizeof(N64InputTypes),
                             copy ? SQLITE_TRANSIENT : SQLITE_STATIC);
  }

  inline int addParameter(bool value) noexcept {
    return addParameter((int)value);
  }
  inline int addParameter(ubyte value) noexcept {
    return addParameter((int)value);
  }
  inline int addParameter(ushort value) noexcept {
    return addParameter((int)value);
  }
  inline int addParameter(uint value) noexcept {
    return addParameter((int64)value);
  }

  inline int addParameter(uint64 value) noexcept {
    return addParameter(reinterpret_cast<int64 &>(value));
  }
};
...
public:
  ...
  inline void addParameter(const char *value, bool copy) noexcept {
    m_bindingIndex++;
    for (sqlite3_stmt *statement : m_statements) {
      sqlite3_bind_text(statement, m_bindingIndex, value, -1,
                        copy ? SQLITE_TRANSIENT : SQLITE_STATIC);
    }
  }
  ...
  inline void addParameter(const string &value, bool copy) noexcept {
    m_bindingIndex++;
    for (sqlite3_stmt *statement : m_statements) {
      sqlite3_bind_text(statement, m_bindingIndex, value.c_str(),
                        (int)value.length(),
                        copy ? SQLITE_TRANSIENT : SQLITE_STATIC);
    }
  }
  ...
  inline void addParameter(const Uuid &value, bool copy) noexcept {
    m_bindingIndex++;
    for (sqlite3_stmt *statement : m_statements) {
      sqlite3_bind_blob(statement, m_bindingIndex, (const void *)value.data(),
                        16, copy ? SQLITE_TRANSIENT : SQLITE_STATIC);
    }
  }
  ...
  inline void addParameter(uint64 value) noexcept {
    addParameter(reinterpret_cast<int64 &>(value));
  }
};
