...
template<typename T> static inline bool tryRead( ReadableFifoPipe &pipe, T &dest ) {
	std::error_code err;
...
template<> inline bool tryRead<Uuid>( ReadableFifoPipe &pipe, Uuid &dest ) {
	std::error_code err;
	ubyte data[16];
	if( !pipe.read( 16, data, err ) ) return false;
	dest = Uuid( data );
	return true;
}

SdlDriver::SdlDriver( const GamepadEventEmitter &eventEmitter ) : IGamepadDriver( eventEmitter ) {
...
void SdlDriver::start() {
	...
	*m_running = true;
	...
	if( m_sync ) args.push_back( "--sync" );

	m_gamepads.clear();
	try {
		m_proxy = AsyncProcess( (BaseDir::share() / (_NFS("parallel-launcher-sdl-relay") EXE_EXTENSION)).u8string(), args );
	} catch( ... ) {
		*m_running = false;
		m_pipe.close();
		throw;
	...
	m_runner = std::thread( SdlDriver::asyncRunner, this, m_running, m_sessionNumber );
}
...
bool SdlDriver::isRunning() const {
	return *m_running;
}
...
GamepadState SdlDriver::getState( GamepadId id ) const {
	...
	if( gamepad == m_gamepads.end() ) return GamepadState{};
	return gamepad->second.state;
}

void SdlDriver::asyncRunner( SdlDriver *self, std::shared_ptr<bool> running, uint sessionNumber ) {
	std::error_code err;
	while( *running && sessionNumber == self->m_sessionNumber ) {
		ubyte eventType;
		if( !self->m_pipe.read( 1, &eventType, err ) ) break;
		if( !handleEvent( self, running, sessionNumber, eventType ) ) break;		
	}
...
bool SdlDriver::handleEvent( SdlDriver *self, std::shared_ptr<bool> running, uint sessionNumber, ubyte eventType ) {
	std::error_code err;
	switch( eventType ) {
		case SPE_CONNECT: {
			...
			for( ubyte i = 0; i < event.info.numButtons; i++ ) {
				ubyte buttonState;
				if( !tryRead( self->m_pipe, buttonState ) ) return false;
			...
			for( ubyte i = 0; i < event.info.numAxes; i++ ) {
				short axisState;
				if( !tryRead( self->m_pipe, axisState ) ) return false;
				state.axes.push_back( (double)axisState / (axisState >= 0 ? (double)0x7FFF : (double)0x8000) );
			}

			state.hats.reserve( event.info.numHats );
			for( ubyte i = 0; i < event.info.numHats; i++ ) {
				ubyte hatState;
				if( !tryRead( self->m_pipe, hatState ) ) return false;
			...
			if( self->m_sync ) {
				...
				QtThread::safeAsync( [=](){
					if( !*running || self->m_sessionNumber != sessionNumber ) return;
		...
		case SPE_DISCONNECT: {
			...
			if( !tryRead( self->m_pipe, event.id ) ) return false;
			QtThread::safeAsync( [=](){
				if( !*running || self->m_sessionNumber != sessionNumber ) return;
		...
		case SPE_BUTTON: {
			...
			ubyte state;
			if( !tryRead( self->m_pipe, event.id ) || !tryRead( self->m_pipe, event.button ) || !tryRead( self->m_pipe, state ) ) return false;
			event.isPressed = (state > 0);
			QtThread::safeAsync( [=](){
				if( !*running || self->m_sessionNumber != sessionNumber ) return;
				if( !self->m_gamepads.count( event.id ) ) return;
		...
		case SPE_AXIS: {
			...
			short state;
			if( !tryRead( self->m_pipe, event.id ) || !tryRead( self->m_pipe, event.axis ) || !tryRead( self->m_pipe, state ) ) return false;
			event.position = (double)state / (state >= 0 ? (double)0x7FFF : (double)0x8000);
			QtThread::safeAsync( [=](){
				if( !*running || self->m_sessionNumber != sessionNumber ) return;
				if( !self->m_gamepads.count( event.id ) ) return;
		...
		case SPE_HAT: {
			...
			ubyte state;
			if( !tryRead( self->m_pipe, event.id ) || !tryRead( self->m_pipe, event.hat ) || !tryRead( self->m_pipe, state ) ) return false;
...
std::string SdlDriver::getSdlLibraryPath() noexcept {
	Dl_info dlInfo;
	if( !dladdr( (const void*)SDL_JoystickOpen, &dlInfo ) ) return std::string();
	if( !dlInfo.dli_fname ) return std::string();
