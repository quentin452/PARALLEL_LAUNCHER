...
#define REGISTER_DRIVER( driverClass ) s_drivers.push_back( std::unique_ptr<IGamepadDriver>( new driverClass( eventEmitter ) ) );
...
void GamepadController::onGamepadConnected( GamepadConnectedEvent &event ) {
	if(
		event.info.controllerId.vendorId == 0x28DE &&
		...
		event.info.controllerId.vendorId == 0 &&
...
void GamepadController::onGamepadDisconnected( const GamepadDisconnectedEvent &event ) {
	instance().fireGamepadDisconnected( event );
}

void GamepadController::onGamepadButtonPressed( const GamepadButtonEvent &event ) {
