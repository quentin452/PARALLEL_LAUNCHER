...
private:
  inline void fireGamepadConnected(const GamepadConnectedEvent &event) {
    emit gamepadConnected(event);
  }
  inline void fireGamepadDisconnected(const GamepadDisconnectedEvent &event) {
    emit gamepadDisconnected(event);
  }
  inline void fireGamepadButtonPressed(const GamepadButtonEvent &event) {
    emit gamepadButtonPressed(event);
  }
  inline void fireGamepadAxisMoved(const GamepadAxisEvent &event) {
    emit gamepadAxisMoved(event);
  }
  inline void fireGamepadHatMoved(const GamepadHatEvent &event) {
    emit gamepadHatMoved(event);
  }
