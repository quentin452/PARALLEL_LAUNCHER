...
template<> void JsonSerializer::serialize<InputDriver>( JsonWriter &jw, const InputDriver &obj ) {
	const InputDriver driver = InputDriverUtil::isSupported( obj ) ? obj : InputDriverUtil::getDefault();
	switch( driver ) {
		case InputDriver::HID: jw.writeString( D_HID ); break;
		case InputDriver::SDL: jw.writeString( D_SDL ); break;
		case InputDriver::DirectInput: jw.writeString( D_DINPUT ); break;
		case InputDriver::JoyDev: jw.writeString( D_JOYDEV ); break;
		default: jw.writeString( "null" ); break;
	}
}
...
template<> InputDriver JsonSerializer::parse<InputDriver>( const Json &json ) {
	...
	if( driver == D_DINPUT && InputDriverUtil::isSupported( InputDriver::DirectInput ) ) {
