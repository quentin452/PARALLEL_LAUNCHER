...
class PosixFile {
	...
	PosixFile( const string &filePath ) :
		m_path( filePath ),
		m_fd( tryOpenNewFile( filePath ) )
	{}

	PosixFile( string &&filePath ) :
		m_path( std::move( filePath ) ),
		m_fd( tryOpenNewFile( m_path ) )
	{}
...
static inline string tryGetEventFile( const string &jsPath ) {
	...
	while( std::getline( devFile, line ).good() && !devFile.eof() ) {
		if( line.substr( 0, 12 ) != "H: Handlers=" ) continue;
		line = line.substr( 12 );
...
static bool tryOpenGamepad( const string &jsPath, JoydevGamepad &gamepad ) {
	...
	if( !jsFile.isValid() ) return false;

	const string eventPath = tryGetEventFile( jsPath );
	if( eventPath.empty() ) return false;

	PosixFile eventFile( eventPath );
	if( !eventFile.isValid() ) return false;
	...
	if( ioctl( jsFile.fd(), JSIOCGNAME(256), charBuffer ) >= 0 ) {
		...
		} else for( size_t i = 0; i < name.size(); i++ ) {
	...
	gamepad.state = {
		std::vector<bool>( numButtons, false ),
...
JoyDevDriver::JoyDevDriver( const GamepadEventEmitter &eventEmitter ) : IGamepadDriver( eventEmitter ) {
	QObject::connect( &s_timer, &QTimer::timeout, [this](){ this->processEvents(); } );
	s_timer.setInterval( 10 );
}

JoyDevDriver::~JoyDevDriver() {
	stop();
}
...
bool JoyDevDriver::isRunning() const {
	return s_inotify >= 0;
}
...
void JoyDevDriver::loadConnectedDevices( bool sendEvents ) const {
	fs::error_code err;
	for( fs::directory_iterator folder( fs::path( "/dev/input" ), err ); !err && folder != fs::end( folder ); folder.increment( err ) ) {
		const fs::directory_entry &file = *folder;

		if( file.is_directory() ) continue;
		if( file.path().filename().string().substr( 0, 2 ) != "js" ) continue;

		JoydevGamepad gamepad;
		if( !tryOpenGamepad( file.path().string(), gamepad ) ) continue;
...
static inline bool tryReadSingleEvent( BasicINotifyEvent &event ) {
	for( size_t size = 2 * sizeof(inotify_event) - 1; size < 8192; size += sizeof(inotify_event) ) {
		Buffer buffer( size );
		...
		if( eventSize < sizeof(inotify_event ) ) return false;
...
void JoyDevDriver::processEvents() const {
	...
	/* poll inotify */ {
		...
		while( poll( fds, 1, 0 ) && (fds[0].revents & POLLIN) ) {
			...
			if( !tryReadSingleEvent( event ) ) continue;
			if( event.name.substr( 0, 2 ) != "js" ) continue;

			const string filePath = "/dev/input/"s + event.name;
			if( event.type == IN_CREATE ) {
				if( tryFindGamepad( filePath, nullptr ) ) continue;
				JoydevGamepad gamepad;
				...
				if( !tryFindGamepad( filePath, &gamepadId ) ) continue;
	...
	foreach_mvalue( device, s_devices ) {
		...
		while( poll( fds, 1, 0 ) && (fds[0].revents & POLLIN) ) {
			js_event event;
			if( read( device.file.fd(), (void*)&event, sizeof( js_event ) ) != sizeof( js_event ) ) {
			...
			switch( event.type & 0x7F ) {
				case JS_EVENT_BUTTON:
					if( (ushort)event.number >= device.info.numButtons ) continue;
					...
					break;
				case JS_EVENT_AXIS: {
					if( (ushort)event.number >= device.info.numAxes ) continue;
...
std::vector<Uuid> JoyDevDriver::getDeviceOrder() {
	...
	foreach_cvalue( uuid, deviceOrder ) {
