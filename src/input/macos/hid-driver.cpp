...
/* Mimic RetroArch's Mac input driver so that the inputs map as expected in
 * RetroArch */
...
static inline ushort tryGetIndex(const HashMap<uint, ushort> &inputMap,
                                 uint cookie) {
...
HidDriver::HidDriver(const GamepadEventEmitter &eventEmitter)
...
static inline ushort getShortProperty(IOHIDDeviceRef &device,
                                      const CFStringRef &key) {
  return (ushort)getIntProperty(device, key);
}

static string getStringProperty(IOHIDDeviceRef &device,
                                const CFStringRef &key) {
  char buffer[1024];
  buffer[0] = '\0';
  CFStringRef ref = (CFStringRef)IOHIDDeviceGetProperty(device, key);
  if (ref)
    CFStringGetCString(ref, buffer, 1024, kCFStringEncodingUTF8);
  buffer[1023] = '\0';
  return string(buffer);
}

static void handleButtonEvent(GamepadId gamepadId, HidDevice &device,
                              IOHIDValueRef &value, uint cookie) {
  ...
  QtThread::safeAsync([=]() {
    s_events.gamepadButtonPressed({gamepadId, (ubyte)index, pressed});
  });
}

static void handleAxisOrButtonEvent(GamepadId gamepadId, HidDevice &device,
                                    IOHIDValueRef &value,
                                    IOHIDElementRef &element, uint cookie) {
  ...
  const double normalizedValue =
      (2.0 * (double)(current - min) / (double)(max - min)) - 1.0;
  if (index < device.state.axes.size()) {
    device.state.axes[index] = normalizedValue;
  }

  QtThread::safeAsync([=]() {
    s_events.gamepadAxisMoved({gamepadId, (ubyte)index, normalizedValue});
  });
}

static void handleHatEvent(GamepadId gamepadId, HidDevice &device,
  ...
  case 2:
    ...
    break;
  case 3:
    direction = HatDirection::Down | HatDirection::Right;
    break;
  case 4:
    direction = HatDirection::Down;
    break;
  case 5:
    direction = HatDirection::Down | HatDirection::Left;
    break;
  case 6:
    direction = HatDirection::Left;
    break;
  case 7:
    direction = HatDirection::Up | HatDirection::Left;
    break;
  default:
    direction = HatDirection::Centre;
    break;
  }
  ...
  QtThread::safeAsync([=]() {
    s_events.gamepadHatMoved({gamepadId, (ubyte)index, direction});
  });
}

static void inputReceived(void *context, IOReturn, void *,
                          IOHIDValueRef value) {
  const GamepadId gamepadId =
      (GamepadId) reinterpret_cast<std::intptr_t>(context);
...
static const HashMap<uint, ushort> s_axisMap = {
    {kHIDUsage_GD_X, 0},         {kHIDUsage_GD_Y, 1},
    {kHIDUsage_GD_Rx, 2},        {kHIDUsage_GD_Ry, 3},
    {kHIDUsage_GD_Z, 4},         {kHIDUsage_GD_Rz, 5},
    {kHIDUsage_Sim_Rudder, 6},   {kHIDUsage_Sim_Throttle, 7},
    {kHIDUsage_Sim_Steering, 8}, {kHIDUsage_Sim_Accelerator, 9},
    {kHIDUsage_Sim_Brake, 10}};

static inline void initInputMap(IOHIDDeviceRef &device, HidInputMap &inputMap) {
  CFArrayRef elementsConst =
  ...
  CFMutableArrayRef elements =
      CFArrayCreateMutableCopy(kCFAllocatorDefault, numInputs, elementsConst);
...
static HidDevice createHidDevice(IOHIDDeviceRef &device) {
  ...
  deviceInfo.info.controllerId.vendorId =
      getShortProperty(device, CFSTR(kIOHIDVendorIDKey));
  deviceInfo.info.controllerId.productId =
      getShortProperty(device, CFSTR(kIOHIDProductIDKey));
  deviceInfo.portId = getIntProperty(device, CFSTR(kIOHIDLocationIDKey));

  // Mimic how SDL makes UUIDs
  ubyte uniqueId[16];
  std::memset(&uniqueId, 0, 16);
  if (deviceInfo.info.controllerId.productId > 0 &&
      deviceInfo.info.controllerId.vendorId > 0) {
    reinterpret_cast<ushort *>(uniqueId)[0] =
        getShortProperty(device, CFSTR(kIOHIDTransportKey));
    reinterpret_cast<ushort *>(uniqueId)[4] =
        deviceInfo.info.controllerId.vendorId;
    reinterpret_cast<ushort *>(uniqueId)[8] =
        deviceInfo.info.controllerId.productId;
    reinterpret_cast<ushort *>(uniqueId)[12] =
        getShortProperty(device, CFSTR(kIOHIDVersionNumberKey));
  } else {
...
static void deviceDiconnected(void *context, IOReturn, void *) {
  const GamepadId gamepadId =
      (GamepadId) reinterpret_cast<std::intptr_t>(context);
  if (s_devices.count(gamepadId) > 0) {
    QtThread::safeAsync([=]() { s_events.gamepadDisconnected({gamepadId}); });
  }
...
static void deviceConnected(void *, IOReturn, void *, IOHIDDeviceRef device) {
  const GamepadId gamepadId =
      (GamepadId)getIntProperty(device, CFSTR(kIOHIDUniqueIDKey));
  ...
  IOHIDDeviceRegisterInputValueCallback(
      ...
      reinterpret_cast<void *>((std::intptr_t)gamepadId));
  IOHIDDeviceRegisterRemovalCallback(
      device, deviceDiconnected,
      reinterpret_cast<void *>((std::intptr_t)gamepadId));
...
static void appendMatcher(CFMutableArrayRef &array, uint page, uint usage) {
...
void HidDriver::start() {
  ...
  IOHIDManagerScheduleWithRunLoop(s_hidManager, CFRunLoopGetCurrent(),
  ...
  CFMutableArrayRef deviceMatcher =
  ...
  IOHIDManagerRegisterDeviceMatchingCallback(s_hidManager, deviceConnected,
                                             nullptr);
...
void HidDriver::stop() {
  if (!s_active || !s_hidManager)
    return;

  IOHIDManagerUnscheduleFromRunLoop(s_hidManager, CFRunLoopGetCurrent(),
                                    kCFRunLoopCommonModes);
...
bool HidDriver::isRunning() const { return s_active; }
