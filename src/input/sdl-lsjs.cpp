...
std::vector<Uuid> lsjs::tryGetPortOrder(bool &success) {
  ...
  try {
    ...
    const string lsjsCmd =
        (BaseDir::share() / L"parallel-launcher-lsjs.exe").u8string();
#else
    const string lsjsCmd =
        escapePath((BaseDir::share() / "parallel-launcher-lsjs").u8string());
#endif
    ...
    while (lineStart < uuidList.length()) {
      ...
      Uuid uuid;
      if (lineEnd - lineStart >= 36 &&
          Uuid::tryParse(uuidList.substr(lineStart, 36), uuid)) {
