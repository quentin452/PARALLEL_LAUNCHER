...
DirectInputDriver::DirectInputDriver(const GamepadEventEmitter &eventEmitter)
...
DirectInputDriver::~DirectInputDriver() { DirectInputDriver::stop(); };

static int CALLBACK configureAxisCallback(const DIDEVICEOBJECTINSTANCE *device,
                                          void *acquiredDevice) {
  DIPROPRANGE range;
  ...
  IDirectInputDevice8_SetProperty(
      static_cast<IDirectInputDevice8 *>(acquiredDevice), DIPROP_RANGE,
      &range.diph);

  return DIENUM_CONTINUE;
}

static int CALLBACK onDeviceConnected(const DIDEVICEINSTANCE *device,
                                      void *events) {
  ...
  string name = Unicode::toUtf8(
      reinterpret_cast<const wchar_t *>(device->tszInstanceName));
  if (name.empty()) {
    name = Unicode::toUtf8(
        reinterpret_cast<const wchar_t *>(device->tszProductName));
  }

  const ushort vendorId = (ushort)(device->guidProduct.Data1 & 0xFFFF);
  const ushort productId =
      (ushort)((device->guidProduct.Data1 & 0xFFFF0000u) >> 16);

  constexpr ushort numButtons = 128;
  constexpr ushort numAxes =
      8; // There can be more, but RetroArch only cares about 8 of them
  constexpr ushort numHats = 4;

  IDirectInputDevice8 *acquiredDevice;
  IDirectInput8_CreateDevice(s_context, device->guidInstance, &acquiredDevice,
                             nullptr);
  IDirectInputDevice8_SetDataFormat(acquiredDevice, &c_dfDIJoystick2);
  IDirectInputDevice8_SetCooperativeLevel(acquiredDevice, GetDesktopWindow(),
                                          DISCL_NONEXCLUSIVE |
                                              DISCL_BACKGROUND);

  IDirectInputDevice8_EnumObjects(acquiredDevice, configureAxisCallback,
                                  (void *)acquiredDevice, DIDFT_ABSAXIS);

  s_idMap[s_nextId] = uuid;
  s_devices[uuid] = {
      s_nextId++, acquiredDevice,
      ControllerInfo{
          name, {vendorId, productId}, uuid, numButtons, numAxes, numHats},
      GamepadState{std::vector<bool>((size_t)128),
...
void DirectInputDriver::start() {
  ...
  DirectInput8Create(GetModuleHandle(nullptr), 0x0800, IID_IDirectInput8,
                     (void **)&s_context, nullptr);

  IDirectInput8_EnumDevices(s_context, DI8DEVCLASS_GAMECTRL, onDeviceConnected,
                            (void *)&m_events, DIEDFL_ATTACHEDONLY);
...
bool DirectInputDriver::isRunning() const { return s_context != nullptr; }

static int CALLBACK enumConnectedDevice(const DIDEVICEINSTANCE *device,
                                        void *deviceList) {
  const Uuid uuid((const ubyte *)&device->guidInstance);
  ...
  GamepadId id = 0;
  static_cast<std::vector<ConnectedGamepad> *>(deviceList)
      ->push_back({
          id++,
          ControllerInfo{
              name, {vendorId, productId}, uuid, numButtons, numAxes, numHats},
...
std::vector<ConnectedGamepad> DirectInputDriver::getConnected() {
  ...
  if (s_context == nullptr) {
    ...
    IDirectInput8_EnumDevices(s_context, DI8DEVCLASS_GAMECTRL,
                              enumConnectedDevice, (void *)&devices,
                              DIEDFL_ATTACHEDONLY);
...
static int CALLBACK getUuidCallback(const DIDEVICEINSTANCE *device,
                                    void *uuids) {
...
std::vector<Uuid> DirectInputDriver::getDeviceOrder() {
  ...
  IDirectInput8_EnumDevices(context, DI8DEVCLASS_GAMECTRL, getUuidCallback,
                            (void *)&uuids, DIEDFL_ATTACHEDONLY);
...
static inline HatDirection getHatPosition(unsigned int pos) {
  // RetroArch only checks for specific angles even if the "hat" supports full
  // analog
  switch (pos) {
  case 0:
    return HatDirection::Up;
  case 4500:
    return HatDirection::Up | HatDirection::Right;
  case 9000:
    return HatDirection::Right;
  case 13500:
    return HatDirection::Right | HatDirection::Down;
  case 18000:
    return HatDirection::Down;
  case 22500:
    return HatDirection::Down | HatDirection::Left;
  case 27000:
    return HatDirection::Left;
  case 31500:
    return HatDirection::Left | HatDirection::Up;
  default:
    return HatDirection::Centre;
  }
}
...
void DirectInputDriver::processEvents() {
  ...
  if (++s_loopIteration >= 250) {
    ...
    IDirectInput8_EnumDevices(s_context, DI8DEVCLASS_GAMECTRL,
                              onDeviceConnected, (void *)&m_events,
                              DIEDFL_ATTACHEDONLY);
    s_loopIteration = 1;
  }
  ...
  for (auto it = s_devices.begin(); it != s_devices.end();) {
    ...
    DIJOYSTATE2 state;
    const HRESULT status = IDirectInputDevice8_GetDeviceState(
        c.device, sizeof(DIJOYSTATE2), &state);
    ...
    long axisValues[8] = {state.lX,           state.lY,          state.lZ,
    ...
    for (int i = 0; i < 8; i++) {
      ...
      if (value != c.state.axes[i]) {
        ...
        m_events.gamepadAxisMoved({c.gamepadId, (ubyte)i, value});
      }
    }
    ...
    for (int i = 0; i < 128; i++) {
      ...
      if (pressed != c.state.buttons[i]) {
        ...
        m_events.gamepadButtonPressed({c.gamepadId, (ubyte)i, pressed});
      }
    }
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (pos != c.state.hats[i]) {
        c.state.hats[i] = pos;
        m_events.gamepadHatMoved({c.gamepadId, (ubyte)i, pos});
      }
    }
