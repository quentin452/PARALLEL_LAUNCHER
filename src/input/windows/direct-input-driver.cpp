...
static int CALLBACK enumConnectedDevice(const DIDEVICEINSTANCE *device, void *deviceList) {
    ...
    static_cast<std::vector<ConnectedGamepad> *>(deviceList)->push_back({
        id++,
        ControllerInfo{name, {vendorId, productId}, uuid, numButtons, numAxes, numHats},
    });
    ...
    for (auto it = s_devices.begin(); it != s_devices.end();) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            if (value != c.state.axes[i]) {
                ...
                m_events.gamepadAxisMoved({c.gamepadId,
                                           (ubyte)i,
                                           value});
            }
        }
        ...
        for (int i = 0; i < 128; i++) {
            ...
            if (pressed != c.state.buttons[i]) {
                ...
                m_events.gamepadButtonPressed({c.gamepadId,
                                               (ubyte)i,
                                               pressed});
            }
        }
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (pos != c.state.hats[i]) {
                c.state.hats[i] = pos;
                m_events.gamepadHatMoved({c.gamepadId,
                                          (ubyte)i,
                                          pos});
            }
        }
