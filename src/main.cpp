...
static inline void loadTranslations(
	...
	for( const fs::path &folder : translationDirectories ) {
		if( translations.load( locale, "parallel-launcher", "_", folder.u8string().c_str() ) ) {
			return;
		}
	}
}

static string makeQtLogMessage( const QMessageLogContext &context, const QString &message ) {
	return QString( "[Qt] %1\nin %2 at %3:%4 (v%5)" )
		.arg( message )
		.arg( context.file )
...
static void onQtMessage( QtMsgType level, const QMessageLogContext &context, const QString &message ) {
	switch( level ) {
...
static inline void fixDiscordIntegrationInFlatpak() {
	...
	if( xdgrd == nullptr || xdgrd[0] == '\0' ) return;

	for( char i = '0'; i <= '9'; i++ ) {
		const fs::path socketPath = fs::path( xdgrd ) / ("discord-ipc-"s + i);
		if( fs::is_socket( socketPath ) ) continue;

		fs::error_code err;
		fs::remove( socketPath, err );
		fs::create_symlink( fs::path( xdgrd ) / "app" / "com.discordapp.Discord" / ("discord-ipc-"s + i), socketPath, err );
	}
}
#endif
...
static inline void createEmulatorCoreInfoFiles() {
	...
	QFile( ":/parallel_n64_next_libretro.info" ).copy( (infoDir / _NFS("parallel_n64_next_libretro.info")).u8string().c_str() );
	QFile( ":/mupen64plus_next_libretro.info" ).copy( (infoDir / _NFS("mupen64plus_next_libretro.info")).u8string().c_str() );
}
...
#ifdef __linux__
	QGuiApplication::setDesktopFileName( "ca.parallel_launcher.ParallelLauncher.desktop" );
#endif
