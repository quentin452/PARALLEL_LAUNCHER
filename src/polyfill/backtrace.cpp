...
static void logStackFrame(std::FILE *logFile, DWORD64 frameAddr, HANDLE process,
                          SYMBOL_INFO *symbol,
                          IMAGEHLP_MODULE64 *moduleInfo) noexcept {
...
void Backtrace::logFromContext(std::FILE *logFile, void *context) noexcept {
  ...
  while (StackWalk64(cpuArch, proc, thread, &frame, cpuContext, nullptr,
...
Backtrace::Backtrace() noexcept { m_size = ::backtrace(m_trace, 32); }
