...
static Locations getLocations() {
  ...
  if (SHGetKnownFolderPath(FOLDERID_Profile,
                           KF_FLAG_NO_ALIAS | KF_FLAG_DEFAULT_PATH |
    ...
    throw std::runtime_error(
        "Failed to get home directory location from Windows.");
  }
  ...
  if (SHGetKnownFolderPath(FOLDERID_LocalAppData,
                           KF_FLAG_CREATE | KF_FLAG_NO_ALIAS |
                               KF_FLAG_DONT_VERIFY | KF_FLAG_DEFAULT_PATH |
                               KF_FLAG_NOT_PARENT_RELATIVE,
                           nullptr, &buffer) == S_OK) {
  ...
  if (result <= 0 || result >= 261) {
    throw std::runtime_error(
        "Failed to get current executable path from Windows.");
  }
  ...
  if (result <= 0 || result >= 261) {
    throw std::runtime_error(
        "Failed to get temporary directory path from Windows.");
  }
...
#else
static fs::path xdg(const char *varName, const fs::path &homePath,
...
static Locations getLocations() {
  ...
  return Locations {
    home, xdg("XDG_CONFIG_HOME", home, ".config") / APP_NAME,
...
const fs::path &BaseDir::home() { return lazyGetLocations().home; }

const fs::path &BaseDir::config() { return lazyGetLocations().config; }

const fs::path &BaseDir::data() { return lazyGetLocations().data; }

const fs::path &BaseDir::cache() { return lazyGetLocations().cache; }

const fs::path &BaseDir::temp() { return lazyGetLocations().temp; }

const fs::path &BaseDir::program() { return lazyGetLocations().program; }
