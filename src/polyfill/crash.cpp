...
static std::FILE *startCrashLog(const char *errorType, const char *errorMessage,
                                bool isWarning) noexcept {
  ...
  if (appContext.deviceId) {
    ...
    fprintf(file, "Screen = %dx%d@%ddpi\n", appContext.screenWidth,
            appContext.screenHeight, appContext.screenDpiY);
...
static LONG WINAPI onWindowsException(LPEXCEPTION_POINTERS seh) {
  ...
  if (ep) {
    try {
      ...
      const auto out =
          startCrashLog(typeid(exception).name(), exception.what(), false);
      const TraceableException *traceableException =
          dynamic_cast<const TraceableException *>(&exception);
    ...
    if (exception == nullptr) {
      const auto out =
          startCrashLog("Structured Exception Event", nullptr, false);
      Backtrace::logFromContext(out, (void *)seh->ContextRecord);
      endCrashLog(out);
    } else {
      ...
      const auto out = startCrashLog(
          "Structured Exception Event",
          Number::toString((uint)exception->ExceptionCode).c_str(), false);
      Backtrace::logFromContext(out, (void *)seh->ContextRecord);
      endCrashLog(out);
    }
  }
...
static int startCrashLog(const char *errorType, const char *errorMessage,
                         bool isWarning) noexcept {
  const AppContextInfo &appContext = getAppContext();
  int fd =
      open(appContext.crashlogFilepath, O_WRONLY | O_CREAT | O_TRUNC, 0644);
  if (fd < 0)
    return fd;
...
static void onTerminate() {
  ...
  try {
    ...
    const auto out =
        startCrashLog("Unhandled Exception",
                      exceptionType ? exceptionType->name() : nullptr, false);
#else
    const auto out = startCrashLog("Unhandled Exception", nullptr, false);
#endif
    Backtrace().log(out);
...
void __log_error_internal(const char *type, const char *message, bool isWarning,
...
void storeCrashLog() {
  ...
  if (!fs::exists(crashlogPath, err) || err)
    return;
  ...
  const int64 timestamp =
      Time::toUnixTime(fs::last_write_time(crashlogPath, err));
#endif
