...
ReadableFifoPipe::ReadableFifoPipe() noexcept
    : m_filePath(""), m_alive(false), m_fd(-1) {}
...
bool ReadableFifoPipe::open(const string &name, std::error_code &err) {
  ...
  m_fd =
      ::open(m_filePath.c_str(), O_RDONLY | O_NONBLOCK | O_NOFOLLOW | O_ASYNC);
  if (m_fd < 0) {
    err = std::make_error_code((std::errc)errno);
...
bool ReadableFifoPipe::connect(std::error_code &err) const noexcept {
  ...
  while (m_alive) {
    ...
    if (r < 0 && errno != EAGAIN) {
      err = std::make_error_code(m_alive ? (std::errc)errno
...
bool ReadableFifoPipe::read(size_t bytes, void *buffer,
  ...
  while (m_alive && bytes > 0) {
    ...
    if (bytesRead < 0) {
      ...
      err = std::make_error_code(m_alive ? std::errc::no_message
                                         : std::errc::operation_canceled);
      return false;
    }
...
WritableFifoPipe::WritableFifoPipe() noexcept
    : m_filePath(""), m_cancelled(false), m_fd(-1), m_created(false) {}
...
bool WritableFifoPipe::connect(std::error_code &err) noexcept {
  while (!m_cancelled) {
    m_fd = ::open(m_filePath.c_str(),
                  O_WRONLY | O_NONBLOCK | O_NOFOLLOW | O_ASYNC);
    if (m_fd > 0)
      break;
...
bool WritableFifoPipe::write(size_t bytes, const void *buffer,
  ...
  while (bytes > 0 && !m_cancelled) {
    ...
    if (bytesWritten < 0) {
      err = m_cancelled ? std::make_error_code(std::errc::operation_canceled)
