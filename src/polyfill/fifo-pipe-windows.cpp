...
ReadableFifoPipe::ReadableFifoPipe() noexcept
    : m_filePath(""), m_alive(false), m_handle(nullptr) {}

static inline bool waitForConnection(const bool &alive, void *handle,
                                     std::error_code &err) noexcept {
  OVERLAPPED status;
  std::memset(&status, 0, sizeof(OVERLAPPED));

  if (ConnectNamedPipe(handle, &status))
    return true;
  DWORD statusCode = GetLastError();

  if (statusCode == ERROR_PIPE_CONNECTED)
    return true;

  if (statusCode != ERROR_IO_PENDING && statusCode != ERROR_PIPE_LISTENING) {
    err = alive ? std::error_code((int)statusCode, std::system_category())
...
bool ReadableFifoPipe::open(const string &name, std::error_code &err) {
  ...
  m_handle =
      CreateNamedPipeA(m_filePath.c_str(),
                       PIPE_ACCESS_INBOUND | FILE_FLAG_FIRST_PIPE_INSTANCE |
                           FILE_FLAG_OVERLAPPED,
                       PIPE_READMODE_BYTE, 2, 4096, 4096, 0, nullptr);
...
bool ReadableFifoPipe::connect(std::error_code &err) const noexcept {
  ubyte ack = 0;
  if (!waitForConnection(m_alive, m_handle, err))
    return false;
  if (!read(1, &ack, err))
    return false;
...
bool ReadableFifoPipe::read(size_t bytes, void *buffer,
  ...
  if (!ReadFile(m_handle, buffer, (DWORD)bytes, nullptr, &status) &&
  ...
  while (m_alive) {
    if (GetOverlappedResultEx(m_handle, &status, &bytesRead, 150, false)) {
      if (bytesRead != bytes) {
        CancelIo(m_handle);
        err = m_alive ? std::make_error_code(std::errc::message_size)
                      : std::make_error_code(std::errc::operation_canceled);
        return false;
      }
    ...
    if (errorCode != WAIT_TIMEOUT) {
      CancelIo(m_handle);
      err = m_alive ? std::error_code((int)errorCode, std::system_category())
                    : std::make_error_code(std::errc::operation_canceled);
      return false;
    }
  }
...
WritableFifoPipe::WritableFifoPipe() noexcept
    : m_filePath(""), m_alive(false), m_handle(nullptr) {}
  ...
  m_handle =
      CreateNamedPipeA(m_filePath.c_str(),
                       PIPE_ACCESS_OUTBOUND | FILE_FLAG_FIRST_PIPE_INSTANCE |
                           FILE_FLAG_OVERLAPPED,
                       PIPE_TYPE_BYTE, 2, 4096, 4096, 0, nullptr);
...
bool WritableFifoPipe::write(size_t bytes, const void *buffer,
                             std::error_code &err) const noexcept {
  if (bytes == 0)
    return true;
