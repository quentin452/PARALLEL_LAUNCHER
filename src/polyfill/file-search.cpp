...
static inline INode toINode(const fs::path &parentPath,
                            const WIN32_FIND_DATAW *wfData) {
  return INode{
      /* path */ parentPath / wfData->cFileName,
      /* isDirectory */ (wfData->dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) !=
          0,
      /* isHidden */ wfData->cFileName[0] == '.' ||
          (wfData->dwFileAttributes & FILE_ATTRIBUTE_HIDDEN),
      /* isSymlink */
          (wfData->dwFileAttributes & FILE_ATTRIBUTE_REPARSE_POINT) &&
          (wfData->dwReserved0 & IO_REPARSE_TAG_SYMLINK)};
}
...
static std::vector<INode> getMatchedNodes(HANDLE searchHandle,
...
static void performSearchTask(const SearchTaskInfo &args,
  ...
  if (args.depth < args.options->maxDepth) {
    HANDLE searchHandle = FindFirstFileExW(
        (args.directory / L"*").wstring().c_str(), FindExInfoBasic, &nodeData,
        FindExSearchLimitToDirectories, nullptr, FIND_FIRST_EX_LARGE_FETCH);
    const std::vector<INode> folders =
        getMatchedNodes(searchHandle, &nodeData, args.directory);

    const std::lock_guard<std::mutex> lock(taskQueueLock);
    for (const INode &folder : folders) {
      if (!folder.isDirectory)
        continue;
      if (folder.isHidden && args.options->ignoreHidden)
        continue;
      if (folder.isSymlink && !args.options->followSymlinks)
        continue;

      taskQueue.push(
          SearchTaskInfo{args.options, folder.path, (ubyte)(args.depth + 1)});
    }
  }
  ...
  /* Get matching files */ {
    HANDLE searchHandle = FindFirstFileExW(
        ...
        FindExInfoBasic, &nodeData, FindExSearchNameMatch, nullptr,
    ...
    for (const INode &file : files) {
      if (file.isDirectory)
        continue;
      if (file.isHidden && args.options->ignoreHidden)
        continue;
...
static void recursiveSearchWorker(
    std::condition_variable &dispatcher, volatile int &activeWorkers,
    std::queue<SearchTaskInfo> &taskQueue, std::vector<fs::path> &matches,
    std::mutex &tasksMutex, std::mutex &matchesMutex, std::mutex &workerMutex,
  ...
  while (true) {
    ...
    performSearchTask(task, taskQueue, matches, tasksMutex, matchesMutex);
...
std::vector<fs::path> FileSearch::find(const fs::path &directory,
                                       const string &pattern, ubyte maxDepth,
                                       bool followSymlinks, bool ignoreHidden,
                                       CancellationToken &cancellationToken) {
  const SearchOptions options = {std::move(pattern), maxDepth, followSymlinks,
                                 ignoreHidden};
  ...
  tasks.push(SearchTaskInfo{&options, directory, 1});
  ...
  for (uint i = 0; i < numWorkers; i++) {
    workers.emplace_back(
        std::thread(recursiveSearchWorker, std::ref(dispatcher),
                    std::ref(activeWorkers), std::ref(tasks), std::ref(matches),
                    std::ref(tasksMutex), std::ref(matchesMutex),
                    std::ref(workerMutex), std::ref(cancellationToken)));
  }
...
static inline string execAndRead(const char *program,
  ...
  pid_t pid;
  int exitCode;
  if (posix_spawnp(&pid, program, &actions, nullptr,
  ...
  while (!cancellationToken.isCancelled()) {
    pollfd fd = {comms[0], POLLIN | POLLHUP, 0};
