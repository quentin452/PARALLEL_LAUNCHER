...
#ifdef __arm64__
bool AppleUtil::isSilicon() noexcept { return true; }
#else
...
static inline bool getIsSilicon() noexcept {
  ...
  return sysctlbyname("sysctl.proc_translated", &result, &rs, nullptr, 0) ==
             0 &&
         result > 0;
}
...
static const ubyte s_armHeader[4] = {0xCA, 0xFE, 0xBA, 0xBE};

bool AppleUtil::isArmLibrary(const char *path) noexcept {
  // I don't know if this check is true in general, but it is for the
  // ParallelN64 core, which is all I care about
  try {
