...
public:
  ProcessLaunchException(const string &process, const std::vector<string> &args,
                         int errorCode)
      : std::runtime_error("Failed to launch child process"),
        TraceableException(), m_process(process), m_args(args),
        m_errorCode(errorCode) {}

  inline const string &process() const noexcept { return m_process; }
  inline const std::vector<string> &processArgs() const noexcept {
    return m_args;
  }
...
extern bool tryGetOutput(const string &command, string &output);

extern bool tryGetErrorOutput(const string &command, string &output);
