...
static AutoPlugin makeAutoPlugin( EmulatorCore emulator, GfxPlugin parallelPlugin, GfxPlugin mupenPlugin ) {
...
static inline AutoPlugin guess( const fs::path &romPath, bool preferHle ) {
	...
	switch( guessTool( romFile ) ) {
		...
		case Tool::SM64Editor:
			return makeAutoPlugin( EmulatorCore::ParallelN64, GfxPlugin::OGRE, GfxPlugin::GLideN64 );
		case Tool::SM64EditorPlusMOP:
			return makeAutoPlugin( EmulatorCore::ParallelN64, GfxPlugin::Rice, GfxPlugin::GLideN64 );
		case Tool::RomManager:
			return makeAutoPlugin( EmulatorCore::ParallelN64, GfxPlugin::GLideN64, GfxPlugin::GLideN64 );
		case Tool::DecompProbably:
		case Tool::BowsersBlueprints:
		default:
			return preferHle ?
				makeAutoPlugin( EmulatorCore::ParallelN64, GfxPlugin::GLideN64, GfxPlugin::GLideN64 ) :
				makeAutoPlugin( EmulatorCore::ParallelN64, GfxPlugin::ParaLLEl, GfxPlugin::ParaLLEl );
	}
}

AutoPlugin AutoPlugin::detect(
	...
	switch( plugin ) {
		case GfxPlugin::ParaLLEl:
			autoPlugin.upscaleTexrects = Flags::has( pluginFlags, PluginFlags::UpscaleTexrects );
			if( Flags::has( pluginFlags, PluginFlags::AllowHleFallback ) ) {
				autoPlugin.emulateFramebuffer = Flags::has( pluginFlags, PluginFlags::EmulateFramebuffer );
				if( autoPlugin.emulateFramebuffer ) {
					autoPlugin.accurateDepthCompare = Flags::has( pluginFlags, PluginFlags::AcccurateDepthCompare );
				}
				if( preferHle ) {
