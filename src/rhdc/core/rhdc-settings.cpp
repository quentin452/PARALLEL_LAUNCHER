...
const RhdcSettings &RhdcSettings::Default() {
  static const RhdcSettings s_default = {
      /* downloadDirectory */ BaseDir::home() / _NFS("Downloads") /
          _NFS("rhdc-hacks"),
...
template <>
void JsonSerializer::serialize<RhdcSettings>(JsonWriter &jw,
                                             const RhdcSettings &obj) {
  jw.writeObjectStart();
...
template <> RhdcSettings JsonSerializer::parse<RhdcSettings>(const Json &json) {
  ...
  return RhdcSettings{
      downloadDir.empty() ? RhdcSettings::Default().downloadDirectory
                          : fs::to_path(downloadDir),
      json[P_ENABLE_STAR_DISPLAY].getOrDefault<bool>(
          RhdcSettings::Default().enableStarDisplay),
      json[P_ALL_SAVE_SLOTS].getOrDefault<bool>(
          RhdcSettings::Default().checkAllSaveSlots),
      SteamDeck::isSteamDeck() ? false
                               : json[P_PREFER_HLE].getOrDefault<bool>(
                                     RhdcSettings::Default().preferHle),
      json[P_ENABLE_DEFAULT_LAYOUTS].getOrDefault<bool>(
          RhdcSettings::Default().enableDefaultLayouts),
      json[P_IGNORE_WIDESCREEN].getOrDefault<bool>(
          RhdcSettings::Default().ignoreWidescreenHint)};
}
