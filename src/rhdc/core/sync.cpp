...
static inline bool shouldUseWidescreen(
	const RhdcHackVersionExt &version
) {
	return(
		Flags::has( version.pluginFlags, PluginFlags::Widescreen ) &&
...
static void addHackVersion(
	...
	DataProvider::addRomInfo({
		...
		shouldUseWidescreen( version ) ? WidescreenMode::ViewportHack : WidescreenMode::Off,
...
static inline const RhdcHackVersionExt &getLatestVersion( const std::vector<RhdcHackVersionExt> &versions ) {
	for( const RhdcHackVersionExt &version : versions ) {
		if( !version.archived ) return version;
	}
	...
	RhdcApi::getFollowedHacksAsync(
		[callback](HashMap<string,FollowedHack> &hacks) {
			...
			foreach_cvalue( hack, hacks ) {
				...
				if( !knownHack ) {
					...
					for( const string &group : localGroups ) {
						if( group == SpecialGroups::Uncategorized ) continue;
					...
					if( shouldDownloadLayout( hack ) ) {
						...
						RhdcApi::downloadFile( hack.layoutUrl, layoutFile, [](int64,int64){}, [hackId,layoutFile](){
							const QString layoutPath = StarLayout::layoutPath( hackId ).u8string().c_str();
							layoutFile->flush();
							layoutFile->close();
			...
			for( const string &hackId : undownloadedHacks ) {
				...
				const RhdcHackVersionExt &latestVersion = getLatestVersion( hack.versions );
				if( latestVersion.archived ) continue;
...
void RHDC::moveRhdcFolder( const fs::path &oldDir, const fs::path &newDir ) {
	...
	if( !fs::existsSafe( oldDir ) ) return;
