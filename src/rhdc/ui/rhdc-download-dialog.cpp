...
fs::path RhdcDownloadDialog::run(
	const string &hackId,
	...
	return RhdcDownloadDialog::run(
		hackId,
		downloadUrl,
		QUrl( downloadUrl.c_str() ).fileName().toStdString()
	);
}
...
fs::path RhdcDownloadDialog::run(
	...
	RhdcApi::getStarpowerAsync( hackId,
		[=,&dialog](Starpower &starpower) {
			...
			dialog.getRecommendedPlugin( hackId, downloadUrl, versionName, dialogExists );
		},
		[=,&dialog](ApiErrorType) {
			if( !*dialogExists ) return;
			dialog.getRecommendedPlugin( hackId, downloadUrl, versionName, dialogExists );
	...
	*dialogExists = false;
	...
	const string name = fs::to_path( QUrl( downloadUrl.c_str() ).fileName().toStdString() ).stem().u8string();
	return FileController::loadRhdcSettings().downloadDirectory / fs::to_path(name + ' ' + hackId + ".z64");
}
		...
		[=,&downloadPath,&dialog,&hackSha1]( RhdcVersionedHack &hack ) {
			...
			if( hack.versions.empty() ) {
				QTimer::singleShot( 100, [](){
					ToastMessageManager::display( ToastType::Error, tr( "Failed to download patch from RHDC" ) );
				});
			...
			for( const RhdcHackVersionExt &v : hack.versions ) {
				DataProvider::addRhdcHackVersion(
					...
					v.name,
					vi++,
					v.plugin.value_or( GfxPlugin::UseDefault ),
					v.hackFlags,
					v.archived
				);
			}
			...
			downloadPath.clear();
			const string downloadUrl = RhdcApi::getDownloadUrl( hack.info.slug, version->name );
			DataProvider::addRomInfo( RomInfo{
				...
				false,
				0,
				Flags::has( version->hackFlags, RhdcHackFlag::DualAnalog ) ? DefaultInputModes::DualAnalog.id : DefaultInputModes::Normal.id,
				WidescreenMode::Off,
...
void RhdcDownloadDialog::getRecommendedPlugin(
	...
	if( !*dialogExists ) return;
	RhdcApi::getLayoutAndRecommendedPluginAsync( hackId, versionName,
		[=](PluginAndLayoutInfo &rec) {
			this->downloadLayout( hackId, downloadUrl, rec, dialogExists );
		},
		[=](ApiErrorType) {
			this->downloadLayout( hackId, downloadUrl, PluginAndLayoutInfo{ std::nullopt, PluginFlags::None, "" }, dialogExists );
		}
	);
}

void RhdcDownloadDialog::downloadLayout(
	const string &hackId,
	...
	RhdcApi::downloadFile(
		...
		[=](int64 downloaded, int64 total) {
			...
			this->m_ui->progressLabel->setText( layoutProgressText( downloaded, total ) );
			this->m_ui->progressBar->setValue( (int)(100.0 * downloaded / total + 0.5) );
		},
		[=]() {
			file->close();
			file->deleteLater();
		...
		[=](ApiErrorType) {
			...
			ToastMessageManager::display( ToastType::Warn, tr( "Failed to download star layout from RHDC" ) );
...
void RhdcDownloadDialog::downloadHack(
	...
	const fs::path tempDownloadPath = BaseDir::temp() / fs::to_path( url.fileName().toStdString() );
	...
	RhdcApi::downloadFile(
		...
		[=]() {
			...
			if( *dialogExists ) {
				...
				if( SM64::tryGetPath( sm64path ) ) {
					std::thread( RhdcDownloadDialog::patchRom, this, hackId, tempDownloadPath, sm64path, hackInfo, dialogExists ).detach();
				} else {
					ToastMessageManager::display( ToastType::Error, tr( "The rom could not be installed because you do not have an unmodified copy of the US version of Super Mario 64 in your known roms list." ) );
					logWarn( "Could not patch RHDC rom because no vanilla SM64 base rom was found" );
		...
		[=](ApiErrorType) {
			...
			if( *dialogExists ) this->close();
		}
	);

}

void RhdcDownloadDialog::patchRom(
	RhdcDownloadDialog *dialog,
	string hackId,
	fs::path patchPath,
	fs::path baseRomPath,
	...
	if( getExtensionLowerCase( patchPath ) == ".zip" ) {
		...
		fs::remove_all( zipDir, err );
		if( !foundPatch ) patchPath = fs::path();
	}

	if( getExtensionLowerCase( patchPath ) != ".bps" ) {
		QtThread::safeAsync( [=](){
			ToastMessageManager::display( ToastType::Error, tr( "The rom could not be installed because no bps patch was found." ) );
			logWarn( "Could not install rom because no bps patch was found" );
			if( *dialogExists ) dialog->close();
		});
		return;
	}

	const string romFilename = patchPath.stem().u8string() + ' ' + hackId + ".z64";
	fs::path romPath = FileController::loadRhdcSettings().downloadDirectory / fs::to_path( romFilename );

	const Bps::BpsApplyError bpsErr = Bps::tryApplyBps( patchPath, baseRomPath, romPath );
	if( bpsErr != Bps::BpsApplyError::None ) {
		fs::remove( patchPath, err );
		QtThread::safeAsync( [=](){
			ToastMessageManager::display( ToastType::Error, tr( "Could not install '%1' because of an unexpected error while applying the patch" ).arg( patchPath.stem().u8string().c_str() ) );
			logWarn( "Could not install '"s + patchPath.stem().u8string() + "' because of an unexpected error while applying the patch" );
			if( *dialogExists ) dialog->close();
		});
		return;
	}
	...
	QtThread::safeAsync( [=](){
		const AutoPlugin autoPlugin = AutoPlugin::detect( romPath, hackInfo.plugin, hackInfo.pluginFlags );
		...
		DataProvider::updatePluginSettings( sha1, autoPlugin.emulator, autoPlugin.parallelPlugin, autoPlugin.mupenPlugin, autoPlugin.upscaleTexrects, true, autoPlugin.emulateFramebuffer, autoPlugin.accurateDepthCompare, autoPlugin.emulateRsp );
		...
		SaveConverter::autoImportFromProject64(
			RetroArch::getSaveFilePath( romPath ),
			internalName
		);
#endif

		ToastMessageManager::display( ToastType::Info, tr( "Romhack installed successfully" ) );
