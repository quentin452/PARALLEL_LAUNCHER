...
static inline double getDifficulty(const RhdcHackExt &hack) {
  const double difficulty = (hack.progress.difficulty > 0)
                                ? (double)hack.progress.difficulty
                                : hack.info.avgDifficulty;
...
static bool compareName(const RhdcHackWidget *a, const RhdcHackWidget *b) {
  return QString::compare(a->hack().info.name.c_str(),
                          b->hack().info.name.c_str(), Qt::CaseInsensitive) < 0;
}

static bool comparePopularity(const RhdcHackWidget *a,
                              const RhdcHackWidget *b) {
...
static bool compareDifficulty(const RhdcHackWidget *a,
                              const RhdcHackWidget *b) {
  return getDifficulty(a->hack()) < getDifficulty(b->hack());
}

static bool compareLastPlayed(const RhdcHackWidget *a,
                              const RhdcHackWidget *b) {
...
static bool compareFixedShuffle(const RhdcHackWidget *a,
                                const RhdcHackWidget *b) {
  return a->shuffleOrderG() < b->shuffleOrderG();
}

static bool compareRandomShuffle(const RhdcHackWidget *a,
                                 const RhdcHackWidget *b) {
...
static bool (*s_hackComparators[])(const RhdcHackWidget *,
                                   const RhdcHackWidget *) = {
    compareName,       comparePopularity,   compareRating,
    compareDifficulty, compareLastPlayed,   comparePlayTime,
    compareStarCount,  compareFixedShuffle, compareRandomShuffle};

RhdcHackRenderer::RhdcHackRenderer(QObject *parent)
    : QStyledItemDelegate(parent) {}
RhdcHackRenderer::~RhdcHackRenderer() {}

QWidget *RhdcHackRenderer::createEditor(QWidget *parent,
                                        const QStyleOptionViewItem &,
                                        const QModelIndex &index) const {
  const RhdcHackListView *view =
      dynamic_cast<const RhdcHackListView *>(this->parent());
  const RhdcHackWidget *widget =
      static_cast<const RhdcHackWidget *>(index.internalPointer());

  if (widget == nullptr)
    return new QWidget();
  RhdcHackWidget *activeWidget = widget->clone(parent, true);
  activeWidget->setFixedWidth(view->width() -
                              view->verticalScrollBar()->width() - 6);

  connect(view, &RhdcHackListView::resized, activeWidget,
          &RhdcHackWidget::editorResized);
  return activeWidget;
}

void RhdcHackRenderer::paint(QPainter *painter,
                             const QStyleOptionViewItem &option,
                             const QModelIndex &index) const {
  const RhdcHackListView *view =
      static_cast<const RhdcHackListView *>(parent());
  RhdcHackWidget *widget =
      static_cast<RhdcHackWidget *>(index.internalPointer());
  if (widget == nullptr)
    return;
  ...
  const QPoint rtlFix = (view->layoutDirection() == Qt::RightToLeft)
                            ? QPoint(view->verticalScrollBar()->width(), 0)
                            : QPoint(0, 0);

  widget->setFixedWidth(view->width() - view->verticalScrollBar()->width() - 6);
  widget->render(painter,
                 view->mapTo(view->window(), view->contentsRect().topLeft() +
                                                 preparedOption.rect.topLeft() +
                                                 rtlFix));
...
QSize RhdcHackRenderer::sizeHint(const QStyleOptionViewItem &,
                                 const QModelIndex &) const {
  const RhdcHackListView *view =
      dynamic_cast<const RhdcHackListView *>(parent());
  if (view->width() < 550) {
...
RhdcHackListView::RhdcHackListView(QWidget *parent) : QListView(parent) {
  ...
  connect(selectionModel(),
          ...
          this,
          SLOT(modelIndexChanged(const QModelIndex &, const QModelIndex &)));
}
...
void RhdcHackListView::modelIndexChanged(const QModelIndex &index,
                                         const QModelIndex &) {
  if (index.isValid() && index.internalPointer()) {
    emit selectedHackChanged(
        &static_cast<const RhdcHackWidget *>(index.internalPointer())->hack());
  } else {
...
void RhdcHackListView::reloadAllHacks(bool reshuffle) {
  if (reshuffle)
    static_cast<RhdcHackListModel *>(model())->prepReshuffle();
  ...
  // FIXME: Dumb hack. Need to figure out what's causing the scrolling to reset
  // in some cases
  QtThread::safeAsync([this, scrollY]() {
    verticalScrollBar()->setRange(0,
                                  this->model()->rowCount(QModelIndex()) * 129);
...
void RhdcHackListView::selectHack(const string &hackId) {
  const QModelIndex index =
      static_cast<RhdcHackListModel *>(model())->findHack(hackId);
...
RhdcHackListModel::RhdcHackListModel(QObject *parent)
...
void RhdcHackListModel::reloadOne(const string &hackId) {
  ...
  const QModelIndex hackIndex =
      createIndex(row->second, 0, (void *)m_hacks[row->second]);
...
void RhdcHackListModel::reloadAll() {
  ...
  for (RhdcHackWidget *hack : m_hacks)
    delete hack;
  m_hacks.clear();

  const std::vector<RhdcHackExt> hacks =
      m_playlist.empty()
  ...
  for (const RhdcHackExt &hack : hacks) {
    RhdcHackWidget *hackWidget =
        new RhdcHackWidget(nullptr, hack, m_shuffleSeed);
...
void RhdcHackListModel::doSort() {
  ...
  if (m_sortAsc) {
    ...
    std::sort(m_hacks.rbegin(), m_hacks.rend(),
              s_hackComparators[(int)m_sortBy]);
  }
...
QModelIndex RhdcHackListModel::index(int row, int column,
                                     const QModelIndex &parent) const {
  if (column != 0 || parent.isValid() || row >= (int)m_hacks.size())
    return QModelIndex();
...
int RhdcHackListModel::columnCount(const QModelIndex &) const { return 1; }
