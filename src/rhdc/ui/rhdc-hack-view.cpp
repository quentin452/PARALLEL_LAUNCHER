...
RhdcHackView::RhdcHackView(QWidget *parent)
    : QWidget(parent), m_ui(new Ui::RhdcHackView), m_blockPlaylistEvents(false),
  ...
  m_ui->sortOrderButton->setIcon(
      uiState.sortBy == RhdcSorting::RandomShuffle
          ? Icon::shuffle()
  ...
  if (!uiState.playlist.empty()) {
    ...
    for (int i = 1; i < m_ui->playlistSelect->count(); i++) {
      if (m_ui->playlistSelect->itemData(i, Qt::UserRole)
              .toString()
              .toStdString() == uiState.playlist) {
  ...
  descriptionPalette.setColor(
      QPalette::Active, QPalette::Base,
      descriptionPalette.color(QPalette::Active, QPalette::Window));
  descriptionPalette.setColor(
      QPalette::Inactive, QPalette::Base,
      descriptionPalette.color(QPalette::Inactive, QPalette::Window));
  descriptionPalette.setColor(
      QPalette::Disabled, QPalette::Base,
      descriptionPalette.color(QPalette::Disabled, QPalette::Window));
  m_ui->hackDescription->setPalette(descriptionPalette);
}

RhdcHackView::~RhdcHackView() { delete m_ui; }
...
void RhdcHackView::updatePlaylists() {
  ...
  m_ui->playlistSelect->addItem(tr("All Hack Lists"),
  ...
  for (const string &playlist : playlists) {
    if (playlist == previousPlaylist)
      newIndex = i;
    m_ui->playlistSelect->addItem(
        GroupName::localize(playlist),
        QVariant::fromValue<QString>(playlist.c_str()));
    i++;
  }
...
RhdcUiState RhdcHackView::uiState() const {
  ...
  return RhdcUiState{
      (RhdcSorting)m_ui->sortSelect->currentIndex(), m_sortAsc,
      m_ui->playlistSelect->currentData(Qt::UserRole).toString().toStdString(),
...
string RhdcHackView::getSelectedVersion() const {
  ...
  if (hack == nullptr)
    return "";
  return m_ui->versionSelect->currentData(Qt::UserRole)
      .toString()
      .toStdString();
}

string RhdcHackView::getCurrentPlaylist() const {
  return m_ui->playlistSelect->currentData(Qt::UserRole)
      .toString()
      .toStdString();
}
...
void RhdcHackView::hackSelected(const RhdcHackExt *hack) {
  ...
  m_ui->authorsLabel->setText((hack->authors.size() == 1) ? tr("Author:")
                                                          : tr("Authors:"));
  ...
  const string activeVersion =
      DataProvider::getLastPlayedVersion(hack->info.hackId);
  const std::vector<RhdcHackVersion> versions =
      DataProvider::fetchRhdcHackVersions(hack->info.hackId);
  for (const RhdcHackVersion &version : versions) {
    if (version.archived && !DataProvider::romWithSha1Exists(version.sha1))
      continue;
...
bool RhdcHackView::tryGetHackIdAndDownloadUrl(string &hackId,
  ...
  downloadUrl = RhdcApi::getDownloadUrl(
      hack->info.slug, m_ui->versionSelect->currentText().toStdString());
  return true;
}
...
void RhdcHackView::changeSortOrder() {
  if (m_ui->sortSelect->currentIndex() != (int)RhdcSorting::RandomShuffle) {
    m_sortAsc = !m_sortAsc;
    m_ui->sortOrderButton->setIcon(m_sortAsc ? Icon::sortAsc()
                                             : Icon::sortDsc());
  }
  sortingChanged();
}
...
void RhdcHackView::playlistChanged() {
  if (m_blockPlaylistEvents)
    return;
  m_ui->hackList->setPlaylist(
      m_ui->playlistSelect->currentData(Qt::UserRole).toString().toStdString());
}
...
void RhdcHackView::reload() { reloadInternal(true); }
...
static inline void addHacktion(QMenu &menu, const QString &name,
                               Hacktion actionType, const char *arg = nullptr) {
  QAction *action = new QAction(name, &menu);
  action->setData(QVariant::fromValue<QString>(
      QString(QChar((char)actionType)).append(arg ? arg : "")));
  menu.addAction(action);
}

static inline bool isAutoManagedPlaylist(const string &playlist) {
  return (playlist == SpecialGroups::WantToPlay ||
...
getFreePlaylists(const string &hackId, bool &canRemoveFromPlaylist) {
  const std::set<string> currentPlaylists =
      DataProvider::fetchGroupsForRhdcHack(hackId);
  ...
  for (const string &playlist : allPlaylists) {
    if (!isAutoManagedPlaylist(playlist) &&
        currentPlaylists.count(playlist) <= 0) {
...
static string createNewPlaylist() {
  const string playlist =
      QInputDialog::getText(
          nullptr, RhdcHackView::tr("Create List"),
          RhdcHackView::tr("Enter a name for your new hack list"))
          .toStdString();
  if (playlist.empty())
    return playlist;
  ...
  for (const string &existingPlaylist : DataProvider::fetchAllGroups()) {
    if (playlist == existingPlaylist ||
        GroupName::getRhdcName(playlist) == existingPlaylist) {
      QMessageBox::critical(
          nullptr, RhdcHackView::tr("List Exists"),
          RhdcHackView::tr("A hack list with this name already exists"));
      return "";
    }
  }
...
void RhdcHackView::actionButtonClicked() {
  ...
  const string currentPlaylist =
      m_ui->playlistSelect->currentData(Qt::UserRole).toString().toStdString();

  RomFile romFile;
  const bool isDownloaded = DataProvider::tryFetchRomFileWithSha1(
                                m_ui->versionSelect->currentData(Qt::UserRole)
                                    .toString()
                                    .toStdString(),
                                currentPlaylist, &romFile) &&
                            romFile.local;

  const fs::path saveFilePath =
      isDownloaded ? RetroArch::getSaveFilePath(romFile.path) : fs::path();
  ...
  bool canRemoveFromPlaylist =
      !currentPlaylist.empty() && !isAutoManagedPlaylist(currentPlaylist);
  const std::vector<string> freePlaylists =
      getFreePlaylists(hack->info.hackId, canRemoveFromPlaylist);
  const bool canMove =
      !currentPlaylist.empty() && !isAutoManagedPlaylist(currentPlaylist);
  ...
  for (const string &playlist : freePlaylists) {
    addHacktion(moveMenu, GroupName::localize(playlist), Hacktion::MoveToList,
                playlist.c_str());
    addHacktion(copyMenu, GroupName::localize(playlist), Hacktion::CopyToList,
                playlist.c_str());
  }
  ...
  if (canRemoveFromPlaylist) {
    addHacktion(
        menu, tr("Remove from '%1'").arg(GroupName::localize(currentPlaylist)),
        Hacktion::RemoveFromList);
  }
  ...
  const QAction *selectedAction = menu.exec(
      m_ui->actionButton->mapToGlobal(m_ui->actionButton->rect().bottomLeft()));
  if (selectedAction == nullptr)
    return;

  const QString actionString = selectedAction->data().toString();
  if (actionString.isNull() || actionString.isEmpty())
    return;

  switch ((Hacktion)actionString.at(0).toLatin1()) {
  case Hacktion::RateHack: {
    RhdcRatingDialog dialog;
    dialog.setRatings(hack->progress.rating, hack->progress.difficulty,
                      hack->info.category == "Kaizo");
    if (dialog.exec() == QDialog::Accepted) {
      DataProvider::updateRhdcHackRating(hack->info.hackId, dialog.getQuality(),
  ...
  case Hacktion::MarkComplete:
    for (const RhdcHackVersion &version :
         DataProvider::fetchRhdcHackVersions(hack->info.hackId)) {
  ...
  case Hacktion::MoveToList: {
    ...
    RhdcApi::addHackToListAsync(
        hack->info.hackId, GroupName::getRhdcName(playlist), []() {},
        RhdcApi::logApiError("Failed to add hack to list"));
    RhdcApi::removeHackFromListAsync(
        hack->info.hackId, GroupName::getRhdcName(currentPlaylist), []() {},
  ...
  case Hacktion::Unfollow:
    if (QMessageBox::question(
            nullptr, tr("Confirm Unfollow"),
            tr("Are you sure you want to unfollow this hack? The ROM will not "
               "be deleted from your computer, but it will be removed from all "
               "of your hack lists on romhacking.com and your progress will no "
               "longer be synced with romhacking.com.")) == QMessageBox::Yes) {
      for (const string &playlist :
           DataProvider::fetchGroupsForRhdcHack(hack->info.hackId)) {
        RhdcApi::removeHackFromListAsync(
            hack->info.hackId, GroupName::getRhdcName(playlist), []() {},
            RhdcApi::logApiError("Failed to remove hack from playlist"));
      }
  ...
  case Hacktion::Uninstall:
    if (QMessageBox::question(nullptr, tr("Confirm Delete"),
                              tr("Are you sure you want to delete this ROM "
