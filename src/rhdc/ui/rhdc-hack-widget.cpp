...
void ThumbnailNotifier::notifyDownloaded() { emit downloaded(); }
...
static QString formatRating(double avgRating, ubyte userRating, bool kaizo) {
  if (avgRating <= 0.0) {
    ...
    return QString("%L1 (%L2*)")
        .arg(avgRating + (kaizo ? 10.0 : 0.0), 0, 'f', 1)
        .arg((int)userRating + (kaizo ? 10 : 0));
  } else {
...
static QPixmap loadColouredIcon(const QString &path, uint colour) {
  QImage icon = QIcon(path).pixmap(16, 16).toImage().convertToFormat(
...
static const IconSet &getIconSetSelected(QWidget *widget) {
  static const IconSet s_iconSet =
      getIconSet(widget->palette().highlightedText().color());
  return s_iconSet;
}
...
RhdcHackWidget::RhdcHackWidget(QWidget *parent, const RhdcHackExt &hack,
                               uint64 shuffleSeed)
    : QWidget(parent), m_ui(new Ui::RhdcHackWidget), m_hack(hack),
      m_shuffleOrder(hashString(hack.info.hackId)), m_thumbnailLoaded(false) {
  reshuffle(shuffleSeed);
  initialize(false);
}

RhdcHackWidget::RhdcHackWidget(QWidget *parent, const RhdcHackExt &hack,
                               bool selected, uint64 shuffleOrderG,
                               uint64 shuffleOrderS)
    : QWidget(parent), m_ui(new Ui::RhdcHackWidget), m_hack(hack),
      m_shuffleOrder(shuffleOrderG), m_seededShuffleOrder(shuffleOrderS),
      m_thumbnailLoaded(false) {
  initialize(selected);
}

RhdcHackWidget::~RhdcHackWidget() { delete m_ui; }
...
void RhdcHackWidget::initialize(bool selected) {
  ...
  if (m_hack.info.avgRating == 0) {
    ...
    m_ui->ratingLabel->setText(
        QString("%L1").arg(m_hack.info.avgRating, 0, 'f', 2));
  }
  m_ui->difficultyLabel->setText(formatRating(m_hack.info.avgDifficulty,
                                              m_hack.progress.difficulty,
                                              m_hack.info.category == "Kaizo"));
  if (m_hack.lastPlayed > 0) {
    m_ui->lastPlayedLabel->setText(
        QDateTime::fromMSecsSinceEpoch(m_hack.lastPlayed)
  ...
  if (m_hack.info.starCount > 0) {
    ...
    if (layoutDirection() == Qt::RightToLeft) {
      m_ui->starCountLabel->setText(QString(" %L2 \\ %L1")
                                        .arg(m_hack.progress.stars)
                                        .arg(m_hack.info.starCount));
    } else {
      m_ui->starCountLabel->setText(QString("%L1 / %L2 ")
                                        .arg(m_hack.progress.stars)
                                        .arg(m_hack.info.starCount));
    }
  } else {
    m_ui->progressWidget->setCurrentIndex(1);
    m_ui->completionLabel->setText(
        m_hack.progress.completed ? tr("Completed") : tr("Incomplete"));

    QPalette textColour = m_ui->completionLabel->palette();
    textColour.setColor(QPalette::WindowText,
                        m_hack.progress.completed ? Qt::darkGreen : Qt::red);
    m_ui->completionLabel->setPalette(textColour);
  }

  const IconSet &iconSet =
      selected ? getIconSetSelected(this) : getIconSetNormal(this);
...
void RhdcHackWidget::loadThumbnail() {
  ...
  const QString thumbnailPath = (BaseDir::cache() / _NFS("thumbnails") /
                                 fs::to_path(m_hack.info.hackId + ".png"))
                                    .u8string()
                                    .c_str();
...
void RhdcHackWidget::editorResized(int width) { setFixedWidth(width); }
