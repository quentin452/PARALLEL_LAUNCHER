...
RhdcLoginDialog::RhdcLoginDialog(QWidget *parent)
    : QDialog(parent), m_ui(new Ui::RhdcLoginDialog), m_exists(new bool(true)) {
  m_ui->setupUi(this);
...
void RhdcLoginDialog::browseForRom() {
  const fs::path romPath = FileDialog::getFile(
      tr("Select SM64 ROM").toStdString().c_str(), {{"z64 ROM", "*.z64"}});
  if (romPath.empty())
    return;

  if (Sha1::compute(romPath) != SM64_SHA1) {
    QMessageBox::critical(
        this, tr("ROM Does not Match"),
        tr("The provided rom does not match the expected checksum."));
    return;
  }

  DataProvider::addManualRomPath(romPath);
  DataProvider::addRomFile(
      {romPath, RomUtil::getLastModified(romPath), SM64_SHA1, true});
  DataProvider::addRomInfo({SM64_SHA1,
                            "Super Mario 64",
                            "SUPER MARIO 64",
                            ...
                            false,
                            false,
                            {N64InputType::Unknown, N64InputType::None,
                             N64InputType::None, N64InputType::None},
                            ""});
...
void RhdcLoginDialog::login() {
  ...
  RhdcApi::loginAsync(
      ...
      [this, dialogExists]() {
        ...
        this->m_ui->buttonTray->button(QDialogButtonBox::Cancel)
            ->setVisible(false);
        ...
        this->m_ui->downloadDirInput->setText(
            rhdcSettings.downloadDirectory.u8string().c_str());
        this->m_ui->enableStarDisplayCheckbox->setChecked(
            rhdcSettings.enableStarDisplay);
        this->m_ui->allSlotsCheckbox->setChecked(
            rhdcSettings.checkAllSaveSlots);
        this->m_ui->preferHleCheckbox->setChecked(rhdcSettings.preferHle);
        this->m_ui->enableDefaultLayoutsCheckbox->setChecked(
            rhdcSettings.enableDefaultLayouts);
        this->m_ui->starDisplaySubsection->setEnabled(
            rhdcSettings.enableStarDisplay);
        this->m_ui->ignoreWidescreenCheckbox->setChecked(
      ...
      [this, dialogExists](ApiErrorType error) {
        ...
        case ApiErrorType::NotAuthorized:
          this->m_ui->loginErrorMessage->setText(
              tr("Username or password is incorrect"));
          break;
        case ApiErrorType::ServerDown:
          this->m_ui->loginErrorMessage->setText(
              tr("The romhacking.com servers appear to be down."));
          break;
        default:
          this->m_ui->loginErrorMessage->setText(
