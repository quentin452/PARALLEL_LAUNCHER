...
static const char *s_difficultyLabels[11] = {
    /*:  0 */ QT_TRANSLATE_NOOP("RhdcRatingDialog",
...
static const char *s_kaizoLabels[11] = {
    /*:  0 */ QT_TRANSLATE_NOOP("RhdcRatingDialog",
                                "I can't decide or have no opinion"),
    /*:  1 */
    QT_TRANSLATE_NOOP("RhdcRatingDialog", "Beginner/Introductory Kaizo"),
    ...
    /*:  8 */
    QT_TRANSLATE_NOOP("RhdcRatingDialog", "Extremely Difficult Kaizo"),
    /*:  9 */
    QT_TRANSLATE_NOOP("RhdcRatingDialog", "Hardest humanly possible Kaizo"),
    /*: 10 */ QT_TRANSLATE_NOOP("RhdcRatingDialog", "TAS/Unbeatable")};

RhdcRatingDialog::RhdcRatingDialog(QWidget *parent)
    : QDialog(parent), m_ui(new Ui::RhdcRatingDialog), m_kaizo(false) {
  m_ui->setupUi(this);
...
RhdcRatingDialog::~RhdcRatingDialog() { delete m_ui; }

void RhdcRatingDialog::setRatings(RhdcRating quality, ubyte difficulty,
                                  bool isKaizo) {
  m_kaizo = isKaizo;
  m_rating = quality;
...
RhdcRating RhdcRatingDialog::getQuality() const { return m_rating; }
...
static QString makeScoreLabel(int score) {
  if (score == 0)
    return RhdcRatingDialog::tr("Not Rated");
...
void RhdcRatingDialog::difficultyChanged(int rating) {
  ...
  m_ui->difficultyInfoLabel->setText(m_kaizo ? tr(s_kaizoLabels[rating])
                                             : tr(s_difficultyLabels[rating]));
}

void RhdcRatingDialog::updateButtons() {
  m_ui->dislikeButton->setChecked(m_rating == RhdcRating::Disliked);
  m_ui->dislikeButton->setIcon((m_rating == RhdcRating::Disliked)
                                   ? Icon::ratingDislikedFilled()
                                   : Icon::ratingDisliked());
  m_ui->neutralButton->setChecked(m_rating == RhdcRating::Neutral);
  m_ui->neutralButton->setIcon((m_rating == RhdcRating::Neutral)
                                   ? Icon::ratingNeutralFilled()
                                   : Icon::ratingNeutral());
  m_ui->likeButton->setChecked(m_rating == RhdcRating::Liked);
  m_ui->likeButton->setIcon((m_rating == RhdcRating::Liked)
                                ? Icon::ratingLikedFilled()
                                : Icon::ratingLiked());
  m_ui->loveButton->setChecked(m_rating == RhdcRating::Loved);
  m_ui->loveButton->setIcon((m_rating == RhdcRating::Loved)
                                ? Icon::ratingLovedFilled()
                                : Icon::ratingLoved());
}
...
void RhdcRatingDialog::likedClicked() { setQualityRating(RhdcRating::Liked); }

void RhdcRatingDialog::lovedClicked() { setQualityRating(RhdcRating::Loved); }
