...
RhdcViewBubble::RhdcViewBubble(QWidget *parent, QPushButton *target)
    : QWidget(parent), m_ui(new Ui::RhdcViewBubble), m_target(target) {
  m_ui->setupUi(this);
...
void RhdcViewBubble::reposition() {
  ...
  const QPoint position = parentWidget()->mapFromGlobal(
      m_target->mapToGlobal(QPoint(m_target->width() / 2, m_target->height())));

  adjustSize();
  setGeometry(position.x() - 264, position.y(), 320, height());
}
