...
static QPixmap pixmapFromData(const std::vector<ubyte> &data) {
  ...
  const QByteArray imageData =
      QByteArray::fromRawData((char *)data.data(), (int)data.size());
  return QPixmap::fromImage(QImage::fromData(imageData));
}

StarDisplayWidget::StarDisplayWidget(QWidget *parent, StarLayout &&layout)
    : QTabWidget(parent), m_layout(layout) {
  setSizePolicy(
...
void StarDisplayWidget::setup(bool editable) {
  ...
  if (missingStarImage.isNull()) {
    QImage shadow =
        collectedStarImage.toImage().convertToFormat(QImage::Format_ARGB32);
  ...
  for (int i = 0; i < (int)m_layout.numSlots; i++) {
    ...
    tab->setSizePolicy(QSizePolicy(QSizePolicy::MinimumExpanding,
    ...
    for (const StarLayout::Group &group : m_layout.groups) {
      ...
      switch (group.side) {
      case StarLayout::Side::Top:
        uiColumn = sides.top;
        break;
      case StarLayout::Side::Right:
        uiColumn = sides.right;
        break;
      case StarLayout::Side::Bottom:
        uiColumn = sides.bottom;
        break;
      default:
        uiColumn = sides.left;
        break;
      ...
      for (const StarLayout::Course &course : group.courses) {
        ...
        for (const StarLayout::StarData &starData : course.stars) {
          StarSetUi *starSet = new StarSetUi(
              nullptr, collectedStarImage, missingStarImage, editable,
              slotStart + starData.offset, activeBit, starData.mask);
...
void StarDisplayWidget::makeScrollable() {
  for (int i = 0; i < count(); i++) {
    ...
    if (dynamic_cast<const QScrollArea *>(tab) != nullptr)
      return;
...
LiveStarDisplayWidget::LiveStarDisplayWidget(QWidget *parent,
  ...
  connect(&m_fileWatcher, &QFileSystemWatcher::fileChanged, this,
          &LiveStarDisplayWidget::saveFileChanged);
...
void LiveStarDisplayWidget::saveFileChanged() {
  ...
  for (StarSetUi *starSet : m_starSets)
    starSet->load(saveData);
}

SDWLayoutSides LiveStarDisplayWidget::makeColumnContainer(QWidget *parent,
                                                          int) {
  ...
  columnsWidget->setSizePolicy(QSizePolicy(QSizePolicy::MinimumExpanding,
                                           QSizePolicy::MinimumExpanding));
...
EditableStarDisplayWidget::EditableStarDisplayWidget(
    QWidget *parent, const fs::path &saveFilePath, StarLayout &&layout)
    : StarDisplayWidget(parent, std::move(layout)),
      m_saveFilePath(saveFilePath) {
  load();
}

SDWLayoutSides EditableStarDisplayWidget::makeColumnContainer(QWidget *parent,
                                                              int slot) {
  ...
  sidesWidget->setSizePolicy(QSizePolicy(QSizePolicy::MinimumExpanding,
                                         QSizePolicy::MinimumExpanding));
