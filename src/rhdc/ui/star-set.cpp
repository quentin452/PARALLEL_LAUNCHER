...
StarSetUi::StarSetUi(QWidget *parent, const QPixmap &checkedImage,
                     const QPixmap &uncheckedImage, bool editable,
                     uint byteIndex, uint activeBit, ubyte mask, ubyte value)
    : QHBoxLayout(parent), m_byteIndex(byteIndex), m_activeByte(activeBit / 8),
      m_activeMask(1 << (7 - (activeBit % 8))), m_mask(mask) {
  ...
  while (mask != 0) {
    ...
    addWidget(new StarBitToggle(nullptr, checkedImage, uncheckedImage, editable,
                                (value & testBit) != 0));
    mask -= testBit;
  }
}
...
void StarSetUi::setValue(ubyte byte) {
  ...
  while (mask != 0) {
    const ubyte testBit = mask - (mask & (mask - 1));
    static_cast<StarBitToggle *>(itemAt(i++)->widget())
        ->setChecked(byte & testBit);
    mask -= testBit;
  }
}
