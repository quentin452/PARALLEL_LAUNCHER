...
static inline OSInfo getOS() {
	...
	if( osVersion == "10" ) {
		string queryResult;
		if( Process::tryGetOutput( "wmic /LOCALE:MS_409 path win32_operatingsystem get caption", queryResult ) ) {
...
QNetworkRequest ApiUtil::makeRequest( const string &url, const char *contentType, bool tryAuthorize ) {
...
void ApiUtil::onResponse(
	...
	QObject::connect( response, &QNetworkReply::finished, response->manager(), [=]() {
		...
		if( statusCode == HttpStatusCode::None ) {
			errorType = ApiUtil::isOnline() ? ApiErrorType::ServerDown : ApiErrorType::Offline;
		}
...
void ApiUtil::logResponseError( QNetworkReply *response ) {
	...
	const string msg =
		"Unexpected API error response."s +
		"\nURL: " + url +
...
std::function<void(ApiErrorType)> RhdcApi::logApiError(
...
string RhdcApi::getDownloadUrl(
	const string &slug,
	const string &versionName
) {
	return string( RHDC_API_HOST "/game/" ) + slug + '/' + ApiUtil::urlEncode( versionName );
}
