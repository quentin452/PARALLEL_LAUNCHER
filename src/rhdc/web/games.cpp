...
static inline string tryGetDownloadUrl( const Json &token ) {
	if( !token.exists() || token.isNull() ) return "";
	return string( RHDC_API_HOST ) + urlDecode( token["directHref"].getOrDefault<string>( "" ) );
}

static string tryGetVersionName( const Json &token ) {
	const string versionUrl =  tryGetDownloadUrl( token );
	if( versionUrl.empty() ) return "";

	const auto namePos = versionUrl.find_last_of( '/' );
	if( namePos == string::npos ) return versionUrl;

	return versionUrl.substr( namePos + 1 );
}

static std::vector<RhdcHackVersionExt> parseVersions( const JArray &versionsJson ) {
...
void RhdcApi::getFollowedHacksAsync(
	...
	ApiUtil::onResponse( response, [=](){
		...
		if( !json.isArray() ) {
			logError( "Unexpected JSON format in response from RHDC following query" );
		...
		for( const Json &hack : followedHacksJson ) {
			const std::vector<RhdcHackVersionExt> versions = parseVersions( hack["versions"].array() );
			...
			const string layoutUpdatedStr = hack["layoutUpdateTime"].getOrDefault<string>( "" );
			if( !layoutUpdatedStr.empty() ) {
				bool validDate = false;
				layoutUpdated = Time::tryParseIso( layoutUpdatedStr, validDate );
				if( !validDate ) layoutUpdated = 0;
			}
...
void RhdcApi::getHackAsync(
	...
	QNetworkReply *response = ApiUtil::send( HttpMethod::Get, string( RHDC_API_HOST "/v3/hacks/hack/" ) + hackSlug );
	ApiUtil::onResponse( response, [=](){
		const Json json = ApiUtil::readJson( response );
		...
		int64 layoutUpdated = 0;
		const string layoutUpdatedStr = json["layoutUpdateTime"].getOrDefault<string>( "" );
		if( !layoutUpdatedStr.empty() ) {
			bool validDate = false;
			layoutUpdated = Time::tryParseIso( layoutUpdatedStr, validDate );
			if( !validDate ) layoutUpdated = 0;
		...
		RhdcVersionedHack hack = {
			...
			parseVersions( json["versions"].array() ),
			std::move( authors ),
			layoutUrl,
			layoutUpdated
		};
...
void RhdcApi::getLayoutAndRecommendedPluginAsync(
	...
	OPTIONAL_LOGIN( [=](){ getLayoutAndRecommendedPluginAsync( hackId, versionName, onSuccess, onFailure ); })
	QNetworkReply *response = ApiUtil::send( HttpMethod::Get, string( RHDC_API_HOST "/v3/hacks/hack/" ) + hackId );
	ApiUtil::onResponse( response, [=](){
		const Json json = ApiUtil::readJson( response );
		if( !json.isObject() || !json["versions"].isArray() ) {
			logError( "Unexpected JSON format in response from RHDC game query" );
		...
		for( const Json &version : versionsJson ) {
			if( tryGetVersionName( version["download"] ) != versionName ) continue;
...
void RhdcApi::addHackToListAsync(
	...
	QNetworkReply *response = ApiUtil::send( HttpMethod::Post, string( RHDC_API_HOST "/v3/playlists/name/" ) + ApiUtil::urlEncode( group ) + "/hack/" + hackId );
...
void RhdcApi::removeHackFromListAsync(
	...
	QNetworkReply *response = ApiUtil::send( HttpMethod::Delete, string( RHDC_API_HOST "/v3/playlists/name/" ) + ApiUtil::urlEncode( group ) + "/hack/" + hackId );
	ApiUtil::onResponse( response, onSuccess, onFailure );
}
...
void RhdcApi::deleteListAsync(
	...
	QNetworkReply *response = ApiUtil::send( HttpMethod::Delete, string( RHDC_API_HOST "/v3/playlists/name/" ) + ApiUtil::urlEncode( group ) );
	ApiUtil::onResponse( response, onSuccess, onFailure );
}
...
void RhdcApi::downloadHack(
	...
	OPTIONAL_LOGIN( [=](){ downloadHack( downloadUrl, destinationFile, onProgress, onSuccess, onFailure ); })
...
void RhdcApi::downloadThumbnail(
	...
	OPTIONAL_LOGIN( [=](){ downloadThumbnail( hackId, destinationFile, onSuccess, onFailure ); })
	QNetworkReply *response = ApiUtil::send( HttpMethod::Get, string( RHDC_API_HOST "/v3/hacks/hack/" ) + hackId );
	ApiUtil::onResponse( response, [=]() {
		const Json json = ApiUtil::readJson( response );
		...
		const string screenshotHref = screenshots[0]["directHref"].getOrDefault<string>( "" );
		if( screenshotHref.empty() ) {
			onFailure( ApiErrorType::NotFound );
			return;
		}

		QNetworkReply *download = ApiUtil::send( HttpMethod::Get, string( RHDC_API_HOST ) + screenshotHref );
...
static void checksumListSerializer( JsonWriter &jw, const std::vector<string> &checksums ) {
	jw.writeArrayStart();
...
void RhdcApi::getPluginSettings(
	...
	const std::function<void(const std::vector<RhdcSettingHintsExt>&)> &onSuccess,
	const std::function<void(ApiErrorType)> &onFailure,
	int timeoutMs
) {
	QNetworkReply *response = ApiUtil::send( HttpMethod::Post, string( RHDC_API_HOST "/v3/hacks/plugins" ), checksums, checksumListSerializer );
...
extern void RhdcApi::getPluginSettings(
	const string &sha1,
	const std::function<void(const std::optional<RhdcSettingHintsExt>&)> &onSuccess,
	const std::function<void(ApiErrorType)> &onFailure,
	int timeoutMs
) {
	RhdcApi::getPluginSettings( std::vector<string>{ sha1 }, [=](const std::vector<RhdcSettingHintsExt> &results) {
		if( results.empty() ) {
