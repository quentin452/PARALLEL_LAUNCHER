...
static bool decodenum(const uint8_t *&ptr, size_t &out) {
  ...
  while (true) {
    ...
    if (!try_shift(addthis, shift))
      return false;
    // unchecked because if it was shifted, the lowest bit is zero, and if not,
    // it's <=0x7F.
...
enum bpserror bps_apply(struct mem patch, struct mem in, struct mem *out,
                        struct mem *metadata, bool accept_wrong_input) {
...
#define write8(byte) (*(outat++) = byte)
    ...
    if (metadata && metadatalen) {
      ...
      metadata->ptr[metadatalen] =
          '\0'; // just to be on the safe side - that metadata is assumed to be
                // text, might as well terminate it
    } else {
      for (size_t i = 0; i < metadatalen; i++)
        (void)read8();
    }
    ...
    while (patchat < patchend) {
      ...
      switch (action) {
      case SourceRead: {
        if (outat - outstart + length > in.len)
          error(bps_broken);
      ...
      case SourceCopy: {
        ...
        if (inreadat < instart || inreadat + length > inend)
          error(bps_broken);
        for (size_t i = 0; i < length; i++)
          write8(*inreadat++);
      } break;
      case TargetCopy: {
        ...
        if (outreadat < outstart || outreadat >= outat ||
            outreadat + length > outend)
          error(bps_broken);
        for (size_t i = 0; i < length; i++)
          write8(*outreadat++);
      } break;
...
enum bpserror bps_create_linear(struct mem sourcemem, struct mem targetmem,
                                struct mem metadata, struct mem *patchmem) {
  if (sourcemem.len >= (SIZE_MAX >> 2) - 16)
    return bps_too_big; // the 16 is just to be on the safe side, I don't think
                        // it's needed.
  if (targetmem.len >= (SIZE_MAX >> 2) - 16)
    return bps_too_big;

  const uint8_t *source = sourcemem.ptr;
  const uint8_t *sourceend = sourcemem.ptr + sourcemem.len;
  if (sourcemem.len > targetmem.len)
    sourceend = sourcemem.ptr + targetmem.len;
  ...
  while (target < targetend) {
    ...
    while (source + numunchanged < sourceend &&
           source[numunchanged] == target[numunchanged])
      numunchanged++;
    ...
    size_t numchanged = 0;
    if (lastknownchange > target)
      numchanged = lastknownchange - target;
    while ((source + numchanged >= sourceend ||
      ...
      numchanged++;
      if (source + numchanged >= sourceend)
        numchanged = targetend - target;
    }
    ...
    if (numchanged) {
      ...
      while (true) {
        if (target[rle1start - 1] == target[rle1start + 0] &&
        ...
        if (target[rle1start - 2] == target[rle1start + 0] &&
      ...
      if (target[-2] == target[0] && target[-1] == target[1] &&
          target[0] == target[2]) {
        // two-byte RLE
        size_t rlelen = 0;
        while (target + rlelen < targetend && target[0] == target[rlelen + 0] &&
               target[1] == target[rlelen + 1])
          rlelen += 2;
        ...
        // one-byte RLE
        size_t rlelen = 0;
        while (target + rlelen < targetend && target[0] == target[rlelen])
          rlelen++;
        writenum((rlelen - 1) << 2 | TargetCopy);
  ...
  // while this may look like it can be fooled by a patch containing exactly one
  // of any other command, it
  //   can't, because the ones that aren't SourceRead requires an argument
  size_t i;
  for (i = mainContentPos; (out[i] & 0x80) == 0x00; i++) {
  }
  if (i == outlen - 12 - 1)
    return bps_identical;
...
void bps_free(struct mem mem) { free(mem.ptr); }
#undef error
...
#define error(why)                                                             \
  ...
  uint8_t top[256];
  if (!patch->read(top, 0, len > 256 ? 256 : len))
    error(bps_io);
  if (memcmp(top, "BPS1", 4))
    error(bps_broken);

  const uint8_t *patchdat = top + 4;
  if (!decodenum(patchdat, ret.size_in))
    error(bps_too_big);
  if (!decodenum(patchdat, ret.size_out))
    error(bps_too_big);

  if (!decodenum(patchdat, ret.meta_size))
    error(bps_too_big);
  ret.meta_start = patchdat - top;

  uint8_t checksums[12];
  if (!patch->read(checksums, len - 12, 12))
    error(bps_io);
  ...
  if (changefrac && ret.size_in > 0) {
    // algorithm: each command adds its length to the numerator, unless it's
    // above 32, in which case
    ...
    while (patchat < patchend && outpos < ret.size_in) {
      ...
      size_t length = (thisinstr >> 2) + 1;
      int action = (thisinstr & 3);
      size_t min_len_32 = (length < 32 ? length : 32);
    ...
    if (patchat > patchend || outpos > ret.size_out)
      error(bps_broken);
    ...
    // this also happens if change fraction is not requested, but it's undefined
    // behaviour anyways.
    ret.change_num = 1;
    ret.change_denom = 1;
  }
...
// Congratulations, you found the undocumented features! They disassemble a
// patch, telling what it
//  does; compare two equivalent BPS patches and tells where each one is more
//  compact.
// They crash or give bogus answers on invalid patches, latter also misbehaves
// on non-equivalent ones. Have fun.
...
#define decodeto(var) decodenum(patchat, var)
  ...
  printf("sanity check: %zu=%zu (%+zi), ", patchat - patch.ptr,
         patchend - patch.ptr, patchat - patchend);
...
void bps_compare(struct mem patch1mem, struct mem patch2mem) {
  ...
  size_t patchcopypos[2][4] = {0, 0}; //[0] and [1] are unused, but this is just
                                      //debug code, it doesn't need to be neat.
...
#define decodeto(id, var)                                                      \
  ...
  size_t tempuint;
  decodeto(0, tempuint);
  decodeto(1, tempuint); // source-size
  decodeto(0, outlen);
  decodeto(1, outlen); // target-size
  decodeto(0, tempuint);
  patch[0] += tempuint; // metadata
  decodeto(1, tempuint);
  patch[1] += tempuint; // metadata

  bool show = false;
  while (patchpos[0] < patchlen[0] && patchpos[1] < patchlen[1]) {
    bool step[2] = {(patchoutpos[0] <= patchoutpos[1]),
                    (patchoutpos[0] >= patchoutpos[1])};
    char describe[2][256];
    for (int i = 0; i < 2; i++) {
      if (step[i]) {
        ...
        // enum { SourceRead, TargetRead, SourceCopy, TargetCopy };
        const char *actionnames[] = {"SourceRead", "TargetRead", "SourceCopy",
                                     "TargetCopy"};
        ...
        if (action == SourceCopy || action == TargetCopy) {
          ...
          sprintf(describe[i], "%s from %zu (%+i) for %zu in %zu",
                  actionnames[action], patchcopypos[i][action], delta, len,
                  patchpos[i] - patchposstart);
          patchcopypos[i][action] += len;
        } else
          sprintf(describe[i], "%s from %zu for %zu in %zu",
                  actionnames[action], patchoutpos[i], len,
                  patchpos[i] - patchposstart);
        if (!step[i ^ 1]) {
          printf("%i: %s\n", i + 1, describe[i]);
          show = true;
    ...
    if (patchoutpos[0] == patchoutpos[1]) {
      size_t used[2] = {patchpos[0] - patchposatmatch[0],
                        patchpos[1] - patchposatmatch[1]};
      char which = '=';
      if (used[0] < used[1])
        which = '+';
      if (used[0] > used[1])
