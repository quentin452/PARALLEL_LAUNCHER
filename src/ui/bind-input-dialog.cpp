...
BindInputDialog::BindInputDialog(const QString &input, GamepadId gamepadId,
                                 bool showSkipButton)
    : QDialog(nullptr), m_ui(new Ui::BindInputDialog), m_gamepadId(gamepadId),
  ...
  connect(&GamepadController::instance(),
          SIGNAL(gamepadAxisMoved(GamepadAxisEvent)), this,
          SLOT(axisMoved(GamepadAxisEvent)));
  connect(&GamepadController::instance(),
          SIGNAL(gamepadHatMoved(GamepadHatEvent)), this,
          SLOT(hatMoved(GamepadHatEvent)));
...
void BindInputDialog::buttonPressed(GamepadButtonEvent event) {
  ...
  m_binding = {BindingType::Button, (ushort)event.button};
  cleanup();
  accept();
}

void BindInputDialog::axisMoved(GamepadAxisEvent event) {
  if (event.id != m_gamepadId)
    return;

  if (std::abs(event.position) > AXIS_MOVE_THRESHOLD &&
      std::abs(event.position - m_initialState.axes.at(event.axis)) >
          AXIS_MOVE_THRESHOLD) {
...
void BindInputDialog::axisHeld() {
  for (const auto &i : m_axisTimers) {
    if (i.second->remainingTime() <= 0) {
      const GamepadState currentState =
          GamepadController::instance().getState(m_gamepadId);
      if (i.first >= currentState.axes.size())
        continue;

      m_binding = {currentState.axes.at(i.first) > 0
                       ? BindingType::AxisPositive
                       : BindingType::AxisNegative,
                   (ushort)i.first};
