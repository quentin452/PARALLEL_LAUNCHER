...
static_assert(sizeof(s_actionNames) ==
              sizeof(char *) * (size_t)ControllerAction::NUM_ACTIONS);

ControllerConfigDialog::ControllerConfigDialog(QWidget *parent)
    : QDialog(parent), m_ui(new Ui::ControllerConfigDialog),
      m_controllerId(-1) {
  m_ui->setupUi(this);
  UiUtil::scaleWindow(this);

  m_profile =
      DefaultProfile::get(FileController::loadAppSettings().inputDriver);
  ...
  connect(m_ui->bindingButtonGroup, SIGNAL(buttonClicked(QAbstractButton *)),
          this, SLOT(bindSingle(QAbstractButton *)));
}

ControllerConfigDialog::~ControllerConfigDialog() { delete m_ui; }

static void updateButtonText(QAbstractButton *button,
                             const ControllerProfile &profile) {
  const Binding &binding =
      profile.bindings[button->property("controllerBinding").toInt()];
  switch (binding.type) {
  case BindingType::None:
    button->setText(
        QCoreApplication::translate("ControllerConfigDialog", "Not Bound"));
    break;
  case BindingType::Button:
    button->setText(
        QCoreApplication::translate("ControllerConfigDialog", "Button %1")
            .arg(Number::toString(binding.buttonOrAxis).c_str()));
    break;
  case BindingType::AxisNegative:
    button->setText(
        QCoreApplication::translate("ControllerConfigDialog", "Axis -%1")
            .arg(Number::toString(binding.buttonOrAxis).c_str()));
    break;
  ...
  case BindingType::HatUp:
    button->setText(
        QCoreApplication::translate("ControllerConfigDialog", "Hat %1 Up")
            .arg(Number::toString(binding.buttonOrAxis).c_str()));
    break;
  case BindingType::HatDown:
    button->setText(
        QCoreApplication::translate("ControllerConfigDialog", "Hat %1 Down")
            .arg(Number::toString(binding.buttonOrAxis).c_str()));
    break;
  case BindingType::HatLeft:
    button->setText(
        QCoreApplication::translate("ControllerConfigDialog", "Hat %1 Left")
            .arg(Number::toString(binding.buttonOrAxis).c_str()));
    break;
  case BindingType::HatRight:
    button->setText(
        QCoreApplication::translate("ControllerConfigDialog", "Hat %1 Right")
            .arg(Number::toString(binding.buttonOrAxis).c_str()));
    break;
  }
}
...
void ControllerConfigDialog::setActiveController(
    const string &name, const Uuid &uuid, int id,
    const ControllerProfile &profile) {
  ...
  m_ui->saveButton->setVisible(!DefaultProfile::exists(
      FileController::loadAppSettings().inputDriver, profile.name));
}
...
void ControllerConfigDialog::save() {
  ...
  std::map<string, ControllerProfile> savedProfiles =
      FileController::loadControllerProfiles(inputDriver);
...
void ControllerConfigDialog::saveAs() {
  QString profileName = QInputDialog::getText(
      this, tr("Enter Profile Name"),
      tr("Enter a name for your new controller profile."));
  if (profileName.isNull() || profileName.isEmpty()) {
    return;
  }

  if (DefaultProfile::exists(FileController::loadAppSettings().inputDriver,
                             profileName.toStdString())) {
    QMessageBox::critical(
        this, tr("Failed to Save Profile"),
        tr("A default controller profile with that name already exists."));
    return;
  }
...
void ControllerConfigDialog::bindAll() {
  ...
  if (cancelled) {
    std::memcpy(m_profile.bindings, previousBindings,
                sizeof(m_profile.bindings));
    for (QAbstractButton *button : m_ui->bindingButtonGroup->buttons()) {
      updateButtonText(button, m_profile);
    }
  }
