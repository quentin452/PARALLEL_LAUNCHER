...
bool CoreInstaller::requireCore(EmulatorCore core) {
  ...
  if (RetroArch::isEmulatorCoreInstalled(core)) {
    ...
    if (core != EmulatorCore::ParallelN64 ||
        FileController::loadInstalledVersions().parallelVersion >=
            Version{2, 18, 3}) {
      return true;
    }
  }

  pl_assert(core == EmulatorCore::ParallelN64 ||
  ...
  if (core == EmulatorCore::Mupen64plusNext) {
    QMessageBox::critical(
        ...
        QCoreApplication::translate(
            "CoreInstaller",
            "Sorry! The Mupen64plus-Next emulator core is not available on "
            "MacOS. Try using ParallelN64 instead."));
    return false;
  }
#endif
  ...
  if (QMessageBox::question(
          ...
          QCoreApplication::translate(
              "CoreInstaller", "This emulator core is not installed or is out "
                               "of date. Would you like to install it now?")) !=
      QMessageBox::Yes) {
    return false;
  }
  ...
  if (core == EmulatorCore::Mupen64plusNext) {
    ...
    const std::optional<ParallelCoreVersion> &coreBuild =
        coreFinder.coreBuild();
    if (coreBuild.has_value()) {
