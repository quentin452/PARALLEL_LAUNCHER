...
static bool installMupenCoreSync( const CoreBuild &coreBuild ) {
	...
	const DownloadResult status = DownloadDialog::download( QT_TRANSLATE_NOOP( "DownloadDialog", "Downloading core..." ), coreBuild.downloadLink, tempPath );
	if( !status.success ) {
		QMessageBox::critical(
			nullptr,
			QCoreApplication::translate( "CoreInstaller", "Download Failed" ),
			QCoreApplication::translate( "CoreInstaller", "Failed to download emulator core" ).append( '\n' ).append( status.errorMessage.c_str() )
		);
		return false;
	}
	...
	if( !Zip::unzip( tempPath, RetroArch::getCorePath() ) ) {
		QMessageBox::critical(
			...
			QCoreApplication::translate( "CoreInstaller", "Failed to install emulator core" )
		);
		return false;
	}
	...
	QMessageBox::information(
		...
		QCoreApplication::translate( "CoreInstaller", "Core installed successfully." )
	);
...
static bool installParallelCoreSync( const ParallelCoreVersion &version ) {
	const fs::path tempPath = BaseDir::temp() / _NFS("parallel_n64_next_libretro.zip");
	const fs::path corePath = RetroArch::getCorePath() / (_NFS("parallel_n64_next_libretro") LIBRARY_EXT);
	const fs::path legacyCorePath = RetroArch::getCorePath() / (_NFS("parallel_n64_libretro") LIBRARY_EXT);

	fs::error_code err;
	fs::remove( tempPath, err );

	const DownloadResult status = DownloadDialog::download( QT_TRANSLATE_NOOP( "DownloadDialog", "Downloading core..." ), version.downloadUrl, tempPath );
	if( !status.success ) {
		QMessageBox::critical(
			nullptr,
			QCoreApplication::translate( "CoreInstaller", "Download Failed" ),
	...
	if( !success ) logError( "Failed to unzip "s + tempPath.string() + " to " + RetroArch::getCorePath().string() );

	if( success ) {
		success = fs::existsSafe( corePath ) || fs::existsSafe( legacyCorePath );
		if( !success ) logError( "Unzip was successful, but the expected file was not found." );
	}
	...
	if( success ) {
		...
		if( !success ) logError( "Checksum of downloaded core did not match the expected value" );
	}
...
bool CoreInstaller::requireCore( EmulatorCore core ) {
	...
	if( RetroArch::isEmulatorCoreInstalled( core ) ) {
		// Require minimum version of ParallelN64 core
		if(
			core != EmulatorCore::ParallelN64 ||
			FileController::loadInstalledVersions().parallelVersion >= Version{ 2, 18, 3 }
		) {
			return true;
		}
	}

	pl_assert( core == EmulatorCore::ParallelN64 || core == EmulatorCore::Mupen64plusNext );

#ifdef __APPLE__
	if( core == EmulatorCore::Mupen64plusNext ) {
		QMessageBox::critical(
			nullptr,
			QCoreApplication::translate( "CoreInstaller", "Core Unavailable" ),
			QCoreApplication::translate( "CoreInstaller", "Sorry! The Mupen64plus-Next emulator core is not available on MacOS. Try using ParallelN64 instead." )
		);
		return false;
	}
#endif
	...
	if( QMessageBox::question(
		nullptr,
		QCoreApplication::translate( "CoreInstaller", "Install Emulator Core?" ),
		QCoreApplication::translate( "CoreInstaller", "This emulator core is not installed or is out of date. Would you like to install it now?" )
	) != QMessageBox::Yes ) {
		return false;
	}
...
static void checkForMupenUpdates(
	...
	MupenCoreBuilds::getLastKnownGood(
		...
		[=](const CoreBuild &build) {
			if( QMessageBox::question(
				nullptr,
				QCoreApplication::translate( "CoreInstaller", "Core Update Available" ),
				QCoreApplication::translate( "CoreInstaller", "An update is available for %1. Would you like to install it now?" ).arg( "Mupen64Plus-Next" )
			) == QMessageBox::Yes ) {
...
static void checkForParallelUpdates(
	const InstalledVersionsInfo &currentVersionInfo,
	std::function<void(void)> then
) {
	...
	ParallelCoreBuilds::getLatest(
		[=](const ParallelCoreVersion &latest){
			const string currentHash = Sha1::compute( RetroArch::getCorePath() / (_NFS("parallel_n64_next_libretro") LIBRARY_EXT) );
			if( currentHash == latest.sha1 && currentVersionInfo.parallelVersion == latest.version ) {
				then();
				return;
			}
			...
			if( QMessageBox::question(
				nullptr,
				QCoreApplication::translate( "CoreInstaller", "Core Update Available" ),
				QCoreApplication::translate( "CoreInstaller", "An update is available for %1. Would you like to install it now?" ).arg( "ParallelN64-Next" )
			) == QMessageBox::Yes ) {
...
void CoreInstaller::checkForUpdatesAsync() {
	...
	checkForParallelUpdates(
		versions,
		[=]() {
			QMetaObject::invokeMethod( QCoreApplication::instance(), [=]() {
				checkForMupenUpdates(
					versions,
					[=](){
						InstalledVersionsInfo updatedVersions = FileController::loadInstalledVersions();
...
void CoreInstaller::updateParallelSync() {
	...
	if( coreBuild.has_value() ) {
		const string currentHash = Sha1::compute( RetroArch::getCorePath() / (_NFS("parallel_n64_next_libretro") LIBRARY_EXT) );
		if( currentHash == coreBuild.value().sha1 ) return;
