...
FlowLayout::FlowLayout(QWidget *parent, QSize &&spacing)
...
int FlowLayout::doLayout(const QRect &rect, bool test) const {
  ...
  const QRect contentRegion = rect.adjusted(
  ...
  for (QLayoutItem *item : m_items) {
    ...
    if (spaceWidth < 0) {
      spaceWidth = item->widget()->style()->layoutSpacing(
          QSizePolicy::PushButton, QSizePolicy::PushButton, Qt::Horizontal);
    }

    int spaceHeight = spacing.height();
    if (spaceHeight < 0) {
      spaceHeight = item->widget()->style()->layoutSpacing(
...
QSize FlowLayout::getItemSpacing() const {
  ...
  if (parent()->isWidgetType()) {
    ...
    return QSize(
        m_spacing.width() >= 0
            ? m_spacing.width()
            : parentWidget->style()->pixelMetric(
                  QStyle::PM_LayoutHorizontalSpacing, nullptr, parentWidget),
        m_spacing.height() >= 0
            ? m_spacing.height()
            : parentWidget->style()->pixelMetric(
                  QStyle::PM_LayoutVerticalSpacing, nullptr, parentWidget));
  } else {
