...
static const char *const s_textNone =
    QT_TRANSLATE_NOOP("HotkeyEditWidget", "None");

HotkeyEditWidget::HotkeyEditWidget(QWidget *parent)
    : QPushButton(parent), m_hotkey(KeyId::INVALID), m_active(false) {
...
void HotkeyEditWidget::clear() { setValue(KeyId::INVALID); }
...
static inline uint getKeyCode(QKeyEvent *event) {
  ...
  case Qt::Key_Shift:
    return 56;
  case Qt::Key_Meta:
    return 59; // Actually Ctrl
  case Qt::Key_Alt:
    return 58;
  case Qt::Key_Control:
    return 55; // Actually Super/Command
  default:
...
HotkeyEditPanel::HotkeyEditPanel(QWidget *parent)
    : QWidget(parent), m_ui(new Ui::HotkeyEditPanel) {
  m_ui->setupUi(this);
  m_ui->clearButton->setIcon(Icon::clear());
  UiUtil::setIconSize(m_ui->clearButton, 16);
  connect(m_ui->hotkeyEdit, SIGNAL(hotkeyChanged(KeyId)), this,
          SIGNAL(hotkeyChanged(KeyId)));
}

HotkeyEditPanel::~HotkeyEditPanel() { delete m_ui; }

KeyId HotkeyEditPanel::value() const { return m_ui->hotkeyEdit->value(); }

void HotkeyEditPanel::setValue(KeyId hotkey) {
  m_ui->hotkeyEdit->setValue(hotkey);
}

void HotkeyEditPanel::clear() { m_ui->hotkeyEdit->clear(); }
