...
static inline size_t min(size_t a, size_t b) noexcept { return a < b ? a : b; }
...
ImportSdCardDialog::ImportSdCardDialog(QWidget *parent,
                                       const fs::path &filePath)
    : QDialog(parent), m_filePath(filePath), m_ui(new Ui::ImportSdCardDialog) {
  m_ui->setupUi(this);
  ...
  if (!name.empty() && m_usedNames.count(name) > 0) {
    ...
    do {
      ...
      name = name.substr(0, min(baseLength, (size_t)11 - suffix.length())) +
             suffix;
    } while (
#ifdef _WIN32
...
#ifdef _WIN32
  if (name == QString("CON") || name == QString("PRN") ||
      name == QString("AUX") || name == QString("NUL") ||
      (name.length() == 4 &&
