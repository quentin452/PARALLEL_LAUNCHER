...
IsViewerWindow::IsViewerWindow() : QMainWindow() {
  ...
  view->setMaximumBlockCount(
      FileController::loadAppSettings().isViewerHistorySize);
#ifdef __APPLE__
...
IsViewerWindow::~IsViewerWindow() { centralWidget()->deleteLater(); }
...
void IsViewerWindow::error(bool connected) {
  const QString message =
      connected ? tr("Error: IS Viewer was disconnected from the emulator.")
  ...
  QTextBlockFormat format = cursor.blockFormat();
  format.setBackground((cursor.blockNumber() % 2)
                           ? view->palette().brush(QPalette::AlternateBase)
                           : view->palette().brush(QPalette::Base));
