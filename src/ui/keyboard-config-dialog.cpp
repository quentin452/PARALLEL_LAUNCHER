...
KeyboardConfigDialog::KeyboardConfigDialog(QWidget *parent)
    : QDialog(parent), m_ui(new Ui::KeyboardConfigDialog) {
  m_ui->setupUi(this);
  ...
  QFormLayout *controlsLayout =
      dynamic_cast<QFormLayout *>(m_ui->controlsArea->layout());
  QFormLayout *hotkeysLayout =
      dynamic_cast<QFormLayout *>(m_ui->hotkeysArea->layout());
  ...
  QLayoutItem *rewindLabel = hotkeysLayout->itemAt(
      (int)((size_t)Hotkey::Rewind - NUM_CONTROLS), QFormLayout::LabelRole);
  pl_assert(rewindLabel != nullptr);
  pl_assert(rewindLabel->widget() != nullptr);

  rewindLabel->widget()->setToolTip(
      tr("You must enable rewind functionality in the RetroArch menu to use "
         "this hotkey."));
}

KeyboardConfigDialog::~KeyboardConfigDialog() { delete m_ui; }
