...
LogViewerEntry::LogViewerEntry(QWidget *parent, LogEntry &&entry)
    : QFrame(parent), m_entry(entry), m_timestampLabel(new QLabel(this)),
  ...
  m_timestampLabel->setText(QDateTime::fromMSecsSinceEpoch(m_entry.timestamp)
                                .toLocalTime()
...
LogViewerDialog::LogViewerDialog(QWidget *parent)
    : QDialog(parent), m_ui(new Ui::LogViewerDialog) {
  m_ui->setupUi(this);
  ...
  for (LogEntry &log : logs) {
    LogViewerEntry *entry =
        new LogViewerEntry(m_ui->logContainer->widget(), std::move(log));
    m_ui->logContainer->widget()->layout()->addWidget(entry);
  }

  static_cast<QVBoxLayout *>(m_ui->logContainer->widget()->layout())
      ->addStretch(1);
}
...
void LogViewerDialog::updateLogVisibility() {
  for (int i = 0; i < m_ui->logContainer->widget()->layout()->count(); i++) {
    LogViewerEntry *entry = dynamic_cast<LogViewerEntry *>(
        m_ui->logContainer->widget()->layout()->itemAt(i)->widget());
    if (entry == nullptr)
      continue;

    entry->setTimestampVisible(m_ui->timestampCheckbox->isChecked());
    if (m_ui->sessionCheckbox->isChecked() &&
        entry->entry().timestamp < s_programStartTime) {
