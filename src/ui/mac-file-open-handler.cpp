...
bool MacFileOpenHandler::eventFilter(QObject *object, QEvent *event) {
  ...
  const QFileOpenEvent *fileOpenEvent =
      dynamic_cast<const QFileOpenEvent *>(event);
...
void MacFileOpenHandler::handleDuringStartup(
    const QFileOpenEvent &event) const {
...
void MacFileOpenHandler::handleAfterStartup(const QFileOpenEvent &event) const {
  ...
  for (QWidget *window : QApplication::topLevelWidgets()) {
    if (!window->isWindow())
      continue;
    if (dynamic_cast<const QMainWindow *>(window) == nullptr)
      return;
  ...
  if (mainWindow == nullptr || mainWindow->isHidden())
    return;
