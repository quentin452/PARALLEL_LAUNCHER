	...
	m_romSettingsWidget( new RomSettingsWidget( this ) )
{
	...
	connect( m_romSettingsWidget, &RomSettingsWidget::rspEmulationChanged, this, &MainWindow::rspEmulationChanged );
	connect( m_romSettingsWidget, &RomSettingsWidget::accurateDepthCompareChanged, this, &MainWindow::accurateDepthCompareChanged );
	connect( m_romSettingsWidget, &RomSettingsWidget::emulateFramebufferChanged, this, &MainWindow::emulateFramebufferChanged );
	connect( m_romSettingsWidget, &RomSettingsWidget::emulatorCoreChanged, this, &MainWindow::emulatorChanged );
	connect( m_romSettingsWidget, &RomSettingsWidget::inputModeChanged, this, &MainWindow::inputModeChanged );
	connect( m_romSettingsWidget, &RomSettingsWidget::mupenPluginChanged, this, &MainWindow::mupenPluginChanged );
	connect( m_romSettingsWidget, &RomSettingsWidget::overclockCpuChanged, this, &MainWindow::overclockCpuChanged );
	connect( m_romSettingsWidget, &RomSettingsWidget::overclockViChanged, this, &MainWindow::overclockViChanged );
	connect( m_romSettingsWidget, &RomSettingsWidget::parallelPluginChanged, this, &MainWindow::parallelPluginChanged );
	connect( m_romSettingsWidget, &RomSettingsWidget::showMorePluginsChanged, this, &MainWindow::showMorePluginsChanged );
	connect( m_romSettingsWidget, &RomSettingsWidget::upscaleTexRectsChanged, this, &MainWindow::upscaleTexRectsChanged );
	connect( m_romSettingsWidget, &RomSettingsWidget::removeBordersChanged, this, &MainWindow::removeBordersChanged );
	connect( m_romSettingsWidget, &RomSettingsWidget::widescreenChanged, this, &MainWindow::widescreenChanged );
	connect( m_romSettingsWidget, &RomSettingsWidget::bigEepromChanged, this, &MainWindow::bigEepromChanged );
	connect( m_romSettingsWidget, &RomSettingsWidget::useInterpreterChanged, this, &MainWindow::useInterpreterChanged );
	connect( m_romSettingsWidget, &RomSettingsWidget::saveSyncingChanged, this, &MainWindow::saveSyncingChanged );
	connect( m_romSettingsWidget, &RomSettingsWidget::activeSdCardChanged, this, &MainWindow::sdCardChanged );
...
margin-bottom: 2px;
	...
	connect( m_ui->romList, SIGNAL(selectionFilteredOut()), this, SLOT(romSelectionChanged()) );
	connect( m_ui->romList, SIGNAL(editSave(fs::path)), this, SLOT(editSave(fs::path)) );
	connect( m_ui->rhdcPage, SIGNAL(editSave(fs::path)), this, SLOT(editSave(fs::path)) );
	...
	connect( m_ui->rhdcPage, SIGNAL(romSelected()), this, SLOT(romSelectionChanged()) );
	...
	if( !FileController::loadAppSettings().knowsAboutRhdcView && !uiState.rhdcView ) {
...
void MainWindow::setView( int view, bool keepSize ) {
	const QSizePolicy preferred = QSizePolicy( QSizePolicy::Preferred, QSizePolicy::Preferred );
	const QSizePolicy ignored = QSizePolicy( QSizePolicy::Ignored, QSizePolicy::Ignored );
	...
	if( !keepSize && rhdcView != wasRhdcView ) {
		if( rhdcView ) {
			...
			FileController::saveUiState( uiState );
			newSize = QSize( uiState.classicViewSize.width, uiState.classicViewSize.height );
		}
	}
	...
	if( newSize.isValid() ) resize( newSize );
}

RomInfo *MainWindow::getSelectedRomInfo() {
	switch( m_ui->romView->currentIndex() ) {
		case VIEW_CLASSIC: return m_ui->romList->tryGetSelectedRom().info;
		case VIEW_RHDC: return m_ui->romList->tryGetRomInfo( m_ui->rhdcPage->getSelectedVersion() );
...
void MainWindow::romSelectionChanged() {
	if( m_ui->romView->currentIndex() == VIEW_RHDC ) {
		...
		RomInfo romInfo;
		if( sha1.empty() || !DataProvider::tryFetchRomByHash( sha1, false, &romInfo ) ) {
		...
		m_romSettingsWidget->setSettings(
			romInfo.inputModeId,
			romInfo.emulator,
			romInfo.parallelPlugin,
			romInfo.mupenPlugin,
			romInfo.overclockCPU,
			romInfo.overclockVI,
			romInfo.widescreen,
			romInfo.bigEEPROM,
			romInfo.useInterpreter,
			romInfo.parallelTexRectUpscaling,
			romInfo.parallelRemoveBorders,
			romInfo.glidenFramebufferEmulation,
			romInfo.glidenCorrectDepthCompare,
			romInfo.glidenParallelRsp,
		...
		m_ui->playSingleplayerButton->setVisible( !showDownloadButton && rom.file != nullptr );
		m_ui->playMultiplayerButton->setVisible( !showDownloadButton && rom.file != nullptr );
	}
}
...
void MainWindow::sdCardChanged( std::string sdCard ) {
	...
	if( sdCard.empty() && romInfo->sdCard == "?" ) return;
	if( sdCard == romInfo->sdCard ) return;
...
void MainWindow::play( bool multiplayer ) {
	...
	if( m_ui->romView->currentIndex() == VIEW_RHDC ) {
		if(
			!DataProvider::tryFetchRomByHash( m_ui->rhdcPage->getSelectedVersion(), false, &romInfo ) ||
			!DataProvider::tryFetchRomFileWithSha1( romInfo.sha1, m_ui->rhdcPage->getCurrentPlaylist(), &romFile )
		) return;
	} else {
...
void MainWindow::addRom() {
	fs::path romPath = FileDialog::getFile(
		tr( "Select a ROM" ).toStdString().c_str(),
		{{ tr( "ROM or Patch File" ).toStdString().c_str(), "*.z64;*.n64;*.v64;*.bps" }}
	);
	...
	static const std::set<string> s_patchExtensions = std::set<string>{
		".bps", ".BPS", ".Bps"
	};
	...
	if( DataProvider::tryFetchRomFile( romPath, nullptr ) ) {
		RomInfo romInfo;
		if( DataProvider::tryFetchRomByPath( romPath, true, &romInfo, nullptr ) && !romInfo.groups.empty() ) {
...
void MainWindow::updateRhdcActions() {
	...
	if( m_ui->romView->currentIndex() == VIEW_RHDC && (!integrationEnabled || !loggedIn) ) {
		setView( m_ui->romList->hasRoms() ? VIEW_CLASSIC : VIEW_NO_ROMS );
		if( palette().buttonText().color().valueF() > 0.5 ) {
			m_ui->rhdcViewToggleButton->setIcon( Icon::rhdcLight() );
		} else {
...
void MainWindow::download() {
	...
	if( m_ui->romView->currentIndex() == VIEW_RHDC ) {
		const string sha1 = m_ui->rhdcPage->getSelectedVersion();
		if(
			DataProvider::romWithSha1Exists( sha1 ) ||
			!m_ui->rhdcPage->tryGetHackIdAndDownloadUrl( hackId, downloadUrl )
		) return;
	} else {
		const RomReference rom = m_ui->romList->tryGetSelectedRom();
		if( rom.file == nullptr || rom.file->local || !rom.rhdc ) return;
...
void MainWindow::openSaveFileDirectory() const {
	FileExplorer::showFolder( BaseDir::data() / _NFS("retro-data") / _NFS("saves") );
}

void MainWindow::openSavestateDirectory() const {
	FileExplorer::showFolder( BaseDir::data() / _NFS("retro-data") / _NFS("states") );
}
