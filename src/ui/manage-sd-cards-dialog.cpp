...
ManageSdCardsDialog::ManageSdCardsDialog(QWidget *parent)
    : QDialog(parent), m_ui(new Ui::ManageSdCardsDialog) {
  m_ui->setupUi(this);
...
static void validateName(QString name, QLineEdit *input,
                         QPushButton *saveButton,
                         const std::vector<SdCardInfo> &cards) {
  ...
  if (uname == QString("CON") || uname == QString("PRN") ||
      uname == QString("AUX") || uname == QString("NUL") ||
      (uname.length() == 4 &&
...
void ManageSdCardsDialog::deleteCard() {
  if (QMessageBox::question(this, tr("Confirm Deletion"),
                            tr("Are you sure you want to delete this virtual "
                               "SD card? This action cannot be undone.")) !=
      QMessageBox::StandardButton::Yes) {
    return;
  }
...
void ManageSdCardsDialog::createAsync(QString name,
  ...
  QtThread::safeAsync([=]() {
    ...
    if (status != 0) {
      QMessageBox::critical(dialog, tr("Unexpected Error"),
                            tr("Failed to create the virtual SD card because "
                               "of an unknown error."));
...
void ManageSdCardsDialog::createCard() {
  ...
  try {
    ...
    QMessageBox::critical(this, tr("Unexpected Error"),
                          tr("Failed to create the virtual SD card because of "
                             "an unknown error."));
  }
...
void ManageSdCardsDialog::browseFiles() {
  ...
  if (card == nullptr) {
    QMessageBox::critical(this, tr("Operation Failed"),
                          tr("Failed to mount the SD card image. For more "
                             "detailed error information, go back to the main "
                             "window and press F7 to view error logs."),
                          QMessageBox::Ok);
    return;
  }
...
void ManageSdCardsDialog::cloneCard() {
  ...
  SdCardManager::copyCard(m_ui->nameInput->text().toStdString(),
...
bool ManageSdCardsDialog::shouldClose() {
  if (m_mountedCards.empty())
    return true;

  if (QMessageBox::information(this, tr("Confirm Close"),
                               tr("All SD cards you have opened for browsing "
