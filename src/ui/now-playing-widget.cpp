...
static inline QLabel *makeLayoutWarning(QWidget *parent) {
  QLabel *warning = new QLabel(
      NowPlayingWidget::tr("The hack author has not provided a star layout for "
                           "this hack. This layout will show unattainable "
                           "stars and may be missing some others."),
      parent);
...
NowPlayingWidget::NowPlayingWidget(QWidget *parent, const fs::path &romPath,
  ...
  m_ui->romNameLabel->setText(isRhdcHack ? hack.info.name.c_str()
                                         : romPath.stem().u8string().c_str());
  ...
  if (isRhdcHack || layoutOverride != nullptr) {
    ...
    if (rhdcSettings.enableStarDisplay) {
      ...
      if (layoutOverride ||
        ...
        m_starDisplay = new LiveStarDisplayWidget(
            this, RetroArch::getSaveFilePath(romPath), std::move(layout));
        ...
        QBoxLayout *centralLayout = qobject_cast<QBoxLayout *>(this->layout());
        int centralLayoutCount = centralLayout->count();
        centralLayout->insertWidget(centralLayoutCount - 1,
                                    makeLayoutWarning(this));
...
NowPlayingWidget::~NowPlayingWidget() { delete m_ui; }
...
void NowPlayingWidget::updateTimers() {
  ...
  m_ui->totalTimeLabel->setText(
      formatTime(sessionTime + m_pastPlayTime).c_str());
}
