...
bool ColourPalette::systemIsDarkMode() {
  ...
  const LSTATUS status = RegGetValueW(
      ...
      L"AppsUseLightTheme", RRF_RT_REG_DWORD, nullptr, (void *)&value, &size);
...
bool ColourPalette::systemIsDarkMode() {
  string output;
  return Process::tryGetOutput("defaults read -g AppleInterfaceStyle",
                               output) &&
         output.find("Dark") != string::npos;
}
#endif
...
bool ColourPalette::systemIsDarkMode() {
  return QApplication::palette()
             .color(QPalette::Active, QPalette::Base)
             .toHsv()
             .value() < 128;
}
#endif
