...
static bool playGame(const RomFile &romFile, const RomInfo &romInfo,
   ...
   * Strangely, just setting it now and then also again in several seconds
   * doesn't work consistently, but always waiting does work.
   */
  ...
  if (setDiscordStatus) {
    ...
    QTimer::singleShot(5000, nullptr, [=]() {
      if (*playingGame)
        Discord::setNowPlaying(romName, startTime);
    });
  }
#else
  if (setDiscordStatus)
    Discord::setNowPlaying(getRomName(romFile, romInfo));
#endif
