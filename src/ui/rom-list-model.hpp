...
private:
  ...
  HashMap<string, std::vector<__RomListModelPrivate::_RomListModelRow *>>
      m_reverseLookup;
  int m_sortBy;
...
public:
  RomListModel(QObject *parent)
      : QAbstractItemModel(parent), m_sortBy(COL_NAME),
  ...
  void fullUpdate();
  void rhdcUpdate();
  void partialUpdate(const RomInfo &changedRom, bool nameChanged,
  ...
  inline bool hasRoms() const noexcept { return !m_romFiles.empty(); }

  virtual int
  columnCount([[maybe_unused]] const QModelIndex &parent) const override {
    return __RomListModelPrivate::NUM_COLUMNS;
  }
  virtual QSize span(const QModelIndex &index) const override {
    return QSize(hasChildren(index) ? __RomListModelPrivate::NUM_COLUMNS : 0,
                 1);
  }
