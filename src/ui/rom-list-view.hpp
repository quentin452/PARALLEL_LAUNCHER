...
public:
  ...
  inline virtual QWidget *
  createEditor(QWidget *parent, const QStyleOptionViewItem &option,
               const QModelIndex &index) const override {
    return m_inner->createEditor(parent, option, index);
  }
  inline virtual void destroyEditor(QWidget *editor,
                                    const QModelIndex &index) const override {
    m_inner->destroyEditor(editor, index);
  }
  inline virtual bool editorEvent(QEvent *event, QAbstractItemModel *model,
                                  const QStyleOptionViewItem &option,
                                  const QModelIndex &index) override {
    return m_inner->editorEvent(event, model, option, index);
  }
  inline virtual bool helpEvent(QHelpEvent *event, QAbstractItemView *view,
                                const QStyleOptionViewItem &option,
                                const QModelIndex &index) override {
    return m_inner->helpEvent(event, view, option, index);
  }
  inline virtual void setEditorData(QWidget *editor,
                                    const QModelIndex &index) const override {
    m_inner->setEditorData(editor, index);
  }
  inline virtual void setModelData(QWidget *editor, QAbstractItemModel *model,
                                   const QModelIndex &index) const override {
...
public:
  ...
  void refetchAll();
  void refetchRhdc();
  void updateRomInfo(const RomInfo &romInfo, bool nameChanged,
