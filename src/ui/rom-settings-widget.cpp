	...
	m_supportsMouse( false )
{
	...
	m_ui->parallelPluginRadioGroup->setId( m_ui->pluginGLideN64Radio, (int)GfxPlugin::GLideN64 );
	m_ui->parallelPluginRadioGroup->setId( m_ui->pluginOgreRadio, (int)GfxPlugin::OGRE );
	...
	m_ui->parallelPluginRadioGroup->setId( m_ui->pluginGlnRadio, (int)GfxPlugin::Gln64 );
	...
	connect( m_ui->parallelPluginRadioGroup, SIGNAL(buttonToggled(int,bool)), this, SLOT(parallelPluginRadioChanged(int,bool)) );
	connect( m_ui->mupenPluginRadioGroup, SIGNAL(buttonToggled(int,bool)), this, SLOT(mupenPluginRadioChanged(int,bool)) );
...
static inline bool isSpecialInputType( N64InputType inputType ) {
	return(
		inputType == N64InputType::Gamecube ||
...
static inline bool hasSpecialInputType( const N64InputTypes &inputTypes ) {
	return(
		isSpecialInputType( inputTypes[0] ) ||
		isSpecialInputType( inputTypes[1] ) ||
		isSpecialInputType( inputTypes[2] ) ||
		isSpecialInputType( inputTypes[3] )
	);
}
...
void RomSettingsWidget::setSettings(
	...
	m_ui->trueWidescreenCheckbox->setVisible(
		(core == EmulatorCore::Mupen64plusNext && mupenPlugin    == GfxPlugin::GLideN64) ||
		(core == EmulatorCore::ParallelN64     && parallelPlugin == GfxPlugin::GLideN64) ||
		(core == EmulatorCore::ParallelN64     && parallelPlugin == GfxPlugin::OGRE)
	);

	m_ui->overclockCpuCheckbox->setText(
	...
	const bool showAdvanced = FileController::loadAppSettings().showAdvancedSettings;
	m_ui->overclockViCheckbox->setVisible( showAdvanced );
	m_ui->bigEepromCheckbox->setVisible( showAdvanced || (core == EmulatorCore::Mupen64plusNext) );
	...
	m_ui->trueWidescreenCheckbox->setChecked( widescreen == WidescreenMode::ViewportHack );
	...
	if( FileController::loadAppSettings().emulateSdCard || !sdCard.empty() ) {
		...
		for( int i = 0; i < m_ui->sdCardSelect->count(); i++ ) {
			if( sdCard == m_ui->sdCardSelect->itemData( i, Qt::UserRole ).toString().toStdString() ) {
	...
	m_supportsMouse = (
		inputTypes[0] == N64InputType::Mouse ||
...
std::string RomSettingsWidget::sdCard() const {
	...
	if( index <= 0 || index > (int)m_sdCards.size() ) return "";
...
void RomSettingsWidget::reloadHotkeyInfo() {
	if( m_supportsMouse ) {
		const KeyId hotkey = FileController::loadHotkeys().at( (int)Hotkey::GrabMouse );
		...
		m_ui->mouseHintLabel->setText( tr( "This ROM supports mouse input. After launching the emulator, your mouse will be captured by the emulator. Press %1 if you need to free the mouse cursor." ).arg( keyName ) );
	} else {
...
void RomSettingsWidget::updatePluginSettingsVisibility() {
	...
	const bool gliden64 = (
		(m_ui->emulatorTabs->currentIndex() == 1 && m_ui->pluginGlidenRadio2->isChecked()) ||
		(m_ui->emulatorTabs->currentIndex() == 0 && m_ui->pluginGLideN64Radio->isChecked())
	);

	const bool ogre = m_ui->emulatorTabs->currentIndex() == 0 && m_ui->pluginOgreRadio->isChecked();

	m_ui->emulateFramebufferCheckbox->setVisible( gliden64 );
	m_ui->accurateDepthCompareCheckbox->setVisible( gliden64 );
	m_ui->rspEmulationCheckbox->setVisible( gliden64 && m_ui->emulatorTabs->currentIndex() == 0 );
...
void RomSettingsWidget::widescreenToggled( bool checked ) {
	...
	emit widescreenChanged( checked ? (ubyte)WidescreenMode::Stretched : (ubyte)WidescreenMode::Off );
}
...
void RomSettingsWidget::trueWidescreenToggled( bool checked ) {
	...
	emit widescreenChanged( checked ? (ubyte)WidescreenMode::ViewportHack : (ubyte)WidescreenMode::Off );
}
...
void RomSettingsWidget::sdCardChanged( int index ) {
	if( m_suppressEvents || !m_ui->sdCardPanel->isVisible() ) return;
	if( index < 0 || index > (int)m_sdCards.size() ) return;
...
void RomSettingsWidget::manageSdCards() {
	...
	for( int i = 0; i < m_ui->sdCardSelect->count(); i++ ) {
		if( selectedCard == m_ui->sdCardSelect->itemData( i, Qt::UserRole ).toString().toStdString() ) {
			m_ui->sdCardSelect->setCurrentIndex( i );
			return;
		}
	}
...
void RomSettingsWidget::updatePluginLangterms( GfxPlugin recommendedPlugin ) {
	const QString recommendedText = QString( " " ) + tr( "(recommended by hack author)" );
	switch( recommendedPlugin ) {
		...
		case GfxPlugin::UseDefault:
			m_ui->pluginParallelRadio->setText( tr( "ParaLLEl (Recommended - very accurate to console)" ) );
			m_ui->pluginGLideN64Radio->setText( tr( "GLideN64 (Recommended for lower end computers)" ) );
	...
	const QString angrylionText = QString( "Angrylion" ) + ((recommendedPlugin == GfxPlugin::Angrylion) ? recommendedText : QString());
	m_ui->pluginAngrylionRadio->setText( angrylionText );
	m_ui->pluginAngrylionRadio2->setText( angrylionText );

	m_ui->pluginRiceRadio->setText( QString( "Rice" ) + ((recommendedPlugin == GfxPlugin::Rice) ? recommendedText : QString() ));
	m_ui->pluginGlidenRadio2->setText( QString( "GLideN64" ) + ((recommendedPlugin == GfxPlugin::GLideN64) ? recommendedText : QString() ));
	m_ui->pluginParallelRadio2->setText( QString( "ParaLLEl" ) + ((recommendedPlugin == GfxPlugin::ParaLLEl) ? recommendedText : QString() ));
}

void RomSettingsWidget::fixTabWidgetSize() {
	m_ui->pluginGroup->setSizePolicy( QSizePolicy::Preferred, m_ui->emulatorTabs->currentIndex() == 0 ? QSizePolicy::Preferred : QSizePolicy::Ignored );
	m_ui->pluginGroup->resize( m_ui->pluginGroup->minimumSizeHint() );
	m_ui->pluginGroup->adjustSize();

	m_ui->mupenPluginGroup->setSizePolicy( QSizePolicy::Preferred, m_ui->emulatorTabs->currentIndex() != 0 ? QSizePolicy::Preferred : QSizePolicy::Ignored );
...
RomSettingsWidget *RomSettingsWidgetSlot::widget() const {
	if( layout()->children().isEmpty() ) return nullptr;
	return layout()->children().isEmpty() ? nullptr : dynamic_cast<RomSettingsWidget*>( layout()->children().first() );
}
