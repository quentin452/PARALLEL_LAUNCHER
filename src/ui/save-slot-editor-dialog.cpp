...
static const char *REDS =
    QT_TRANSLATE_NOOP("SaveSlotEditorDialog", "8 Red Coins");
static const char *COINS =
    QT_TRANSLATE_NOOP("SaveSlotEditorDialog", "100 Coins");
...
static const char *s_starNames[26][7] = {
    ...
    {QT_TRANSLATE_NOOP("SaveSlotEditorDialog", "Toad 1"),
     ...
     QT_TRANSLATE_NOOP("SaveSlotEditorDialog", "MIPS 2"), nullptr, nullptr},
    ...
    {QT_TRANSLATE_NOOP("SaveSlotEditorDialog", "Big Bob-Omb on the Summit"),
     ...
     QT_TRANSLATE_NOOP("SaveSlotEditorDialog",
    ...
    {QT_TRANSLATE_NOOP("SaveSlotEditorDialog", "Chip Off Whomp's Block"),
     ...
     QT_TRANSLATE_NOOP("SaveSlotEditorDialog", "Blast Away the Wall"), COINS},
    /* Jolly Rodger Bay */
    ...
    {QT_TRANSLATE_NOOP("SaveSlotEditorDialog", "Slip Slidin' Away"),
     ...
     QT_TRANSLATE_NOOP("SaveSlotEditorDialog", "Wall Kicks Will Work"), COINS},
    /* Big Boo's Haunt */
    ...
    {QT_TRANSLATE_NOOP("SaveSlotEditorDialog", "In the Talons of the Big Bird"),
     ...
     QT_TRANSLATE_NOOP("SaveSlotEditorDialog", "Pyramid Puzzle"), COINS},
    /* Dire Dire Docks */
    {QT_TRANSLATE_NOOP("SaveSlotEditorDialog", "Board Bowser's Sub"),
     ...
     QT_TRANSLATE_NOOP("SaveSlotEditorDialog", "Collect the Caps..."), COINS},
    /* Snowman's Land */
    {QT_TRANSLATE_NOOP("SaveSlotEditorDialog", "Snowman's Big Head"),
     ...
     QT_TRANSLATE_NOOP("SaveSlotEditorDialog", "Into the Igloo"), COINS},
    /* Wet Dry World */
    ...
    {QT_TRANSLATE_NOOP("SaveSlotEditorDialog", "Pluck the Piranha Flower"),
     ...
     QT_TRANSLATE_NOOP("SaveSlotEditorDialog", "Make Wiggler Squirm"), COINS},
    /* Tick Tock Clock */
...
const bool s_vanillaCannons[26] = {false,
...
inline static void addLevelEntry(QComboBox *select, const char *name,
                                 SM64::LevelId levelId) {
  select->addItem(QCoreApplication::translate("SaveSlotEditorDialog", name),
                  QVariant::fromValue<uint>((uint)levelId));
}

SaveSlotEditorDialog::SaveSlotEditorDialog(const fs::path &saveFilePath,
                                           int slot)
    : QDialog(nullptr), m_ui(new Ui::SaveSlotEditorDialog),
      m_filePath(saveFilePath), m_slot(slot) {
  m_ui->setupUi(this);
  ...
  addLevelEntry(m_ui->capLevel, "Inside the Castle",
                SM64::LevelId::CastleInterior);
  addLevelEntry(m_ui->capLevel, "Castle Courtyard",
                SM64::LevelId::CastleCourtyard);
  for (int i = 0; i < 25; i++) {
  ...
  for (int i = 0; i < m_ui->capLevel->count(); i++) {
    if (m_ui->capLevel->itemData(i, Qt::UserRole).value<uint>() ==
        (uint)saveSlot.capLevel) {
  ...
  for (QAbstractButton *radio : m_ui->flagButtons->buttons()) {
    const SM64::SaveFileFlag flag =
        (SM64::SaveFileFlag)radio->property("saveFlag").value<uint>();
    radio->setChecked(Flags::has(saveSlot.flags, flag));
  }

  for (int i = 0; i < 9; i++) {
    m_ui->gridLayout->addWidget(new QLabel(tr(s_columnHeaders[i])), 0, i + 1,
                                Qt::AlignCenter);
  }

  for (int i = 0; i < 26; i++) {
    m_ui->gridLayout->addWidget(new QLabel(tr(s_courseNames[i])), i + 1, 0,
...
void SaveSlotEditorDialog::accept() {
  ...
  slot.capLevel =
      (LevelId)m_ui->capLevel->currentData(Qt::UserRole).value<uint>();
