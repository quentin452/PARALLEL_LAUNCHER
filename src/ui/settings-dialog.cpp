	...
	m_ui( new Ui::SettingsDialog )
{
	...
	m_ui->languageSelect->addItem( tr( "Automatic (%1)" ).arg( userLocale ), QVariant::fromValue<QString>( "" ) );
	m_ui->languageSelect->addItem( "English (Canadian)", QVariant::fromValue<QString>( "en_CA" ) );
	m_ui->languageSelect->addItem( "English (American)", QVariant::fromValue<QString>( "en_US" ) );
	m_ui->languageSelect->addItem( "English (British)", QVariant::fromValue<QString>( "en_GB" ) );
	...
	m_ui->languageSelect->addItem( "Français (Québécois)", QVariant::fromValue<QString>( "fr_CA" ) );
...
void SettingsDialog::showEvent( QShowEvent *event ) {
	...
	switch( settings.defaultParallelPlugin ) {
		case GfxPlugin::Glide64: m_ui->parallelPluginSelect->setCurrentIndex( 1 ); break;
		case GfxPlugin::Angrylion: m_ui->parallelPluginSelect->setCurrentIndex( 2 ); break;
		case GfxPlugin::Rice: m_ui->parallelPluginSelect->setCurrentIndex( 3 ); break;
		case GfxPlugin::Gln64: m_ui->parallelPluginSelect->setCurrentIndex( 4 ); break;
		case GfxPlugin::GLideN64: m_ui->parallelPluginSelect->setCurrentIndex( 5 ); break;
		case GfxPlugin::OGRE: m_ui->parallelPluginSelect->setCurrentIndex( 6 ); break;
		default: m_ui->parallelPluginSelect->setCurrentIndex( 0 ); break;
	}
	...
	m_ui->hideNotificationsCheckbox->setChecked( settings.hideRetroArchNotifications );
	...
	for( int i = 0; i < m_ui->languageSelect->count(); i++ ) {
		if( settings.locale == m_ui->languageSelect->itemData( i, Qt::UserRole ).toString().toStdString() ) {
	...
	m_ui->historySizeSpinner->setValue( settings.isViewerHistorySize ? settings.isViewerHistorySize : 1050 );
...
void SettingsDialog::save() {
...
#endif
	settings.locale = m_ui->languageSelect->currentData( Qt::UserRole ).toString().toStdString();
	...
	settings.defaultParallelPlugin = m_parallelPlugins[ m_ui->parallelPluginSelect->currentIndex() ];
	...
	settings.hideRetroArchNotifications = m_ui->hideNotificationsCheckbox->isChecked();
	...
	if( settings.discordIntegration && !Discord::pluginInstalled() ) {
		if( QMessageBox::question( this, tr( "Install Discord Plugin?" ), tr( "You have enabled Discord integration, but the Discord plugin is not currently installed. Would you like to install it now?" ) ) == QMessageBox::Yes ) {
			fs::error_code err;
			...
			if( !result.success ) {
				QMessageBox::critical( this, tr( "Download Failed" ), tr( "Failed to download Discord plugin" ).append( "\n" ).append( result.errorMessage.c_str() ) );
				settings.discordIntegration = false;
			}
		} else {
			settings.discordIntegration = false;
	...
	settings.isViewerHistorySize = (m_ui->historySizeSpinner->value() <= 1000) ? m_ui->historySizeSpinner->value() : 0;

	settings.rtcOffset = m_ui->manualTimeRadio->isChecked() ? m_ui->rtcInput->offset() : 0;
...
void SettingsDialog::windowScaleChanged( int index ) {
	...
	const string autoRes = " (x" + Number::toString( autoScale ) + " - " + Number::toString( autoWidth ) + 'x' + Number::toString( autoHeight ) + ')';
	m_ui->upscalingSelect->setItemText( 0, tr( "Auto" ) + QString( autoRes.c_str() ) );
}

void SettingsDialog::browseForBpsPath() {
	fs::path folderPath = FileDialog::getDirectory( tr( "Select a Folder" ).toStdString().c_str() );
	if( !folderPath.empty() ) {
...
void SettingsDialog::resetConfig() {
	if( QMessageBox::question(
		this,
		tr( "Confirm Reset" ),
		tr( "This will reset your RetroArch config file, undoing any changes you have made within RetroArch. Your Parallel Launcher settings will not be affected. Do you want to continue?" )
	) == QMessageBox::Yes ) {
		if( RetroArch::resetConfig() ) {
			const fs::path systemDir = BaseDir::data() / _NFS("retro-data") / _NFS("system");
			fs::error_code err;
			fs::remove_all( systemDir / _NFS("ParallelN64") / _NFS("shaders"), err );
			fs::remove_all( systemDir / _NFS("Mupen64plus") / _NFS("shaders"), err );
			QMessageBox::information( this, tr( "Config Reset" ), tr( "Your RetroArch config has been reset." ) );
		} else {
			QMessageBox::critical( this, tr( "Oops" ), tr( "An unknown error occurred. Your RetroArch config has not been reset." ) );
		}
	}
}
...
void SettingsDialog::applyAll_accurateDepthCompare() {
	if( QMessageBox::question( this, tr( "Confirm Apply All" ), tr( "Apply this setting to all current roms?" ) ) == QMessageBox::Yes ) {
...
void SettingsDialog::applyAll_removeBorders() {
	if( QMessageBox::question( this, tr( "Confirm Apply All" ), tr( "Apply this setting to all current roms?" ) ) == QMessageBox::Yes ) {
		DataProvider::setDefaultParallelRemoveBorders( m_ui->removeBordersCheckbox->isChecked() );
	}
}
...
void SettingsDialog::rtcSyncToggled( bool syncToClock ) {
	...
	if( syncToClock ) m_ui->rtcInput->setOffset( 0 );
}
