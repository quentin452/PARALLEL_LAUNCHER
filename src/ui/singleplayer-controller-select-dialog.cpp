...
SingleplayerControllerSelectDialog::SingleplayerControllerSelectDialog()
    : QDialog(nullptr), m_ui(new Ui::SingleplayerControllerSelectDialog),
  ...
  connect(&GamepadController::instance(),
          &GamepadController::gamepadButtonPressed, this,
          &SingleplayerControllerSelectDialog::buttonPressed);
}
...
void SingleplayerControllerSelectDialog::buttonPressed(
    GamepadButtonEvent event) {
  if (!event.isPressed)
    return;
  for (const ConnectedGamepad &gamepad :
