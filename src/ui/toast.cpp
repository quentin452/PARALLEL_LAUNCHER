...
static void toastLoop() {
  if (s_toast == nullptr || s_window == nullptr ||
      s_toast->parent() == nullptr || !s_window->isVisible())
    return;
  ...
  const bool underMouse = s_toast->geometry().contains(
      QCursor::pos() - s_window->geometry().topLeft());

  s_toast->graphicsEffect()->setEnabled(underMouse);
  s_toast->setStyleSheet(underMouse ? s_cssFade : s_cssNormal);
  s_toast->setGeometry(20, s_window->height() - 20 - TOAST_HEIGHT,
                       s_window->width() - 40, TOAST_HEIGHT);
}
...
static inline QColor getColour(ToastType type) {
  ...
  case ToastType::Error:
    return QColor(0xC80000);
  case ToastType::Warn:
    return QColor(0xE8AF00);
  case ToastType::Info:
    return QColor(0x0000C8);
  default:
    return QColor(0x00C800);
  }
}
