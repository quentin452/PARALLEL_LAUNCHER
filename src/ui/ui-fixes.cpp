...
static constexpr QMessageBox::StandardButtons s_positiveButtons =
    QMessageBox::Ok | QMessageBox::Apply | QMessageBox::Yes |
    QMessageBox::YesToAll;

static constexpr QMessageBox::StandardButtons s_negativeButtons =
    QMessageBox::Cancel | QMessageBox::Abort | QMessageBox::No |
    QMessageBox::NoToAll | QMessageBox::Close;
...
bool WindowsIconFixer::eventFilter(QObject *object, QEvent *event) {
  ...
  if (dialog != nullptr) {
    for (QAbstractButton *dialogButton : dialog->buttons()) {
      const QMessageBox::StandardButtons buttonType =
          dialog->standardButton(dialogButton);
  ...
  if (buttonTray != nullptr) {
    for (QAbstractButton *dialogButton : buttonTray->buttons()) {
      const QDialogButtonBox::StandardButton buttonType =
          buttonTray->standardButton(dialogButton);
      switch (buttonType) {
...
static inline void applyWindowsFixes(QApplication &app) {
#ifdef __APPLE__
  fs::directory_iterator pluginDir(BaseDir::program().parent_path() /
                                   "PlugIns" / "styles");
#else
  ...
  for (const auto &plugin : pluginDir) {
    ...
    if (!pluginLoader->load()) {
      std::cerr << "Failed to load style plugin "
                << plugin.path().filename().u8string() << std::endl
                << std::flush;
  ...
  const int dpi = std::max({BASE_DPI, QApplication::desktop()->logicalDpiX(),
  ...
  cssTweaks += "QPushButton { qproperty-iconSize: "s + defaultIconSize + ' ' +
               defaultIconSize + "; }\n";
  cssTweaks +=
      string("QMainWindow#MainWindow QPushButton#refreshButton, "
  ...
  const fs::path fontDir =
      BaseDir::program().parent_path() / "Resources" / "fonts";
  const int fontId = QFontDatabase::addApplicationFont(
      (fontDir / "NotoSans-Regular.ttf").c_str());
  ...
  QFontDatabase::addApplicationFont(
      (fontDir / "NotoSans-BoldItalic.ttf").c_str());
  QFontDatabase::addApplicationFont((fontDir / "NotoMono-Regular.ttf").c_str());
  QFont defaultFont =
      (fontId >= 0)
          ? QFont(QFontDatabase::applicationFontFamilies(fontId).at(0))
          : QFont("Noto Sans");
#else
...
bool RtlLabelFixer::eventFilter(QObject *object, QEvent *event) {
  ...
  if (label != nullptr && label->layoutDirection() == Qt::RightToLeft) {
    ...
    case Qt::AlignJustify:
      label->setAlignment((label->alignment() & Qt::AlignVertical_Mask) |
                          Qt::AlignRight);
      break;
    default:
      break;
