...
template <>
void JsonSerializer::serialize<UiState>(JsonWriter &jw, const UiState &obj) {
  jw.writeObjectStart();
...
template <> UiState JsonSerializer::parse<UiState>(const Json &json) {
  UiState state = {
      {fs::to_path(json[P_SELECTED_ROM].getOrDefault<string>("")),
       ...
       std::vector<string>()},
      {json[P_RHDC_SORT_BY].getOrDefault<RhdcSorting>(
           UiState::Default.rhdcList.sortBy),
       json[P_RHDC_ASC].getOrDefault<bool>(UiState::Default.rhdcList.sortAsc),
       json[P_RHDC_PLAYLIST].getOrDefault<string>(
           UiState::Default.rhdcList.playlist),
       json[P_RHDC_HACK].getOrDefault<string>(
           UiState::Default.rhdcList.hackId)},
      {json[P_CLASSIC_WINDOW][P_WIDTH].getOrDefault<int>(
           UiState::Default.classicViewSize.width),
       json[P_CLASSIC_WINDOW][P_HEIGHT].getOrDefault<int>(
           UiState::Default.classicViewSize.height)},
      {json[P_RHDC_WINDOW][P_WIDTH].getOrDefault<int>(
           UiState::Default.rhdcViewSize.width),
       json[P_RHDC_WINDOW][P_HEIGHT].getOrDefault<int>(
           UiState::Default.rhdcViewSize.height)},
      json[P_SHOW_ALL_PLUGINS].getOrDefault<bool>(
          UiState::Default.showAllPlugins),
  ...
  // Legacy format
  if (json[P_WINDOW_WIDTH].exists() && json[P_WINDOW_HEIGHT].exists() &&
      !json[P_CLASSIC_WINDOW].exists()) {
    state.classicViewSize.width = state.rhdcViewSize.width =
        json[P_WINDOW_WIDTH].get<int>();
    state.classicViewSize.height = state.rhdcViewSize.height =
        json[P_WINDOW_HEIGHT].get<int>();
  }
