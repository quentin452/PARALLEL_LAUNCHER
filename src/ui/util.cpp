...
void UiUtil::shrinkToFit(QDialog *dialog,
...
void UiUtil::setIconSize(QAbstractButton *button, int baseIconSize) {
  static const int dpi =
      std::max({BASE_DPI, QApplication::desktop()->logicalDpiX(),
...
void UiUtil::scaleWindow(QWidget *window) {
  const double scale =
      (double)QApplication::desktop()->logicalDpiY() / (double)BASE_DPI;
  if (scale > 1.0)
    window->resize(window->size() * scale);
}
