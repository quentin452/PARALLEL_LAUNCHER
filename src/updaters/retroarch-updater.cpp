...
enum VersionMarch { Patch = 0, Minor = 1, Major = 2 };

static inline string buildDownloadUrl(const Version &version) {
#ifdef __linux__
  return "https://buildbot.libretro.com/stable/"s + version.toString() +
...
static inline Version marchVersion(Version version, VersionMarch versionMarch) {
  switch (versionMarch) {
  case VersionMarch::Patch:
    version.patch++;
    break;
  case VersionMarch::Minor:
    version.minor++;
    version.patch = 0;
    break;
  case VersionMarch::Major:
    version.major++;
    version.minor = 0;
    version.patch = 0;
    break;
...
static void getDownloadHelper(QNetworkAccessManager *web,
                              Version lastFoundVersion, Version earliestVersion,
  ...
  headAsync(web, url, [=](bool found) {
    if (found) {
      getDownloadHelper(web, earliestVersion,
                        marchVersion(earliestVersion, VersionMarch::Patch),
                        VersionMarch::Patch, cancelled, callback);
    } else if (versionMarch == VersionMarch::Major) {
      callback(lastFoundVersion);
    } else {
      const VersionMarch newVersionMarch =
          (VersionMarch)((int)versionMarch + 1);
      getDownloadHelper(web, lastFoundVersion,
                        marchVersion(earliestVersion, newVersionMarch),
                        newVersionMarch, cancelled, callback);
    }
  });
}

static inline Version getLatestVersion(const Version &earliestVersion) {
  ...
  getDownloadHelper(web, earliestVersion, earliestVersion, VersionMarch::Patch,
                    cancelled,
                    [cancelled, &latestVersion, &dialog](Version foundVersion) {
                      if (*cancelled)
                        return;
...
static void installWorker(QProgressDialog *dialog,
  ...
  AsyncProcess extraction(
      ...
      "7zr",
#endif
      {"x", (BaseDir::temp() / _NFS("retroarch.7z")).u8string(), "-t7z", "-aoa",
       "-y", "-o"s + installDir.u8string()});
  const uint exitCode = extraction.join();

  if (exitCode == 0) {
    if (fs::isDirectorySafe(installDir / RA_ROOT)) {
      const fs::path tempDir =
          installDir.parent_path() / _NFS("retroarch-temp");
      ...
      const fs::path tempData = installDir /
...
static inline bool downloadAndInstall(const Version &version) {
  ...
  DownloadResult status = DownloadDialog::download(
      ...
      buildDownloadUrl(version), tempBundlePath);
